  ...
  class Plugin::Beholder < Msf::Plugin
    ...
    class BeholderWorker
      ...
      def initialize(framework, config, driver)
        ...
        self.thread    = framework.threads.spawn('BeholderWorker', false) {
          begin
            ...
            $stderr.puts "BeholderWorker: #{e.class} #{e} #{e.backtrace}"
          end
      ...
      def verify_migration(sid)
        ...
        if this_ps && this_ps['name'].to_s.casecmp('explorer.exe').zero?
        ...
        target_ps = ps.select { |x| x['name'].to_s.casecmp('explorer.exe').zero? && x['user'].to_s != '' }.first
        unless target_ps
