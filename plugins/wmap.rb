  ...
  class Plugin::Wmap < Msf::Plugin
    class WmapCommandDispatcher
        ...
        while (arg = args.shift)
          ...
          when '-s'
            ...
            if u.include? 'http'
              ...
              self.targets = Hash.new unless targets
      ...
      def cmd_wmap_nodes(*args)
        self.rpcarr = Hash.new unless rpcarr
      ...
      def cmd_wmap_run(*args)
        ...
        trap("INT") do
          ...
          self.masstop = true
          rpc_kill_node('ALL', 'ALL') if killwhenstop
        end
        ...
        while (arg = args.shift)
              ...
              begin
                File.open(profile).each do |str|
                  unless str.include? '#'
                    # Not a comment
                    modname = str.strip
                    eprofile << modname unless modname.empty?
                  end
                  using_p = true
                end
              rescue
          ...
          when '-m'
            ...
            print_status("Using module #{mname}.") if mname
            using_m = true
          when '-p'
            mode |= wmap_expl
            ...
            print_status("Using initial path #{inipathname}.") if inipathname
            usinginipath = true
        ...
        execmod = true
        execmod = false if mode & wmap_show != 0
        ...
        targets.each_with_index do |t, idx|
          ...
          wmapmodules.each do |w|
            case w[2]
            when :wmap_server
              matches1[w] = true if run_wmap_server
            when :wmap_query
              matches2[w] = true if run_wmap_query
            when :wmap_unique_query
              matches5[w] = true if run_wmap_unique_query
            when :wmap_generic
              matches10[w] = true if run_wmap_generic
            when :wmap_dir, :wmap_file
              matches[w] = true if run_wmap_dir_file
            when :wmap_ssl
              matches3[w] = true if run_wmap_ssl
            end
          end
                ...
                test_tree.each do |node|
                  ...
                  when :wmap_file
                    if node.is_leaf? && !node.is_root?
                      ...
                      #
                      excludefilestr = if framework.datastore['WMAP_EXCLUDE']
                                         framework.datastore['WMAP_EXCLUDE']
                                       else
                                         wmap_exclude_files
                                       end

                      unless strpath.match(excludefilestr)
                        if !usinginipath || (usinginipath && strpath.match(inipathname))
                  ...
                  when :wmap_dir
                    if (node.is_leaf? && (!strpath.include? ".")) || node.is_root? || !node.is_leaf?
                      if !usinginipath || (usinginipath && strpath.match(inipathname))
            ...
            begin
              ...
              if mode & wmap_expl != 0
                ...
                w.web_forms.each do |form|
                  ...
                  next unless utest_query.key?(signature(form.path, datastr)) == false

                  modopts['METHOD'] = form.method.upcase
                  modopts['PATH'] =  form.path
                  modopts['QUERY'] = form.query
                  if form.method.casecmp('GET').zero?
                    modopts['QUERY'] = datastr
                    modopts['DATA'] = ""
                  end
                  modopts['DATA'] = datastr if form.method.casecmp('POST').zero?
                  modopts['TYPES'] = typestr

                  #
                  # TODO: Add headers, etc.
                  #
                  next unless !usinginipath || (usinginipath && form.path.match(inipathname))
            ...
            begin
              ...
              if mode & wmap_expl != 0
                ...
                w.web_forms.each do |req|
                  ...
                  if req.method.casecmp('GET').zero?
                    modopts['QUERY'] = datastr
                    modopts['DATA'] = ""
                  end
                  modopts['DATA'] = datastr if req.method.casecmp('POST').zero?
                  modopts['TYPES'] = typestr
      ...
      def add_web_site(url)
        ...
        site = framework.db.report_web_site(wait: true, host: uri.host, port: uri.port, vhost: vhost, ssl: ssl)
        ...
        target_whitelist.each do |ent|
          ...
          sites.each do |site|
            ...
            if !targets[ckey]
              ...
              targets[ckey].merge!(vhost: site.vhost,
                                   host: host.address,
                                   port: serv.port,
                                   ssl: (serv.name == "https"),
                                   path: inipath)
      ...
      def print_tree(tree, ip, maxlevel, limitlevel)
        initab = " " * 4
        indent = 6
        if !tree.nil? && (tree.depth <= maxlevel)
          print initab + (" " * indent * tree.depth)
          print "|" + ("-" * (indent - 1)) + "/" if tree.depth > 0
          if tree.depth >= 0
            if tree.depth == 0
              print "[#{tree.name}] (#{ip})\n" + initab + (" " * indent) + "\n"
      ...
      def local_module_exec(mod, mtype, opts, _nmaxjobs)
        ...
        case mtype
        when 'auxiliary'
          Msf::Simple::Auxiliary.run_simple(modinst, 'Action' => opts['ACTION'],
        ...
        when 'exploit'
          ...
          sess = Msf::Simple::Exploit.exploit_simple(modinst, 'Payload' => opts['PAYLOAD'],
                                                              'Target' => opts['TARGET'],
                                                              'LocalOutput' => driver.output,
                                                              'RunAsJob' => jobify,
                                                              'Options'  => opts)
      ...
      def rpc_round_exec(mod, mtype, opts, nmaxjobs)
        ...
        local_module_exec(mod, mtype, opts, nmaxjobs) if runlocal && !masstop
      end
      ...
      def rpc_kill_node(i, j)
        ...
        rpcarr.each do |_k, rpccon|
          if (idx == i.to_i) || i.casecmp('ALL').zero?
            # begin
            if !rpccon
              ...
              n.each do |id, name|
                if (j == id.to_s) || j.casecmp('ALL').zero?
      ...
      def rpc_mon_nodes
        ...
        loop do
          ...
          rpcarr.each do |k, rpccon|
            ...
            # begin
            @stdio = Rex::Ui::Text::Output::Stdio.new unless @stdio
      ...
      def view_modules
        ...
        load_wmap_modules(true) unless wmapmodules
