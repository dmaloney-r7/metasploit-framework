  ...
  class Plugin::Wiki < Msf::Plugin
    ...
    class WikiCommandDispatcher
      ...
      def wiki(wiki_type, *args)
        ...
        tbl_opts[:namespace] = case wiki_type
                               when "dokuwiki"
                                 'notes:targets:hosts:'
                               else
                                 ''
      ...
      def creds_to_table(opts = {})
        ...
        framework.db.creds.each do |cred|
          unless opts[:hosts].nil? || opts[:hosts].empty?
      ...
      def loot_to_table(opts = {})
        ...
        framework.db.loots.each do |loot|
          ...
          unless opts[:ports].nil? || opts[:ports].empty?
            next if loot.service.nil? || loot.service.port.nil? || (!opts[:ports].include? loot.service.port)
          end
          if loot.service
        ...
        def to_mediawiki
          ...
          unless rows.count.eql? 0
            rows.each do |row|
              ...
              r = row.join("|| ")
              r.each_char do |c|
                str << if bad.include? c
                         Rex::Text.html_encode(c)
                       else
                         c
                       end
              end
