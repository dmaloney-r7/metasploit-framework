...
$LOAD_PATH.unshift(File.join(File.dirname(__FILE__), '..', '..', '..', 'lib'))
$LOAD_PATH.unshift(File.join(File.dirname(__FILE__), '..', '..', '..', 'test', 'lib'))
...
module MsfTest
  ...
  describe "Msfconsole" do
    ...
    FileUtils.rm_rf(@temp_directory) if File.directory? @temp_directory
    ...
    def start_console_and_run_rc(name, rc_file, database_file = false)
      ...
      msfconsole_string = if database_file
                            "ruby #{@working_directory}/../../../msfconsole -o #{output_file} -r #{rc_file} -y #{database_file}"
                          else
                            "ruby #{@working_directory}/../../../msfconsole -o #{output_file} -r #{rc_file}"
                          end

      system(msfconsole_string.to_s)

      data = hlp_file_to_string(output_file.to_s)
    end
    ...
    def generate_win64_meterpreter_session(_input, _output)
      raise "Not Implemented"
   end

    def generate_java_meterpreter_session(_input, _output)
      raise "Not Implemented"
   end

    def generate_php_meterpreter_session(_input, _output)
      raise "Not Implemented"
  end
