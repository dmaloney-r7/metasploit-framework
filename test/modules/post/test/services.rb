...
class MetasploitModule < Msf::Post
  ...
  def test_info
    it "should return info on a given service  #{datastore['QSERVICE']}" do
      ...
      ret &&= results.is_a? Hash
      if ret
        ret &&= results.key? :display
        ret &&= (results[:display] == "Windows Management Instrumentation")
        ret &&= results.key? :starttype
        ret &&= results.key? :path
        ret &&= results.key? :startname
      end
