...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    syslogins = if vernum.join != "2000"
                  mssql_query("select loginname from master.sys.syslogins")[:rows]
                else
                  mssql_query("select loginname from master..syslogins")[:rows]
                end
    if !syslogins.nil?
    ...
    sysadmins = if vernum.join != "2000"
                  mssql_query("select name from master.sys.syslogins where sysadmin = 1")[:rows]
                else
                  mssql_query("select name from master..syslogins where sysadmin = 1")[:rows]
                end
    if !sysadmins.nil?
    ...
    winusers = if vernum.join != "2000"
                 mssql_query("select name from master.sys.syslogins where isntuser = 1")[:rows]
               else
                 mssql_query("select name from master..syslogins where isntuser = 1")[:rows]
               end
    ...
    wingroups = if vernum.join != "2000"
                  mssql_query("select name from master.sys.syslogins where isntgroup = 1")[:rows]
                else
                  mssql_query("select name from master..syslogins where isntgroup = 1")[:rows]
                end
    ...
    sameasuser = []
    sameasuser = if vernum.join != "2000"
                   mssql_query("SELECT name FROM sys.sql_logins WHERE PWDCOMPARE\(name, password_hash\) = 1")[:rows]
                 else
                   mssql_query("SELECT name FROM master.dbo.syslogins WHERE PWDCOMPARE\(name, password\) = 1")[:rows]
                 end
    ...
    blankpass = []
    blankpass = if vernum.join != "2000"
                  mssql_query("SELECT name FROM sys.sql_logins WHERE PWDCOMPARE\(\'\', password_hash\) = 1")[:rows]
                else
                  mssql_query("SELECT name FROM master.dbo.syslogins WHERE password IS NULL AND isntname = 0")[:rows]
                end
