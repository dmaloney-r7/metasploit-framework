...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    if res.headers['Server'] =~ /2wire Gateway/i
    ...
    if res.body =~ /<td class="data">(\w{4}-\w{4}-\w{4}-\w{4}-\w{4})<\/td>/i
    ...
    if res && (res.code == 200) && res.body.match(/<title>System Setup - Password<\/title>/i)
