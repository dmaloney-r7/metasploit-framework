...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    unless check_oracle_version(oracle_ver)
    ...
    begin
      ...
      sid_data.each do |sid|
        ...
        return if guess && datastore['STOP_ON_SUCCESS']
      end
  ...
  def parse_isqlplus_response(res, sid)
    ...
    if res.nil?
      ...
      if res.body =~ /ORA-01017:/ || res.body =~ /ORA-28273:/
        ...
        guess = true
      elsif res.body =~ /(ORA-12170):/ || res.body =~ /(ORA-12154):/ || res.body =~ /(ORA-12162):/
        vprint_status("#{msg} Incorrect SID: '#{sid.strip}' (got error code #{Regexp.last_match(1)})")
  ...
  def sid_data
    if datastore['SID'] && !datastore['SID'].empty?
      [datastore['SID']]
    elsif sid_file && ::File.readable?(sid_file)
