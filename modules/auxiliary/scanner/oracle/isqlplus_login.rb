...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    unless check_oracle_version(ver)
  ...
  def do_login(user = 'DBSNMP', pass = 'DBSNMP', version = 9.0)
    ...
    begin
      ...
      if res.code == 200
        ...
        if res.body =~ /Connected as/ || res.body =~ /Angemeldet als/ || res.body =~ /Arbejdssk/
          success = true
        elsif res.body =~ /ORA-01017:/ || res.body =~ /ORA-28273:/
          ...
          success = false
        elsif res.body =~ /ORA-12170:/ || res.body =~ /ORA-12154:/ || res.body =~ /ORA-12162:/ || res.body =~ /ORA-12560:/
  ...
  def report_isqlauth_info(ip, user, pass, sid)
    ...
    ora_info[:user] = if sid.nil? || sid.empty?
                        user
                      else
                        "#{sid}/#{user}"
                      end
