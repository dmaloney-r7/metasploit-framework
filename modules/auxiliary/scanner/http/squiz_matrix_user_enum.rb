  ...
  def do_enum(asset)
    begin
      ...
      if datastore['VERBOSE']
        if res && (res.code = 403) && res.body && res.body =~ /You do not have permission to access <i>(\w+)<\/i>/
          print_status("#{full_uri}?a=#{asset} - Trying Asset: '#{asset}' title '#{Regexp.last_match(1)}'")
        else
      ...
      if res && (res.code = 403) && res.body && res.body =~ /You do not have permission to access <i>~(\w+)<\/i>/
        user = Regexp.last_match(1).strip
        ...
        if res && (res.code = 403) && res.body && res.body =~ /You do not have permission to access <i>Inbox<\/i>/
          tmpasset = asset - 2
          res = send_request_cgi({
                                   'uri' => "#{uri}?a=#{tmpasset}",
                                   'method' => 'GET'
          ...
          if res && (res.code = 403) && res.body && res.body =~ /You do not have permission to access <i>(.*)<\/i>/
            fullname = Regexp.last_match(1).strip
