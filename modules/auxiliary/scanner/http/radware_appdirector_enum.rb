...
class MetasploitModule < Msf::Auxiliary
  ...
  def is_app_radware?
    ...
    if res && res.headers['Server'] && res.headers['Server'].include?("Radware-web-server")
  ...
  def do_login(user, pass)
    ...
    if res && (res.code == 302) && res.headers['Location'].include?('redirectId')
