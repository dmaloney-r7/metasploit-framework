...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    if datastore['SUBDOM_LIST'] && ::File.file?(datastore['SUBDOM_LIST'])
      valstr = IO.readlines(datastore['SUBDOM_LIST']).map do |e|
        e.gsub(".#{datastore['DOMAIN']}", "").chomp
      end
    else
      valstr = [
        "admin",
    ...
    2.times do |n|
      ...
      begin
        ...
        resparr[n] = noexistsres.body if noexistsres
