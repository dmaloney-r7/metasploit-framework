...
class MetasploitModule < Msf::Auxiliary
  ...
  def is_version_tested?(version)
    ...
    return true if Gem::Version.new(version) < Gem::Version.new('7.5')

    false
  end

  def get_system_name(res)
    return res.body.scan(/fullsystemname = "(.+)"/i).flatten[0] || '' if res

    ''
  end

  def anonymous_access?(res)
    return true if res && res.body =~ /username = "hpsmh_anonymous"/
    false
  end
  ...
  def run_host(ip)
    res = send_request_cgi('uri' => datastore['CPQLOGIN'],
                           'method' => 'GET',
                           'vars_get' => {
