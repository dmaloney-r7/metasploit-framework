...
class MetasploitModule < Msf::Auxiliary
  ...
  def crawler_process_page(t, page, cnt)
    ...
    info[:ctype] = page.headers['content-type'] if page.headers['content-type']

    info[:cookie] = page.cookies unless page.cookies.empty?

    info[:auth] = page.headers['authorization'] if page.headers['authorization']

    info[:location] = page.headers['location'] if page.headers['location']
    ...
    return unless [302, 301, 200, 500, 401, 403, 404].include?(page.code)
    ...
    if page.doc
      page.doc.css("form").each do |f|
        ...
        if f['action'] && !f['action'].strip.empty?
          action = f['action']
          ...
          if (action[0, 1] != "/") && action !~ /\:\/\//
            ...
            page.doc.css("base").each do |bref|
              base = bref['href'] if bref['href']
            end
            action = (base + "/").sub(/\/\/$/, '/') + action
          end

          target = begin
                     page.to_absolute(URI(action))
                   rescue
                     next
                   end

          unless page.in_domain?(target)
            ...
            if target_uri.host.index(".").nil? || (target_uri.host == "127.0.0.1")
        ...
        next if target.to_s =~ get_link_filter
        ...
        f.css('select').each do |s|
          ...
          value ||= begin
                      s.children.first['value'] || s.children.first.content
                    rescue
                      ''
                    end
  ...
  def form_from_url(website, url)
    ...
    # ex: /customers/343
    if !(form[:path]) && url.path.to_s =~ /(.*)\/(\d+)$/
      path_base = Regexp.last_match(1)
      path_info = Regexp.last_match(2)
      form[:method] = 'PATH'
      form[:path]   = path_base
    ...
    if !(form[:path]) && url.path.to_s =~ /(.*\/[a-z0-9A-Z]{3,256}\.[a-z0-9A-Z]{2,8})(\/.*)/
      path_base = Regexp.last_match(1)
      path_info = Regexp.last_match(2)
      form[:method] = 'PATH'
      form[:path]   = path_base
