...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    if !datastore['QUERY'] || datastore['QUERY'].empty?
      datastore['QUERY'] = nil
      gvars = nil
    else
    ...
    if !datastore['DATA'] || datastore['DATA'].empty?
      datastore['DATA'] = nil
      pvars = nil
    else
  ...
  def detection_b(normalr, truer, falser, tarr)
    ...
    if normalr && truer
      if falser
        ...
        if ((truer.body.length - tarr[1].length) != normalr.body.length) && ((falser.body.length - tarr[2].length) == normalr.body.length)
  ...
  def detection_c(normalr, truer, falser, _tarr)
    # print_status("C")
    ...
    if normalr && truer
      if falser
        if (truer.code.to_i != normalr.code.to_i) && (falser.code.to_i == normalr.code.to_i)
  ...
  def detection_d(normalr, truer, falser, _tarr)
    # print_status("D")
    ...
    if normalr && truer
      if falser
        ...
        if (diff_nt_perc <= max_diff_perc) && diff_nt_f_perc > min_diff_perc
        ...
        if (diff_nf_perc <= max_diff_perc) && diff_nf_t_perc > min_diff_perc
          print_status("Detected by test D")
          return true
        end
      end
