...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    res = send_request_cgi('uri' => tpath,
                           'method' => 'GET')
    return if res.nil?
  ...
  def plugin_search(tpath, papp, ip, osize)
    res = send_request_cgi('uri' => "#{tpath}#{papp}",
                           'method' => 'GET')
    return if res.nil?
    ...
    if (res.code == 200) && res.body !~ /#404 Component not found/ && res.body !~ /<h1>Joomla! Administration Login<\/h1>/ && (osize != nsize)
      ...
      if papp =~ /passwd/ && res.body =~ /root/
      ...
      elsif papp =~ />alert/ && res.body =~ />alert/
        ...
        vars = papp.split('_')
        pages = vars[1].delete('/')
        res1 = send_request_cgi('uri' => "#{tpath}index.php?option=com_#{pages}",
                                'method' => 'GET')
        if res1.code == 200
