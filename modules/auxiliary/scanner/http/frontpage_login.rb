...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(target_host)
    port = if (datastore['RPORT'].to_i == 80) || (datastore['RPORT'].to_i == 443)
             ""
           else
             ":" + datastore['RPORT'].to_s
           end
    ...
    if res =~ /HTTP\/1.1 200 OK/
      ...
      if (fpversion = res.match(/FPVersion="(.*)"/))
        ...
        if (fpauthor = res.match(/FPAuthorScriptUrl="([^"]*)/))
          ...
          opts[:port] = datastore['RPORT'] unless port.empty?
          report_note(opts)
        end
  ...
  def check_account(info, fpversion, target_host)
    ...
    if res && res.match(/^HTTP\/1\.[01]\s+([^\s]+)\s+(.*)/)
      retcode = Regexp.last_match(1)
      retmsg  = Regexp.last_match(2).strip
      ...
      when /^200/
        ...
        report_vuln(
          ...
          proto: 'tcp',
          name: name,
          info: "Module #{fullname} confirmed access to #{info} [#{retcode}]",
          refs: references,
