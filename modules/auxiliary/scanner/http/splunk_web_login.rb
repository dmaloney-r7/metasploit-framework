...
class MetasploitModule < Msf::Auxiliary
  ...
  def is_app_splunk?
    ...
    (res && (res.code == 200) && res.body =~ /Splunk/)
  end
  ...
  def try_default_credential
    ...
    do_login(user, pass) if user && pass
  end
  ...
  def is_auth_required?
    ...
    res = send_request_raw('uri' => '/en-US/app/launcher/home',
                           'cookie' => "uid=#{uid}; #{session_id_port}=#{session_id}; cval=#{cval}")

    res && res.body =~ /Logged in as (.+)/ ? false : true
  end
  ...
  def do_login(user, pass)
    ...
    begin
      cval = ''
      uid, session_id_port, session_id, cval = get_login_cookie
      if !uid || !session_id_port || !session_id || !cval
