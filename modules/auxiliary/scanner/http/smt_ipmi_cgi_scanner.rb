...
class MetasploitModule < Msf::Auxiliary
  ...
  def check_close_window
    ...
    unless res && (res.code == 200) && res.body.to_s =~ /Can't find action/
      return false
    end

    res = send_close_window_request(trigger_check)

    return false unless res && (res.code == 500)
  ...
  def check_login
    ...
    unless res && (res.code == 200) && res.body.to_s =~ /ATEN International Co Ltd\./ && res.body.to_s =~ /top\.location\.href = location\.href/
      return false
    end
  ...
  def run_host(_ip)
    ...
    if result
      ...
      report_vuln(host: rhost,
                  ...
                  refs: references.select { |ref| ref.ctx_val == "2013-3621" })
    end
    ...
    if result
      ...
      report_vuln(host: rhost,
                  ...
                  refs: references.select { |ref| ref.ctx_val == "2013-3623" })
    end
  end
end
