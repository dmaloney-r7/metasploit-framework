...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    if res && (res.code == 200) && res.body =~ /failed to open stream\: No such file/
      print_error("Cannot read '#{fname}'. File does not exist.")

    elsif res && (res.code == 200) && res.body =~ /failed to open stream\: Permission denied/
      print_error("Cannot read '#{fname}'. Permission denied.")

    elsif res && (res.code == 200) && res.body =~ /Failed opening required/
