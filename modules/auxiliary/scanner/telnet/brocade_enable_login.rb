...
class MetasploitModule < Msf::Auxiliary
  ...
  def get_username_from_config(un_list, ip)
    ["config", "running-config"].each do |command|
      ...
      loop do
        ...
        # the config.
        break if config.match(/>$/) && config.match(/end/)
      end

      config.each_line do |un|
        next unless un =~ /^username/
  ...
  def run_host(ip)
    ...
    un_list.push(datastore['USERNAME']) if datastore['USERNAME'] # put the provided username on the array to try
