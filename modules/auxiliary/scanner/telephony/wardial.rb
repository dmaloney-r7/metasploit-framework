...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    if dialrange
      ...
      select = dialrange.select do |_key, value|
        case @target
        when :carrier then value[:carrier] == true
        when :fax then value[:fax] == true
        end
      end
    ...
    begin
      ...
      loop do
        ...
        if nextnum == true
          unidentified = dialrange.select do |_key, value|
            value[:identified] == false
          end
          if redialbusy
            unidentified += unidentified.select do |_key, value|
              value[:busy] == true
            end
          end
          break if unidentified.empty?
