...
class MetasploitModule < Msf::Auxiliary
  ...
  def enum_instance(rhost)
    ...
    begin
      ...
      if res.code == 200
        body = res.body
        if body =~ /<property>CentralServices<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          centralservices = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>SAPSYSTEM<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          sapsystem = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>SAPSYSTEMNAME<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          sapsystemname = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>SAPLOCALHOST<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          saplocalhost = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>INSTANCE_NAME<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          instancename = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>ICM<\/property><propertytype>NodeURL<\/propertytype><value>([^<]+)<\/value>/
          icmurl = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>IGS<\/property><propertytype>NodeURL<\/propertytype><value>([^<]+)<\/value>/
          igsurl = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>ABAP DB Connection<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          dbstring = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>J2EE DB Connection<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          j2eedbstring = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>Webmethods<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
          webmethods = Regexp.last_match(1).strip
          success = true
        end
        if body =~ /<property>Protected Webmethods<\/property><propertytype>Attribute<\/propertytype><value>([^<]+)<\/value>/
