...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    begin
      ...
      output_data << "Contact     : #{sysContact.strip}" unless sysContact.empty?

      sysLocation = snmp.get_value('1.3.6.1.2.1.1.6.0').to_s
      output_data << "Location    : #{sysLocation.strip}" unless sysLocation.empty?
      ...
      snmp.walk([
        ...
        if username.value.to_s !~ /noSuchInstance/
          username = Regexp.last_match(2) if username.value.to_s =~ /^JobAcct(\d+)=(.*)/
        else
          username = ''
        end

        if client.value.to_s !~ /noSuchInstance/
          client = Regexp.last_match(2) if client.value.to_s =~ /^JobAcct(\d+)=(.*)/
        else
          client = ''
        end

        if domain.value.to_s !~ /noSuchInstance/
          domain = Regexp.last_match(2) if domain.value.to_s =~ /^JobAcct(\d+)=(.*)/
        else
          domain = ''
        end

        if timestamp.value.to_s !~ /noSuchInstance/
          timestamp = Regexp.last_match(2) if timestamp.value.to_s =~ /^JobAcct(\d+)=(.*)/
        else
          timestamp = ''
        end

        if app_name.value.to_s !~ /noSuchInstance/
          app_name = Regexp.last_match(2) if app_name.value.to_s =~ /^JobAcct(\d+)=(.*)/
        else
          app_name = ''
        end

        if app_command.value.to_s !~ /noSuchInstance/
          app_command = Regexp.last_match(2) if app_command.value.to_s =~ /^JobAcct(\d+)=(.*)/
        else
          app_command = ''
        end
        ...
        unless timestamp.empty?
          ...
          output_data << "Username    : #{username}" unless username.empty?
          output_data << "Client      : #{client}" unless client.empty?
          output_data << "Domain      : #{domain}" unless domain.empty?
          output_data << "Timestamp   : #{timestamp}" unless timestamp.empty?
          output_data << "Application : #{app_name} (#{app_command})" unless app_name.empty?
          output_data << ""
        end
      end
