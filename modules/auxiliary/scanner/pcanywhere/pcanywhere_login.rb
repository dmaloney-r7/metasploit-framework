...
class MetasploitModule < Msf::Auxiliary
  ...
  def do_login(user, pass, nsock = sock)
    ...
    unless res && res.include?('Enter password')
  ...
  def pca_handshake(ip, nsock = sock)
    ...
    res = nsock.get_once(-1, 5)
    unless res && res.include?("\x78\x02\x1b\x61")
    ...
    res = nsock.get_once(-1, 5)
    unless res && (res == "\x1b\x62\x00\x02\x00\x00\x00")
    ...
    res = nsock.get_once(-1, 5)
    unless res && res.include?("\x00\x7D\x08")
  ...
  def pca_at_login?(res)
    return true if res && (res.include?('Enter login name') || res.include?('Enter user name'))
    false
  end
