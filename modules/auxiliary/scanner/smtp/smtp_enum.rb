...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    banner&.chomp!
    if banner =~ /microsoft/i && datastore['UNIXONLY']
    ...
    domain = 'localhost' if (domain == '') || !domain || domain.casecmp('hello').zero?
  ...
  def kiss_and_make_up(cmd)
    ...
    result, code = smtp_send(cmd.to_s)
    result.chomp!
    cmd.chomp!
