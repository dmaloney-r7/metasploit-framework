...
class MetasploitModule < Msf::Auxiliary
  ...
  def process_phase(phase_num, phase_name, prepend = '', initial_cmds = [])
    ...
    @evilchars.each do |evilstr|
      ...
      while count <= datastore['ENDSIZE']
        begin
          connect
          evil = if datastore['FASTFUZZ']
                   Rex::Text.pattern_create(count)
                 else
                   evilstr * count
                 end
          ...
          if (e.class.name == 'Rex::ConnectionRefused') || (e.class.name == 'EOFError') || ((e.class.name == 'Errno::ECONNRESET') && datastore['CONNRESET']) || (e.class.name == 'Errno::EPIPE')
  ...
  def ftp_commands
    if datastore['FtpCommands'].to_s.casecmp("DEFAULT").zero?
      @commands
    else
  ...
  def run_host(ip)
    ...
    @evilchars = [''] if datastore['FASTFUZZ']
      ...
      ftp_commands.each do |cmd|
        ...
        while count <= datastore['ENDSIZE']
          ...
          begin
            ...
            @error_cnt = 0 if @error_cnt >= @nr_errors
          end
          count += datastore['STEPSIZE']
        end
        sock.put("QUIT\r\n")
