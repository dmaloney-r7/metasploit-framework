...
class MetasploitModule < Msf::Auxiliary
  ...
  def on_request_uri(cli, request)
    ...
    when 'OPTIONS'
      ...
      sync_options unless datastore['SYNCFILE'].nil?
  ...
  def handle_relay(cli_sock, hash)
    ...
    if message[8, 1] != "\x03"
      ...
      cli_sock.send_response(response)
      cli_sock.close
      ser_sock.close if protocol == 'HTTP'
      return
    else
  ...
  def parse_args
    ...
    if !datastore['PUTDATA'].nil? && !datastore['FILEPUTDATA'].nil?
    ...
    if !framework.db.active && !datastore['VERBOSE']
  ...
  def sync_options
    ...
    unless ::File.readable?(datastore['SYNCFILE'])
  ...
  def http_relay_toserver(hash, ser_sock = nil)
    ...
    unless datastore['HTTP_HEADERFILE'].nil?
      print_status("Including extra headers from: #{datastore['HTTP_HEADERFILE']}")
      # previous request might create the file, so error thrown at runtime
      unless ::File.readable?(datastore['HTTP_HEADERFILE'])
    ...
    ser_sock = connect(opts) unless type3
  ...
  def smb_relay_toservert1(_hash)
    ...
    ser_sock.client.session_request if datastore['RPORT'] == '139'
  ...
  def smb_enum(ser_sock)
    ...
    print_status("Shares enumerated #{datastore['RHOST']} #{shares}")
    shares
  end
