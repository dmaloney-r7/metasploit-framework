...
class MetasploitModule < Msf::Auxiliary
  ...
  def on_client_command_user(c, arg)
    @state[c][:user] = arg
    if !datastore['FTPUSER'] || (arg == datastore['FTPUSER'])
  ...
  def on_client_command_pass(c, arg)
    @state[c][:pass] = arg
    if !datastore['FTPPASS'] || (arg == datastore['FTPPASS'])
      c.put "230 Login OK\r\n"
      @state[c][:auth] = true
    else
  ...
  def on_client_command_list(c, _arg)
    unless @state[c][:auth]
      c.put "500 Access denied\r\n"
      return
    end
  ...
  def on_client_command_cwd(c, arg)
    ...
    unless ::File.exist?(npath) && ::File.directory?(npath)
