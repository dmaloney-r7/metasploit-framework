...
class MetasploitModule < Msf::Auxiliary
  ...
  def target_host(addr = nil)
    ...
    if target.blank?
      ::Rex::Socket.source_address(addr) if addr
    else
  ...
  def run
    ...
    @log_console = true if datastore['LogConsole']

    @log_database = true if datastore['LogDatabase']
    ...
    @bypass = datastore['TARGETACTION'].casecmp("BYPASS").zero?
      ...
      while @run
        ...
        request.each_question do |name, typeclass|
          ...
          when 'IN::A'
            ...
            if (@match_target && !@bypass) || (!@match_target && @bypass)
