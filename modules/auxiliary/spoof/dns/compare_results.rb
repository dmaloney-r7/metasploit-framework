...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    names.each do |entry|
      ...
      unless base_res && targ_res && !base_res.empty? && !targ_res.empty?
        print_error("  Error: The baseline server did not respond to our request.") unless base_res && !base_res.empty?
        print_error("  Error: The target server did not respond to our request.") unless targ_res && !targ_res.empty?
        next
      end
    ...
    results.each_key do |entry|
      ...
      results[entry][:base]&.each_key do |rtype|
        ...
        results[entry][:base][rtype].sort.each do |ref|
          unless results[entry][:targ][rtype].include?(ref)
            n_sub << "  - #{entry} #{rtype} #{ref}"
          end
        end
        results[entry][:targ][rtype].sort.each do |ref|
          unless results[entry][:base][rtype].include?(ref)
