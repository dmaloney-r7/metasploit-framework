...
class MetasploitModule < Msf::Auxiliary
  ...
  def run
    ...
    if datastore['AUTO_ADD']
      ...
      @srchosts_autoadd_cache = {} if datastore['BIDIRECTIONAL']
    end
    ...
    begin
      ...
      @listener&.kill if datastore['LISTENER']
  ...
  def arp_poisoning
    ...
    @mutex_cache = Mutex.new if datastore['AUTO_ADD']

    # Start the listener
    start_listener(@dsthosts_cache, @srchosts_cache) if datastore['LISTENER']
    ...
    loop do
      if datastore['AUTO_ADD']
        @mutex_cache.lock
        unless @dsthosts_autoadd_cache.empty?
          @dsthosts_cache.merge!(@dsthosts_autoadd_cache)
          @dsthosts_autoadd_cache = {}
        end
        if datastore['BIDIRECTIONAL']
          unless @srchosts_autoadd_cache.empty?
  ...
  def start_listener(dsthosts_cache, srchosts_cache)
    args = if datastore['BIDIRECTIONAL']
             { BIDIRECTIONAL: true, dhosts: dsthosts_cache.dup, shosts: srchosts_cache.dup }
           else
             { BIDIRECTIONAL: false, dhosts: dsthosts_cache.dup, shosts: @shosts.dup }
           end
    ...
    @listener = Thread.new(args) do |args|
      begin
        ...
        loop do
          ...
          pkt = PacketFu::Packet.parse(pkt_bytes)
          next unless pkt.is_arp?
          next unless pkt.arp_opcode == 1
