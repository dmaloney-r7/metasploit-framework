...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    begin
      ...
      print_error("No response received from remote host") unless response
  ...
  def parse_reply(pkt)
    ...
    testn = Regexp.last_match(1).strip.to_s.split(';')[0] if pkt[0] =~ /^To\:\s*(.*)$/i
