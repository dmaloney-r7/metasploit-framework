...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    unless file?(scutil_path)
      print_error("Aborting, scutil binary not found.")
      return
    end

    unless file?(networksetup_path)
  ...
  def parse_vpn_connection_names(data, type = :connected)
    ...
    lines.each do |line|
      connection_names << Regexp.last_match(1) if line.start_with?(comp_str) && line =~ /"(.*)"/
    end
    connection_names
  end
