...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    print_status("This session is running as root!") if running_root
  ...
  def log_folder_create(log_path = nil)
    ...
    logs = if log_path
             ::File.join(log_path, 'logs', 'enum_osx', host + file_name_info)
           else
             ::File.join(Msf::Config.log_directory, 'post', 'enum_osx', host + file_name_info)
           end
  ...
  def enum_accounts(log_folder, ver_num)
    ...
    shell_commands = if ver_num =~ /10\.(7|6|5)/
                       leopard_commands
                     else
                       tiger_commands
                     end
