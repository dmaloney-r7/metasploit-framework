...
class MetasploitModule < Msf::Post
  ...
  def parse_vmx(vmx_data, filename)
    ...
    unless vmx_data.nil? || vmx_data.empty?
      ...
      vmx_data.each_line do |line|
        ...
        case data[0]
        when "memsize "
          vm['memsize'] = data[1].delete!("\"").lstrip.chomp
        when "displayName "
          vm['name'] = data[1].delete!("\"").lstrip.chomp
        when "guestOS "
          vm['os'] = data[1].delete!("\"").lstrip.chomp
        when "ethernet0.connectionType "
          vm['eth_type'] = data[1].delete!("\"").lstrip.chomp
        when "ethernet0.generatedAddress "
          vm['mac'] = data[1].delete!("\"").lstrip.chomp
        when "numvcpus "
          vm['cpus'] = data[1].delete!("\"").lstrip.chomp
        when "sharedFolder0.hostPath "
          vm['SharedFolders'] << data[1].delete!("\"").lstrip.chomp
        end
      end
      vm['cpus'] ||= "1"
    end
  ...
  def session_has_search_ext
    begin
      return !!(session.fs && session.fs.file)
