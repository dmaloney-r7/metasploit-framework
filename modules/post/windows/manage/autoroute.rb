...
class MetasploitModule < Msf::Post
  ...
  def autoadd_routes
    ...
    session.net.config.each_route do |route|
      ...
      next if switch_board.route_exists?(route.subnet, route.netmask)
      begin
  ...
  def autoadd_interface_routes
    ...
    session.net.config.each_interface do |interface| # Step through each of the network interfaces
      (0..(interface.addrs.size - 1)).each do |index| # Step through the addresses for the interface
        ...
        next unless subnet
        next if switch_board.route_exists?(subnet, netmask)
        begin
  ...
  def get_subnet_octet(net, mask)
    ...
    (subnet_range * multi) # Multiply to get subnet octet
  end
  ...
  def add_default
    ...
    unless switch_board.route_exists?(subnet, mask)
      begin
