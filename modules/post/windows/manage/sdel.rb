...
class MetasploitModule < Msf::Post
  ...
  def size_on_disk(file)
    ...
    real_size = if size_.last != 0
                  (size_.first * sizeC) + sizeC
                else
                  size_.first * sizeC
                end
  ...
  def file_overwrite(file, type, n)
    ...
    n.times do
      ...
      random = Rex::Text.rand_text(real_size, nil) if type == 1
  ...
  def comp_encr(file)
    ...
    return true if (type & 0x4800).nonzero?
    false
  end
end
