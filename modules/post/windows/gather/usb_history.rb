  ...
  def info_hash_to_str(str, hash)
    ...
    if hash.key?('ParentIdPrefix')
      ...
      @drives.each do |x, y|
        ...
        next unless x =~ /\#/	# truncated disk volume that doesnt apply to removable media
        tmp = x.split(/\#/)[2].delete!("\u0000")	# ParentIdPrefix will be 3rd item, trip internal \x00
        tmp.gsub!(/\&RM$/i, '')	# get rid of RM on end if its there
        mounted_as = y if tmp.casecmp(hash['ParentIdPrefix'].downcase).zero?
      end
