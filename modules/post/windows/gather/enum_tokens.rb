...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    get_system if session.sys.config.getuid !~ /SYSTEM/ && datastore['GETSYSTEM']
    ...
    domain_admins.each do |da_user|
      ...
      session.sys.process.get_processes.each do |proc|
        next unless proc['user'] == "#{domain}\\#{da_user}"
        sid = session.sid
        peer = session.session_host
        target_pid = proc['pid']
      ...
      next unless !tbl_pids.rows.empty? && session.framework.db.active
      report_note(
