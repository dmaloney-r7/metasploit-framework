    ...
    unless next_account_id.nil?
      ...
      registry_enumkeys(@key_base).each do |k|
        ...
        if type == "POP3"
          ...
          unless pop3_use_spa.nil? # Account for SPA (NTLM auth)
          ...
          if http_password.nil?
            ...
            portnum = if http_server_url.include? "h\x00t\x00t\x00p\x00s"
                        443
                      else
                        80
                      end
          ...
          unless http_use_spa.nil?     # Account for SPA (NTLM auth)
            print_status("     Secure Password Authentication (SPA): Enabled")
          end
          ...
          unless imap_use_spa.nil?     # Account for SPA (NTLM auth)
            print_status("     Secure Password Authentication (SPA): Enabled")
          end
