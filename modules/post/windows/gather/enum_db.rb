...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    results += enumerate_mssql if check_mssql
    results += enumerate_oracle if check_oracle
    results += enumerate_db2 if check_db2
    results += enumerate_mysql if check_mysql
    results += enumerate_sybase if check_sybase
  ...
  def enumerate_oracle
    ...
    basekey_set.each do |basekey|
      ...
      instances.each do |i|
        ...
        unless exist?(val_ORACLE_HOME + "\\NETWORK\\ADMIN\\tnsnames.ora")
    ...
    print_error("\t\t! Oracle instances not found") unless found_key
    return results
  rescue
    print_error("\t\t! could not identify information")
    return results || []
  ...
  def enumerate_mysql
    ...
    instances = registry_enumkeys(basekey)
    return results if instances.nil? || instances.empty?
    instances.each do |i|
      key = "#{basekey}\\#{i}"
  ...
  def enumerate_sybase
    ...
    unless exist?(location + "\\ini\\sql.ini")
