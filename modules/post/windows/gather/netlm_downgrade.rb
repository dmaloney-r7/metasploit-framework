...
class MetasploitModule < Msf::Post
  ...
  def run
    ...
    if netlm == 0
      ...
      print_status("NetLM is Disabled: #{subkey}#{v_name} == #{netlm}")
      ...
      print_good("NetLM is Enabled:  #{subkey}#{v_name} == #{post_netlm}")
      ...
      if v
        print_status("Cleanup Completed: #{subkey}#{v_name} == #{netlm}")
      else
