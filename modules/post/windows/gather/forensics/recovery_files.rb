...
class MetasploitModule < Msf::Post
  ...
  def get_datarun_location(datarun)
    ...
    int_offset = if offset.size == 2
                   offset.unpack('v*')[0]
                 else
                   offset.unpack('V')[0]
                 end
    [int_log_cluster, int_offset]
  end
    ...
    1.upto(offset) do |_i|
      ...
      if (rf['lpBuffer'][0, 4] == "\x46\x49\x4c\x45") && (rf['lpBuffer'][22, 2] == "\x00\x00")
        ...
        unless name.nil?
          ...
          if (files != "") && files.include?(File.extname(name.capitalize)[1..-1])
  ...
  def get_name(entry)
    ...
    data_name[90, length * 2].delete("\000")
  end

  def get_size(entry)
    data = get_attribute(entry, "\x80\x00\x00\x00")
    return if data.nil?
    data[48, 8].unpack('Q<*')[0]
  end
  ...
  def file_system_features(handle)
    ...
    client.railgun.kernel32.DeviceIoControl(handle, fsctl_get_ntfs_volume_data, "", 0, 200, 200, 4, nil)
  end
  ...
  def mft_data_runs(handle)
    ...
    get_data_runs(rf['lpBuffer'][56..-1])
  end
