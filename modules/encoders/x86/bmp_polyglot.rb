  ...
  def new_size_long
    ...
    0.upto(2) do |i|
      ...
      if byte_2 + byte_3 + byte_4 > 0 # this jmp would be too large
        size = increment_size(size, i) if byte_0 > 0xfd
...
class MetasploitModule < Msf::Encoder
  ...
  def calc_new_size(orig_size, stub_length)
    ...
    { new_size: size, post_pad: post_pad, pre_pad: pre_pad }
  end
  ...
  def make_destego_stub(shellcode_size, padding, lsbs = 1)
    ...
    finalize.generate([ Rex::Arch::X86::EBP, Rex::Arch::X86::ESP ])
  end
  ...
  def validate_dib_header(dib_header)
    size, _, _, _, bbp, compression, = dib_header.unpack('VVVvvVVVVVV')
