...
class MetasploitModule < Msf::Encoder::XorAdditiveFeedback
  ...
  def generate_shikata_block(state, length, cutoff)
    ...
    key_reg = if state.context_encoding
                Rex::Poly::LogicalRegister::X86.new('key', 'eax')
              else
                Rex::Poly::LogicalRegister::X86.new('key')
              end
    ...
    init_key = if state.context_encoding
                 Rex::Poly::LogicalBlock.new('init_key',
