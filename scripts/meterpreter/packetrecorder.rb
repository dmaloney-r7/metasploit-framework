...
def log_file(log_path = nil)
  ...
  logs = if log_path
           ::File.join(log_path, 'logs', 'packetrecorder', host + filenameinfo)
         else
           ::File.join(Msf::Config.log_directory, "scripts", 'packetrecorder', host + filenameinfo)
         end
  ...
  Rex::FileUtils.clean_path(logfile)
end
...
def packetrecord(packtime, logfile, intid)
  ...
  while rec == 1
    ...
    while bytes_all > 0
      ...
      bytes_pct = pct if pct > bytes_pct
