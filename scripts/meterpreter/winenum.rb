...
def list_exec(cmdlst)
  ...
  cmdlst.each do |cmd|
    if i < 10
      ...
      sleep(0.10) && a.delete_if { |x| !x.alive? } until a.empty?
      i = 0
    end
  end

  a.delete_if { |x| !x.alive? } until a.empty?
end
  ...
  begin
    ...
    wmiccmds.each do |wmi|
      if i < 10
        ...
        sleep(0.01) && a.delete_if { |x| !x.alive? } until a.empty?
        i = 0
      end
    end
    a.delete_if { |x| !x.alive? } until a.empty?
