...
def enum_recent_mounts(base_key)
  ...
  if explorer_keys.include?("Map Network Drive MRU")
    registry_enumvals(full_path).each do |k|
      recent_mounts << registry_getvaldata(full_path, k) unless k =~ /MRUList/
    end
  end
  recent_mounts
end
...
def enum_conf_shares
  ...
  shares_key = if target_os =~ /Windows 7|Vista|2008/
...
if client.platform =~ /win32|64/
  ...
  if !is_system?
    ...
    user_sid.each do |us|
      mount_history += enum_recent_mounts("HKU\\#{us.chomp}")
      run_history += enum_run_unc("HKU\\#{us.chomp}")
    end
  end
