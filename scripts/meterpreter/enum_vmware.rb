...
def enum_vmwarewrk
  ...
  enum_users.each do |u|
    ...
    fav_file << account_file.read until account_file.eof?
  end
...
if client.platform =~ /win32|win64/
  if check_vmsoft
    ...
    enum_vcenter if vmware_products.include?("VMware VirtualCenter")
    if vmware_products.include?("VMware Virtual Infrastructure Client")
      enum_viclient
    end
    enum_vum if vmware_products.include?("VMware Update Manager")

    enum_vdm if vmware_products.include?("VMware VDM")
    enum_powercli if vmware_products.include?("VMware vSphere PowerCLI")
    if vmware_products.include?("VMware vSphere Host Update Utility 4.0")
      enum_vihosupdt
    end
    enum_vmwarewrk if vmware_products.include?("VMware Workstation")
  else
