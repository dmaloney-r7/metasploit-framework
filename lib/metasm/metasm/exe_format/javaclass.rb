...
module Metasm
  class JavaClass < ExeFormat
    ...
    class ConstantPool < SerialStruct
      ...
      def decode(c)
        ...
        while i < @constant_pool_count
          ...
          next unless entry.tag =~ /Long|Double/
    ...
    def encode_u1(val)
      Expression[val].encode(:u8, @endianness)
    end

    def encode_u2(val)
      Expression[val].encode(:u16, @endianness)
    end

    def encode_u4(val)
      Expression[val].encode(:u32, @endianness)
    end

    def decode_u1(edata = @encoded)
      edata.decode_imm(:u8, @endianness)
    end

    def decode_u2(edata = @encoded)
      edata.decode_imm(:u16, @endianness)
    end

    def decode_u4(edata = @encoded)
      edata.decode_imm(:u32, @endianness)
    end
    ...
    def string_at(idx)
      loop do
        ...
        return tmp.bytes if tmp.is_a? ConstantUtf8
        idx = tmp.index
      end
    end
    ...
    def cp_find(tag)
      constant_pool.constant_pool.each do |e|
        next if !e || (e.tag != tag)
        return e.idx if yield(e.info)
      end
      nil
    end
