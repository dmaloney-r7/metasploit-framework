  ...
  class MIB
    ...
    class << self
        ...
        def import_supported?
          `smidump --version` =~ /^smidump 0.4/ && $CHILD_STATUS == 0
        end
    ...
    def load_module(module_name, mib_dir = DEFAULT_MIB_PATH)
      ...
      @by_name.merge!(oid_hash) do |key, _old, _value|
    ...
    def varbind_list(object_list, option = :KeepValue)
      ...
      if object_list.respond_to? :to_str
        ...
        object_list.each do |item|
          varbind = if item.respond_to? :to_str
                      oid(item).to_varbind
                    else
                      item.to_varbind
                      end
    ...
    def apply_option(varbind, option)
      if option == :NullValue
        ...
        raise ArgumentError, "invalid option: #{option}", caller
      end
      varbind
    end
