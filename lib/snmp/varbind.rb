...
module SNMP
    ...
    def initialize(varbind_list = [])
      ...
      if varbind_list.respond_to? :to_str
        ...
        varbind_list.each do |item|
          self << if item.respond_to? :to_str
                    ObjectId.new(item.to_str).to_varbind
                  else
                    item.to_varbind
                  end
        end
  ...
  class ObjectId < Array
    ...
    def index(parent_tree)
      parent_tree = make_object_id(parent_tree)
      if !subtree_of?(parent_tree)
        raise ArgumentError, "#{self} not a subtree of #{parent_tree}"
      elsif length == parent_tree.length
    ...
    def make_object_id(oid)
      oid.is_a?(ObjectId) ? oid : ObjectId.new(oid)
    end
    end
  ...
  class UnsignedInteger < Integer
    ...
    def self.decode(value_data)
      new(decode_uinteger_value(value_data))
    end
  end
  ...
  class VarBind
    ...
    class << self
        def decode(data)
          ...
          [VarBind.new(name, value), remaining_varbind_data]
        end
    ...
    def to_s
      "[name=#{@name}, value=#{@value} (#{@value.asn1_type})]"
    end
