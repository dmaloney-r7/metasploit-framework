...
module SNMP
  ...
  class UDPTransport
    def initialize(socket = nil)
      ...
      @socket = UDPSocket.open if socket.nil?
    end
  ...
  class Manager
    ...
    def get_value(object_list)
      if object_list.respond_to? :to_ary
        get(object_list).vb_list.collect(&:value)
      else
  ...
  class TrapListener
    ...
    def process_traps(trap_listener)
      @handler_init&.call(trap_listener)
      loop do
