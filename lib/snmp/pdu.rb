    ...
    class << self
        ...
        def decode_pdu(version, data)
          ...
          when SNMPv1_Trap_PDU_TAG
            raise InvalidPduTag, "SNMPv1-trap not valid for #{version}" if version != :SNMPv1
            pdu = SNMPv1_Trap.decode(pdu_data)
          when GetBulkRequest_PDU_TAG
            raise InvalidPduTag, "get-bulk not valid for #{version}" if version != :SNMPv2c
            pdu = PDU.decode(GetBulkRequest, pdu_data)
          when InformRequest_PDU_TAG
            raise InvalidPduTag, "inform not valid for #{version}" if version != :SNMPv2c
            pdu = PDU.decode(InformRequest, pdu_data)
          when SNMPv2_Trap_PDU_TAG
            raise InvalidPduTag, "SNMPv2c-trap not valid for #{version}" if version != :SNMPv2c
