      ...
      class Glassfish < HTTP
        ...
        def check_setup
          begin
            ...
            unless [200, 302].include?(res.code)
            ...
            # same port.
            if !ssl && res.headers['Location'] =~ /^https:/
              self.ssl = true
              res = send_request('uri' => '/common/index.jsf')
              return "Connection failed after SSL redirection" if res.nil?
              if res.code != 200
        ...
        def send_request(opts)
          ...
          self.jsession = Regexp.last_match(1) if res && res.get_cookies =~ /JSESSIONID=(\w*);/i
        ...
        def is_secure_admin_disabled?(res)
          res.body =~ /Secure Admin must be enabled/i ? true : false
        end
        ...
        def extract_version(banner)
          ...
          @version = if banner =~ /(GlassFish Server|Open Source Edition)[[:blank:]]*(\d\.\d)/
                       Regexp.last_match(2)
