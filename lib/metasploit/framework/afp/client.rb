    ...
    module AFP
      module Client
        ...
        def open_session
          ...
          response = sock.timed_read(1024)
          parse_open_session_response(response)
        end

        def login(user, pass)
          return no_user_authent_login if user == ''
          ...
          padded_user = (user.length + 1).odd? ? user + "\x00" : user
          ...
          begin
            response = sock.timed_read(1024, login_timeout)
          rescue Timeout::Error
            raise "AFP Login timeout (AFP server delay response for 20 - 22 seconds after 7 incorrect logins)"
          end
        ...
        def parse_open_session_response(response)
          _, _, _, error_code, = parse_header(response)
          error_code == 0 ? true : false
        end
        ...
        def parse_info_response(response)
          ...
          flags, command, request_id, error_code, length, reserved = parse_header(response)
          raise "AFP Server response with error" if error_code != 0
          body = get_body(response, length)
