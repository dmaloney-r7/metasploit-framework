...
module Rex
  module Parser
    ...
    load_nokogiri && class NexposeRawDocument < Nokogiri::XML::SAX::Document
                       ...
                       NEXPOSE_HOST_DETAIL_FIELDS = %w(nx_device_id nx_site_name nx_site_importance nx_scan_template nx_risk_score).freeze
                       ...
                       def report_vuln(&block)
                         ...
                         return unless @report_data[:vuln][:matches].is_a? Array
                         ...
                         ::ActiveRecord::Base.connection_pool.with_connection do
                           ...
                           vuln_ids.each do |vid|
                             ...
                             vuln.refs += refs unless refs.empty?

                             vuln.save! if vuln.changed?
                           end
                           ...
                           vdet_info[:nx_tags]         = @report_data[:vuln_tags].sort.uniq.join(", ") if @report_data[:vuln_tags].is_a?(::Array) && !@report_data[:vuln_tags].empty?
                           ...
                           %w(published added modified).each do |tf|
                             next unless @report_data[:vuln][tf]
                             ts = begin
                                    DateTime.parse(@report_data[:vuln][tf])
                                  rescue
                                    nil
                                  end
                             next unless ts
                       ...
                       def record_vuln_description(_attrs)
                         @report_data[:vuln_description_stack] = []
                       end

                       def record_vuln_solution(_attrs)
                       ...
                       def record_formatted_content(name, eattrs)
                         ...
                         stack = @report_data[:vuln_solution_stack] if in_tag("solution")

                         stack = @report_data[:vuln_description_stack] if in_tag("description")

                         stack = @report_data[:vuln_proof_stack] if in_tag("test")
                       ...
                       def report_test
                         ...
                         if in_tag("endpoint") && @state[:test][:port]
                           # Verify this port actually has some relation to our tracked state
                           # since it may not due to greedy vulnerability matching
                           if @state[:cached_service_object] && (@state[:cached_service_object].port.to_i == @state[:test][:port].to_i)
                         ...
                         if @state[:test][:nx_vulnerable_since]
                           ts = begin
                                  ::DateTime.parse(@state[:test][:nx_vulnerable_since])
                                rescue
                                  nil
                                end
                         ...
                         ::ActiveRecord::Base.connection_pool.with_connection do
                           ...
                           if vuln
                             ...
                             if in_tag("endpoint") && vuln.service
                       ...
                       def report_fingerprint(host_object)
                         return unless host_object.is_a? ::Mdm::Host
                         return unless @report_data[:os].is_a? Hash
                         note = {
                       ...
                       def collect_service_data
                         ...
                         if @state[:service]
                           sname = if state[:service]["name"] == "<unknown>"
                                     nil
                                   else
                                     db.service_name_map(@state[:service]["name"])
                                   end
                       ...
                       def actually_vulnerable(test)
                         return false unless test.key? "status"
                         return false unless test.key? "id"
                       ...
                       def record_host(attrs)
                         ...
                         if host_attrs["status"] == "alive"
                           ...
                           NEXPOSE_HOST_DETAIL_FIELDS.each do |f|
                             fs = f.to_sym
                             fk = f.sub(/^nx_/, '').tr('_', '-')
                             @state[fs] = host_attrs[fk] if host_attrs[fk]
                           end
                         end
                       end
                       ...
                       def collect_host_data
                         ...
                         if @state[:mac]
                           @report_data[:mac] = if @state[:mac] =~ /[0-9a-fA-f]{12}/
                                                  @state[:mac].scan(/.{2}/).join(":")
                                                else
                                                  @state[:mac]
                                                end
                         end
