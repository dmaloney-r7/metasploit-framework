  ...
  module Parser
    ...
    load_nokogiri && class OpenVASDocument < Nokogiri::XML::SAX::Document
                       ...
                       def end_element(name = nil)
                         ...
                         when 'cvss_base'
                           @state[:has_text] = true if in_tag('result') && in_tag('nvt')
                         when 'subnet'
                           @state[:has_text] = true
                         when 'result'
                           record_vuln
                         ...
                         when 'port'
                           if in_tag('result')
                             @state[:has_text] = true
                             if @text && @text.index('(')
                               @state[:proto] = @text.split('(')[1].split('/')[1].delete(')')
                               @state[:port] = @text.split('(')[1].split('/')[0].delete(')')
                       ...
                       def record_vuln
                         if (@state[:cves] && (@state[:cves] == "NOCVE")) && (@state[:bid] && (@state[:bid] == "NOBID"))
                           return
                         end

                         if @state[:cves] && (@state[:cves] != "NOCVE") && !@state[:cves].empty?
                         ...
                         if @state[:bid] && (@state[:bid] != "NOBID") && !@state[:bid].empty?
