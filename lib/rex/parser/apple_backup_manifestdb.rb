  ...
  module Parser
    class AppleBackupManifestDB
      ...
      def self.from_files(mbdb_file, mbdx_file)
        ...
        new(mbdb_data, mbdx_data)
      end

      def parse_mbdb
        raise ArgumentError, "Not valid MBDB data" if mbdb_data[0, 4] != "mbdb"
        self.mbdb_offset = 4
        self.mbdb_offset = mbdb_offset + 2 # Maps to \x05 \x00 (unknown)

        while mbdb_offset < mbdb_data.length
          info = {}
          info[:start_offset] = mbdb_offset
          ...
          info[:properties]   = {}
          1.upto(property_count) do |_i|
            k = mbdb_read_string
            v = mbdb_read_string
            info[:properties][k] = v
          end
          entry_offsets[ info[:start_offset] ] = info
        end
        self.mbdb_data = ""
      end

      def parse_mbdx
        raise ArgumentError, "Not a valid MBDX file" if mbdx_data[0, 4] != "mbdx"

        self.mbdx_offset = 4
        self.mbdx_offset = mbdx_offset + 2 # Maps to \x02 \x00 (unknown)

        file_count = mbdx_read_int(4)

        while mbdx_offset < mbdx_data.length
          file_id = mbdx_data[mbdx_offset, 20].unpack("C*").map { |c| "%02x" % c }.join
          self.mbdx_offset += 20
          ...
          next unless entry
          entries[file_id] = entry.merge(mbdx_mode: mode, file_id: file_id)
        end
        self.mbdx_data = ""
      end

      def mbdb_read_string
        raise "Corrupted MBDB file" if mbdb_offset > mbdb_data.length
        len = mbdb_data[mbdb_offset, 2].unpack("n")[0]
        self.mbdb_offset += 2
        return '' if len == 65535
        val = mbdb_data[self.mbdb_offset, len]
      ...
      def mbdb_read_int(size)
        val = 0
        size.downto(1) do |_i|
          val = (val << 8) + mbdb_data[self.mbdb_offset, 1].unpack("C")[0]
          self.mbdb_offset += 1
        end
        val
      end
      ...
      def mbdx_read_string
        raise "Corrupted MBDX file" if self.mbdx_offset > mbdx_data.length
        len = mbdx_data[self.mbdx_offset, 2].unpack("n")[0]
        self.mbdx_offset += 2
        return '' if len == 65535
        val = mbdx_data[self.mbdx_offset, len]
      ...
      def mbdx_read_int(size)
        val = 0
        size.downto(1) do |_i|
          val = (val << 8) + mbdx_data[self.mbdx_offset, 1].unpack("C")[0]
          self.mbdx_offset += 1
        end
        val
      end
