    ...
    load_nokogiri && class AppscanDocument < Nokogiri::XML::SAX::Document
                       ...
                       def end_element(name = nil)
                         ...
                         when "Issue" # Wrap it up
                           ...
                           @state = @state.select do |k|
                             [:current_tag, :web_sites].include? k
                           end
                         ...
                         when "OriginalHttpTraffic" # Request and response
                           ...
                           if page_info
                             ...
                             report_web_vuln(form_info, &block) if form_info
                           end
                           @text = nil
                         end
                       ...
                       def report_web_vuln(form_info, &block)
                         ...
                         return unless form_info.is_a? Hash
                         return unless @state[:issue]
                         return unless @state[:issue]["Noise"]
                         return unless @state[:issue]["Noise"].to_s.casecmp("false").zero?
                       ...
                       def collect_entity(attrs)
                         ...
                         return unless @state[:issue].is_a? Hash
                         ent_hash = attr_hash(attrs)
                         return unless ent_hash
                         return unless ent_hash["Type"].to_s.casecmp("parameter").zero?
                         @state[:issue][:vuln_param] = ent_hash["Name"]
                       end

                       def report_web_form(page_info, &block)
                         return unless in_issue && has_text
                         return unless page_info.is_a? Hash
                       ...
                       def record_request_and_response
                         ...
                         response_body = '' if response_body.to_s.empty?
                       ...
                       def record_issue
                         return unless in_issue
                         return unless @report_data[:issue].is_a? Hash
                         return unless @state[:web_site]
                         return if @state[:issue]["Noise"].to_s.casecmp("true").zero?
                       end
                       ...
                       def resolve_address(host)
                         ...
                         address = begin
                                     Rex::Socket.resolv_to_dotted(host)
                                   rescue
                                     nil
                                   end
