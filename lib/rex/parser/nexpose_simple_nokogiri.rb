...
module Rex
  module Parser
    ...
    load_nokogiri && class NexposeSimpleDocument < Nokogiri::XML::SAX::Document
                       ...
                       def record_host_fingerprint_data(_name, attrs)
                         return unless in_tag("device")
                         return if in_tag("service")
                       ...
                       def report_host_fingerprint(host_object)
                         return unless host_object.is_a? ::Mdm::Host
                         return unless @report_data[:host_fingerprint].is_a? Hash
                         @report_data[:host_fingerprint].reject! { |_k, v| v.nil? || v.empty? }
                       ...
                       def collect_service_data
                         ...
                         @state[:service].each do |k, v|
                           ...
                           when "name"
                             ...
                             port_hash[:name] = if sname == "<unknown>"
                                                  nil
                                                else
                                                  db.nmap_msf_service_map(sname)
                                                end
                           end
                         end
                         if @state[:service_fingerprint]
                           port_hash[:info] = (@state[:service_fingerprint]).to_s
                         end
