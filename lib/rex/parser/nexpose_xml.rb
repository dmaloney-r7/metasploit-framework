    ...
    class NexposeXMLStreamParser
      ...
      def tag_start(name, attributes)
        ...
        when "os"
          ...
          if !(@host["os_certainty"]) || (@host["os_certainty"].to_f < attributes["certainty"].to_f)
        ...
        when "fingerprint"
          @host["endpoints"].last.merge!(attributes) if @state == :in_service
        when "test"
          if !@only_vuln_states_needed || (@vulnerable_markers.include?(attributes["status"].to_s.chomp) && @only_vuln_states_needed)
            @state = :in_test
            ...
            unless @host["endpoints"].empty?
              @host["vulns"][@current_vuln_id]["endpoint_data"] = @host["endpoints"].last
            end
            if attributes["key"]
              @host["notes"] ||= []
      ...
      def tag_end(name)
        case name
        when "node"
          callback&.call(:host, @host)
          reset_state
        when "vulnerability"
          callback&.call(:vuln, @vuln)
          reset_state
