  ...
  module Parser
    ...
    load_nokogiri && class NmapDocument < Nokogiri::XML::SAX::Document
                       ...
                       def better_os_match(orig_hash, new_hash)
                         return false unless new_hash.key? "accuracy"
                         return true unless orig_hash.key? "accuracy"
                       ...
                       def record_host_osclass(attrs)
                         ...
                         @state[:os] ||= {}
                         temp_hash = attr_hash(attrs)
                         @state[:os] = temp_hash if better_os_match(@state[:os], temp_hash)
                       end
                       ...
                       def record_host_script(attrs)
                         ...
                         if temp_hash["id"] && temp_hash["output"]
                           @state[:scripts] ||= []
                       ...
                       def collect_host_data
                         @report_data[:state] = if @state[:host_alive]
                                                  Msf::HostState::Alive
                                                else
                                                  Msf::HostState::Dead
                                                end
                         if @state[:addresses]
                           if @state[:addresses].key? "ipv4"
                             @report_data[:host] = @state[:addresses]["ipv4"]
                           elsif @state[:addresses].key? "ipv6"
                             @report_data[:host] = @state[:addresses]["ipv6"]
                           end
                         end
                         if @state[:addresses] && @state[:addresses].key?("mac")
                           @report_data[:mac] = @state[:addresses]["mac"]
                         end
                         @report_data[:name] = @state[:hostname] if @state[:hostname]
                         @report_data[:last_boot] = @state[:uptime]["lastboot"] if @state[:uptime]
                         if @state[:trace] && @state[:trace].key?(:hops)
                           @report_data[:traceroute] = @state[:trace]
                         end
                         @report_data[:scripts] = @state[:scripts] if @state[:scripts]
                       end

                       def collect_port_data
                         return unless in_tag("host")
                         if @args[:fix_services]
                           return if @state[:port]["state"] == "filtered"
                         end
                         @report_data[:ports] ||= []
                         port_hash = {}
                         extra = []
