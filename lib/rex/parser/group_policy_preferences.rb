  ...
  module Parser
    ...
    class GPP
      ...
      def self.parse(data)
        ...
        return [] unless xml && xml.elements && xml.elements.to_a("//Properties")
        ...
        xml.elements.to_a("//Properties").each do |node|
          ...
          result[:EXPIRES] = expires unless expires.nil? || expires.empty?
          result[:NEVER_EXPIRES] = never_expires.to_i unless never_expires.nil? || never_expires.empty?
          result[:DISABLED] = disabled.to_i unless disabled.nil? || disabled.empty?
          result[:PATH] = path unless path.nil? || path.empty?
          result[:DATASOURCE] = dsn unless dsn.nil? || dsn.empty?
          result[:DRIVER] = driver unless driver.nil? || driver.empty?
          result[:TASK] = app_name unless app_name.nil? || app_name.empty?
          result[:SERVICE] = service unless service.nil? || service.empty?
