    ...
    module Meterpreter
      ...
      module PacketDispatcher
        ...
        def initialize_passive_dispatcher
          ...
          resource_uri = "/" + conn_id.to_s.gsub(/(^\/|\/$)/, '') + "/"

          self.passive_service = passive_dispatcher
          passive_service.remove_resource(resource_uri)
          passive_service.add_resource(resource_uri,
                                       'Proc'             => proc { |cli, req| on_passive_request(cli, req) },
                                       'VirtualDirectory' => true)
        end

        def shutdown_passive_dispatcher
          return unless passive_service
          ...
          if passive_service.resources.empty?
            Rex::ServiceManager.stop_service(passive_service)
          end
        ...
        def send_request(packet, timeout = response_timeout)
        ...
        def keepalive
          if @ping_sent
            if Time.now.to_i - last_checkin.to_i > PING_TIME * 2
              dlog("No response to ping, session #{sid} is dead", LEV_3)
              self.alive = false
            end
          else
            pkt = Packet.create_request('core_channel_eof')
            pkt.add_tlv(TLV_TYPE_CHANNEL_ID, 0)
            add_response_waiter(pkt, proc { @ping_sent = false })
            send_packet(pkt)
            @ping_sent = true
          end
        end
        ...
        def monitor_socket
          ...
          self.receiver_thread = Rex::ThreadFactory.spawn("MeterpreterReceiver", false) do
            while alive
              begin
                rv = Rex::ThreadSafe.select([ sock.fd ], nil, nil, PING_TIME)
                if rv
                  packet = receive_packet
                  @pqueue << packet if packet
                elsif send_keepalives && @pqueue.empty?
                  keepalive
                end
              rescue ::Exception => e
          ...
          self.dispatcher_thread = Rex::ThreadFactory.spawn("MeterpreterDispatcher", false) do
            begin
              while alive
                ...
                backlog << @pqueue.pop
                backlog << @pqueue.pop until @pqueue.empty?
                ...
                # queue is empty.
                if !backlog.empty? && backlog.length == incomplete.length
                  ::IO.select(nil, nil, nil, 0.10)
                end

                @pqueue << incomplete.shift until incomplete.empty?

                next unless @pqueue.length > 100
                removed = []
                25.times do
                  removed << @pqueue.pop
                end
                dlog("Backlog has grown to over 100 in monitor_socket, dropping older packets: #{removed.map(&:inspect).join(' - ')}", 'meterpreter', LEV_1)
              end
            rescue ::Exception => e
        ...
        def notify_response_waiter(response)
          ...
          waiters.each do |waiter|
            next unless waiter.waiting_for?(response)
        ...
        def dispatch_inbound_packet(packet)
          ...
          # waiters
          if packet.response?
            return true if notify_response_waiter(packet)
          end

          # Enumerate all of the inbound packet handlers until one handles
          # the packet
          @inbound_handlers.each do |handler|
            handled = nil
            begin

              handled = if packet.response?
                          handler.response_handler(self, packet)
                        else
                          handler.request_handler(self, packet)
                        end
