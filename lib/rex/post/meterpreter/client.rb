      ...
      class Client
        ...
        def init_meterpreter(sock, opts = {})
          ...
          if opts[:passive_dispatcher]
            ...
            swap_sock_plain_to_ssl if capabilities[:ssl] && !(opts[:skip_ssl])
        ...
        def swap_sock_plain_to_ssl
          ...
          if !(ssl.respond_to?(:accept_nonblock) && Rex::Compat.is_windows)
            ssl.accept
          else
            begin
              ssl.accept_nonblock
            ...
            # Ruby 1.9.2+ uses IO::WaitReadable/IO::WaitWritable
              ...
              if ::IO.const_defined?('WaitReadable') && e.is_a?(::IO::WaitReadable)
                IO.select([ ssl ], nil, nil, 0.10)
                retry
              end

              if ::IO.const_defined?('WaitWritable') && e.is_a?(::IO::WaitWritable)
          ...
          sock.sslsock = ssl
          sock.sslctx  = ctx
          sock.sslhash = Rex::Text.sha1_raw(ctx.cert.to_der)

          tag = sock.get_once(-1, 30)
          raise "Could not read the HTTP hello token" if !tag || tag !~ /^GET \//
        end
        ...
        def method_missing(symbol, *_args)
          ...
          ext_aliases.aliases[symbol.to_s]
        end
        ...
        def deregister_extension_alias(name)
          ext_aliases.aliases.delete(name)
        end
        ...
        def dump_extension_tree
          items = []
          items.concat(ext.dump_alias_tree('client.ext'))
          items.concat(ext_aliases.dump_alias_tree('client'))
        ...
        def unicode_filter_encode(str)
          encode_unicode ? Rex::Text.unicode_filter_encode(str) : str
        end
        ...
        def unicode_filter_decode(str)
          encode_unicode ? Rex::Text.unicode_filter_decode(str) : str
        end
