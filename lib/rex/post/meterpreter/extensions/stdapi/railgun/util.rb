          ...
          module Railgun
            ...
            class Util
              ...
              def is_null_pointer(pointer)
                pointer = unpack_pointer(pointer) if pointer.is_a? String
              ...
              def read_wstring(pointer, length = nil)
                ...
                return '' if is_null_pointer(pointer)

                # If length not provided, use lstrlenW
                length = railgun.kernel32.lstrlenW(pointer)['return'] if length.nil?
              ...
              def read_string(pointer, length = nil)
                ...
                length = railgun.kernel32.lstrlenA(pointer)['return'] unless length
              ...
              def memread(address, size, buffer = nil)
                if buffer.nil?
                  railgun.memread(address, size)
                else
                  buffer[address..(address + size - 1)]
                end
              end
              ...
              def read_data(type, position, buffer = nil)
                ...
                return read_struct(type, buffer, position) if is_struct_type?(type)
              ...
              def sizeof_type(type)
                return pointer_size if is_pointer_type?(type)
                ...
                return sizeof_struct(type) if is_struct_type?(type)

                if TYPE_DEFINITIONS.key?(type)
                  primitive = TYPE_DEFINITIONS[type]

                  return pointer_size if primitive == :pointer

                  if PRIMITIVE_TYPE_SIZES.key?(primitive)
              ...
              def sizeof_struct(struct)
                ...
                size_no_padding + calc_padding(size_no_padding)
              end
              ...
              def struct_offsets(definition, offset)
                ...
                definition.each do |mapping|
                  key, data_type = mapping
                  offset += padding if sizeof_type(data_type) > padding

                  offsets.push(offset)

                  offset += sizeof_type(data_type)
              ...
              def split_array_type(type)
                if type =~ /^(\w+)\[(\w+)\]$/
                  element_type = Regexp.last_match(1)
                  length = Regexp.last_match(2)
                  length = railgun.const(length) unless length =~ /^\d+$/

                  [element_type.to_sym, length.to_i]
                else
