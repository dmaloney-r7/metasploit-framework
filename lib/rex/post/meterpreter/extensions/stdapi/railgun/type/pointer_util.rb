            ...
            module Type
              module PointerUtil
                ...
                def self.pack_pointer(pointer, platform)
                  return pack_pointer(0, platform) if pointer.nil?
                  ...
                  when PlatformUtil::X86_32
                    ...
                    raise "platform symbol #{platform} not supported"
                  end
                end
                ...
                def self.null_pointer(_pointer, platform)
                ...
                def self.is_null_pointer?(pointer, platform)
                  pointer = unpack_pointer(pointer, platform) if pointer.is_a?(String)
                ...
                def self.is_pointer_type?(type)
                  return true if type == :pointer

                  return true if type.is_a?(String) && type =~ /^L?P/
