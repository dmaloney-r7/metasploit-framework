        ...
        module Stdapi
          module Sys
            ...
            class Registry
              ...
              def self.load_key(root_key, base_key, hive_file)
                ...
                response.get_tlv(TLV_TYPE_RESULT).value
              end

              def self.unload_key(root_key, base_key)
              ...
              def self.open_key(root_key, base_key, perm = KEY_READ)
                ...
                Rex::Post::Meterpreter::Extensions::Stdapi::Sys::RegistrySubsystem::RegistryKey.new(
              ...
              def self.check_key_exists(root_key, base_key)
                request = Packet.create_request('stdapi_registry_check_key_exists')
                request.add_tlv(TLV_TYPE_ROOT_KEY, root_key)
                request.add_tlv(TLV_TYPE_BASE_KEY, base_key)
                response = client.send_request(request)
                response.get_tlv(TLV_TYPE_BOOL).value
              end
              ...
              #
              def self.open_remote_key(target_host, root_key)
                ...
                Rex::Post::Meterpreter::Extensions::Stdapi::Sys::RegistrySubsystem::RemoteRegistryKey.new(
              ...
              #
              def self.create_key(root_key, base_key, perm = KEY_READ)
              ...
              #
              def self.delete_key(root_key, base_key, recursive = true)
                request = Packet.create_request('stdapi_registry_delete_key')
                flags   = 0

                flags |= DELETE_KEY_FLAG_RECURSIVE if recursive

                request.add_tlv(TLV_TYPE_ROOT_KEY, root_key)
                request.add_tlv(TLV_TYPE_BASE_KEY, base_key)
                request.add_tlv(TLV_TYPE_FLAGS, flags)

                return true unless client.send_request(request).nil?
              ...
              def self.enum_key_direct(root_key, base_key, perm = KEY_READ)
              ...
              #
              def self.set_value(hkey, name, type, data)
              ...
              def self.set_value_direct(root_key, base_key, name, type, data, perm = KEY_WRITE)
              ...
              #
              def self.query_value_direct(root_key, base_key, name, perm = KEY_READ)
              ...
              def self.query_value(hkey, name)
              ...
              # registry key.
              #
              def self.delete_value(hkey, name)
              ...
              def self.enum_value_direct(root_key, base_key, perm = KEY_READ)
              ...
              def self.key2str(key)
                if (key == 'HKLM') || (key == 'HKEY_LOCAL_MACHINE')
                  HKEY_LOCAL_MACHINE
                elsif (key == 'HKCU') || (key == 'HKEY_CURRENT_USER')
                  HKEY_CURRENT_USER
                elsif (key == 'HKU') || (key == 'HKEY_USERS')
                  HKEY_USERS
                elsif (key == 'HKCR') || (key == 'HKEY_CLASSES_ROOT')
                  HKEY_CLASSES_ROOT
              ...
              def self.splitkey(str)
                if str =~ /^(.+?)\\(.*)$/
                  [ key2str(Regexp.last_match(1)), Regexp.last_match(2) ]
                else
                  [ key2str(str), nil ]
                end
              end
