      ...
      module Ui
        ...
        class Console::CommandDispatcher::Stdapi::Net
          ...
          def cmd_netstat(*args)
            ...
            @@netstat_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-S'
                search_term = val
                if search_term.nil?
            ...
            connection_table.each do |connection|
          ...
          def cmd_arp(*args)
            ...
            @@arp_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-S'
                search_term = val
                if search_term.nil?
          ...
          def cmd_ipconfig(*_args)
            ...
            if ifaces.empty?
              ...
              ifaces.sort_by(&:index).each do |iface|
                print("\n" + iface.pretty + "\n")
              end
            end
          end
          ...
          def cmd_route(*args)
            ...
            @@route_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-h'
          ...
          def cmd_portfwd(*args)
            ...
            # Parse the options
            @@portfwd_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-h'
                cmd_portfwd_help
                return true
            ...
            if client.is_a?(PortForwardTracker) == false
            ...
            when 'list'
              ...
              service.each_tcp_relay do |lhost, lport, rhost, rport, opts|
                ...
                table << if opts['Reverse'] == true
                           [cnt + 1, "#{rhost}:#{rport}", "#{lhost}:#{lport}", 'Reverse']
                         else
                           [cnt + 1, "#{lhost}:#{lport}", "#{rhost}:#{rport}", 'Forward']
                         end
            ...
            when 'add'
              ...
              if reverse
                ...
                begin
                  ...
                  print_error("Failed to create relay: #{e}")
                  return false
                end
                ...
                service.start_tcp_relay(lport,
                                        ...
                                        'PeerPort'          => rport,
                                        'MeterpreterRelay'  => true,
                                        'OnLocalConnection' => proc { |relay, _lfd| create_tcp_channel(relay) })
              end
            ...
            when 'delete', 'remove', 'del', 'rm'
              ...
              unless index.nil?
                ...
                print_error("Invalid index: #{index}") unless found
              end
          ...
          def cmd_resolve(*args)
            ...
            @@resolve_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-h'
                ...
                return false
              when '-f'
                if val.casecmp('ipv6').zero?
                  family = AF_INET6
                elsif !val.casecmp('ipv4').zero?
            ...
            response.each do |result|
              table << if result[:ip].nil?
                         [result[:hostname], '[Failed To Resolve]']
                       else
