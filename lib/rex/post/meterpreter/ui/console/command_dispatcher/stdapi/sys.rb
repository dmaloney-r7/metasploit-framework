      ...
      module Ui
        ...
        class Console::CommandDispatcher::Stdapi::Sys
          ...
          def cmd_execute(*args)
            ...
            @@execute_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-a"
                cmd_args = val
              when "-c"
            ...
            shell.interact_with_channel(p.channel) if interact && p.channel
          end
          ...
          def cmd_kill(*args)
            ...
            if args.empty? || ((args.length == 1) && (args[0].strip == "-h"))
              cmd_kill_help
              return true
            end
            ...
            if self_destruct
              ...
              args.uniq!
              diff = args - valid_pids.map(&:to_s)
              unless diff.empty? # then we had an invalid pid
                print_error("The following pids are not valid:  #{diff.join(', ')}.  Quitting")
                return false
              end
            end

            # kill kill kill
            print_line("Killing: #{valid_pids.join(', ')}")
          ...
          def validate_pids(pids, allow_pid_0 = false, allow_session_pid = false)
            return [] if (pids.class != Array) || pids.empty?
            valid_pids = []
            ...
            clean_pids = pids.compact.uniq.map(&:to_i)
            # now we look up the pids & remove bad stuff if nec
            clean_pids.delete_if do |p|
              (((p == 0) && !allow_pid_0) || ((p == mypid) && !allow_session_pid))
            end
          ...
          def cmd_ps(*args)
            ...
            # Parse opts
            @@ps_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-S'
                search_term = val
                if search_term.nil?
              ...
              when "-A"
                ...
                processes.each do |proc|
                  next if proc['arch'].nil? || proc['arch'].empty?
              ...
              when "-U"
                ...
                processes.each do |proc|
                  ...
                  searched_procs << proc if proc["user"] =~ /#{val}/
                end
                processes = searched_procs
              end
            end
          ...
          def cmd_reboot(*args)
            ...
            @@reboot_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-f"
                force = val.to_i
              end
          ...
          def cmd_reg(*args)
            ...
            @@reg_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-h"
                print_line(
            ...
            begin
              ...
              when "enumkey"
                ...
                print_line("No children.") if vals.empty? && keys.empty?
          ...
          def cmd_getprivs(*args)
            cmd_getprivs_help if args.include? "-h"
            print_line("=" * 60)
          ...
          def cmd_shutdown(*args)
            ...
            @@shutdown_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-f"
                force = val.to_i
              end
          ...
          def cmd_suspend(*args)
            ...
            diff = args - valid_pids.map(&:to_s)
            unless diff.empty? # then we had an invalid pid
              print_error("The following pids are not valid:	#{diff.join(', ')}.")
              if continue
            ...
            targetprocess = nil
            if resume
              print_status("Resuming: #{valid_pids.join(', ')}")
            else
              print_status("Suspending: #{valid_pids.join(', ')}")
            end
            begin
              ...
              print_error "Error acting on the process:  #{e}."
