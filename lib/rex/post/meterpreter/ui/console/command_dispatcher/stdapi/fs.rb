    ...
    module Meterpreter
      module Ui
        ...
        class Console::CommandDispatcher::Stdapi::Fs
          ...
          CHECKSUM_ALGORITHMS = %w(md5 sha1).freeze
          ...
          def cmd_checksum(*args)
            ...
            algorithm&.downcase!
            unless !args.empty? && CHECKSUM_ALGORITHMS.include?(algorithm)
          ...
          def cmd_download(*args)
            ...
            @@download_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-r"
                recursive = true
                opts['recursive'] = true
            ...
            dest = ::File.dirname(dest) if client.fs.file.is_glob?(dest)
          ...
          def list_path(path, columns, sort, order, short, recursive = false, depth = 0, search_term = nil)
            ...
            client.fs.dir.entries_with_info(path).each do |p|
              ...
              next unless fname != '.' && fname != '..'
              if row.join(' ') =~ /#{search_term}/
                tbl << row
                items += 1
              end

              next unless recursive && ffstat && ffstat.directory?
          ...
          def cmd_ls(*args)
            ...
            # Parse the args
            @@ls_opts.parse(args) do |opt, _idx, val|
              case opt
              # Sort options
              when '-s'
                sort = 'Size'
            ...
            if is_glob
              unless client.commands.include?('stdapi_fs_search')
          ...
          def cmd_upload(*args)
            ...
            @@upload_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-r"
                recursive = true
              when nil
            ...
            src_items.each do |src|
              ...
              if stat.directory?
                ...
                if client.fs.file.exist?(dest) && client.fs.file.stat(dest).directory?
                  ...
                  client.fs.file.upload_file(dest, src) do |step, src, dst|
                    print_status("#{step.ljust(11)}: #{src} -> #{dst}")
                    client.framework.events.on_session_upload(client, src, dest) if msf_loaded?
                  end
                end
