      ...
      module Ui
        ...
        class Console::CommandDispatcher::Extapi::Clipboard
          ...
          def cmd_clipboard_set_text(*args)
            ...
            client.extapi.clipboard.set_text(args.join(" "))
          end
          ...
          def cmd_clipboard_monitor_start(*args)
            ...
            @@monitor_start_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-i"
                # default this to true
                capture_images = !val.casecmp('false').zero?
              when "-h"
            ...
            client.extapi.clipboard.monitor_start( # random class and window name so that it isn't easy
          ...
          def cmd_clipboard_monitor_dump(*args)
            ...
            @@monitor_dump_opts.parse(args) do |opt, _idx, val|
              ...
              when "-d"
                download_path = val
              when "-i"
                download_images = !val.casecmp('false').zero?
              when "-f"
                download_files = !val.casecmp('false').zero?
              when "-p"
                purge = !val.casecmp('false').zero?
              when "-h"
            ...
            dump = client.extapi.clipboard.monitor_dump(include_images: download_images,
                                                        purge: purge)
          ...
          def cmd_clipboard_monitor_stop(*args)
            ...
            @@monitor_stop_opts.parse(args) do |opt, _idx, val|
              ...
              when "-d"
                download_path = val
              when "-x"
                dump_data = !val.casecmp('false').zero?
              when "-i"
                download_images = !val.casecmp('false').zero?
              when "-f"
                download_files = !val.casecmp('false').zero?
            ...
            dump = client.extapi.clipboard.monitor_stop(dump: dump_data,
          ...
          def download_file(dest_folder, source)
            ...
            if stat.directory?
              client.fs.dir.download(dest, source, { "recursive" => true }, true) do |step, src, dst|
              ...
              elements.each do |type, v|
                ...
                when 'Files'
                  ...
                  v.each do |f|
                    ...
                    download_file(loot_dir, f[:name]) if get_files
                    print_line
                    total += f[:size]
                  end
