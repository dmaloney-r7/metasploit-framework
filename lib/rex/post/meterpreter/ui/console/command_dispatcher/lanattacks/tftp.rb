      ...
      module Ui
        ...
        class Console::CommandDispatcher::Lanattacks::Tftp
          ...
          def cmd_tftp_start(*args)
            @@tftp_start_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-h'
          ...
          def cmd_tftp_stop(*args)
            @@tftp_stop_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-h'
          ...
          def cmd_tftp_reset(*args)
            @@tftp_reset_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-h'
          ...
          def cmd_tftp_add_file(*args)
            @@tftp_add_file_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-h'
