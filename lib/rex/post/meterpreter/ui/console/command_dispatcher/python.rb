      ...
      module Ui
        ...
        class Console::CommandDispatcher::Python
          ...
          def cmd_python_import(*args)
            ...
            @@python_import_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-f'
                source = val
              when '-n'
            ...
            if ::File.directory?(source)
              ...
              fail_with("No .py files found in #{source}") if files.empty?
          ...
          def cmd_python_execute(*args)
            ...
            @@python_execute_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-r'
                result_var = val
              end
          ...
          def handle_exec_result(result, result_var)
            ...
            elsif result[:stdout].empty? && result[:stderr].empty?
