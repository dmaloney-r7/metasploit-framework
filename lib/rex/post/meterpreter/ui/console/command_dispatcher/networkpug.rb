      ...
      module Ui
        ...
        class Console::CommandDispatcher::NetworkPug
          ...
          def cmd_networkpug_start(*args)
            ...
            @@options.parse(args) do |opt, _idx, val|
            ...
            if filter.nil? && (virtual_nic == true)
              filter = "ether host #{mac}"
            elsif !filter.nil? && (virtual_nic == true)
            ...
            if @channel
              @thread_stuff = Rex::ThreadFactory.spawn("MeterpreterNetworkPUGReceiver", false) do
                proxy_packets
              end
