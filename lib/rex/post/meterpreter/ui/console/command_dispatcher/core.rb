      ...
      module Ui
        ...
        class Console::CommandDispatcher::Core
          ...
          def commands
            ...
            c["info"] = "Displays information about a Post module" if msf_loaded?
          ...
          def cmd_channel(*args)
            ...
            # Parse options
            @@channel_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-l"
                mode = :list
              when "-c", "-k"
            ...
            when :list
              ...
              client.channels.each_pair do |cid, channel|
          ...
          def cmd_channel_tabs(_str, words)
            case words.length
            when 1
          ...
          def cmd_set_timeouts(*args)
            ...
            @@set_timeouts_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-c'
          ...
          def cmd_ssl_verify(*args)
            ...
            @@ssl_verify_opts.parse(args) do |opt, _idx, _val|
              case opt
              when '-q'
                query = true
                settings += 1
          ...
          def cmd_transport(*args)
            ...
            valid = true
            transport_index = 0
            @@transport_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-c'
                opts[:cert] = val
              when '-u'
            ...
            when 'remove'
              ...
              if !transport_index.zero? && @transport_map.key?(transport_index)
                # validate the URL
          ...
          def cmd_migrate(*args)
            if args.empty? || args.any? { |arg| %w(-h --pid --name).include? arg }
              cmd_migrate_help
              return true
            end
            ...
            @@migrate_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-t'
            ...
            if service
              service.each_tcp_relay do |lhost, lport, _rhost, _rport, opts|
          ...
          def cmd_load(*args)
            ...
            @@load_opts.parse(args) do |opt, _idx, _val|
              case opt
              when "-l"
                exts = SortedSet.new
            ...
            args.each do |m|
              ...
              begin
                ...
                add_extension_client(md) if client.core.use(md) == true
              rescue
                print_line
                log_error("Failed to load extension: #{$ERROR_INFO}")
                next
              end
          ...
          def cmd_load_tabs(_str, _words)
            ...
            [msf_path, gem_path].each do |path|
              ::Dir.entries(path).each do |f|
                if ::File.file?(::File.join(path, f)) && f =~ /ext_server_(.*)\.#{client.binary_suffix}/
                  tabs.add(Regexp.last_match(1)) unless extensions.include?(Regexp.last_match(1))
                end
              end
            end
            tabs.to_a
          ...
          def cmd_run(*args)
            ...
            begin
              ...
              print_error("Error in script: #{$ERROR_INFO.class} #{$ERROR_INFO}")
              elog("Error in script: #{$ERROR_INFO.class} #{$ERROR_INFO}")
              dlog("Callstack: #{$ERROR_POSITION.join("\n")}")
            end
          end

          def cmd_run_tabs(_str, words)
            tabs = []
            if !(words[1]) || !words[1].match(/^\//)
              begin
                tabs += tab_complete_postmods if msf_loaded?
                [
          ...
          def cmd_bgrun(*args)
            ...
            # Get the script name
            bgjobs[jid] = Rex::ThreadFactory.spawn("MeterpreterBGRun(#{args[0]})-#{jid}", false, jid, args) do |myjid, xargs|
          ...
          #
          def cmd_bglist(*_args)
            bgjobs.each_index do |jid|
              print_status("Job #{jid}: #{bgjobs[jid][:args].inspect}") if bgjobs[jid]
            end
          end
          ...
          def cmd_write(*args)
            ...
            @@write_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-f"
                src_file = val
              else
            ...
            if !cid || !(channel = client.find_channel(cid))
              print_error("Invalid channel identifier specified.")
              return true
            end
          ...
          def add_extension_client(mod)
            ...
            shell.enstack_dispatcher(klass)
          ...
          def tab_complete_postmods
            tabs = client.framework.modules.post.map do |name, _klass|
              mod = client.framework.modules.post.create(name)
              mod.fullname.dup if mod && mod.session_compatible?(client)
            end
