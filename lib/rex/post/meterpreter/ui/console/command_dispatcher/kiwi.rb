      ...
      module Ui
        ...
        class Console::CommandDispatcher::Kiwi
          ...
          def cmd_golden_ticket_create(*args)
            ...
            @@golden_ticket_create_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-u'
                user = val
              when '-d'
              ...
              when '-i'
                id = val.to_i
              when '-g'
                group_ids = val.split(',').map(&:to_i).to_a
              when '-s'
                sid = val
              end
            end
          ...
          def cmd_kerberos_ticket_list(*args)
            ...
            @@kerberos_ticket_list_opts.parse(args) do |opt, _idx, val|
              case opt
              when '-e'
                export = true
              when '-p'
          ...
          #
          def cmd_kerberos_ticket_purge(*_args)
          ...
          def cmd_creds_all(*args)
            method = proc { client.kiwi.all_pass }
          ...
          def cmd_creds_wdigest(*args)
            method = proc { client.kiwi.wdigest }
          ...
          def cmd_creds_livessp(*args)
            method = proc { client.kiwi.livessp }
          ...
          def cmd_creds_kerberos(*args)
            method = proc { client.kiwi.kerberos }
