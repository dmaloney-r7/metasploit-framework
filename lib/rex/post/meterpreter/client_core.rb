    ...
    module Meterpreter
      ...
      class ClientCore < Extension
        ...
        def get_loaded_extension_commands(extension_name)
          ...
          commands = []
          response.each(TLV_TYPE_STRING) do |c|
            commands << c.value
          end
        ...
        def transport_list
          ...
          response.each(TLV_TYPE_TRANS_GROUP) do |t|
            result[:transports] << {
        ...
        def load_library(opts)
          ...
          load_flags &= ~LOAD_LIBRARY_FLAG_LOCAL if opts['UploadLibrary']
          load_flags |= LOAD_LIBRARY_FLAG_ON_DISK if opts['SaveToDisk']
          load_flags |= LOAD_LIBRARY_FLAG_EXTENSION if opts['Extension']
          ...
          if (load_flags & LOAD_LIBRARY_FLAG_LOCAL) != LOAD_LIBRARY_FLAG_LOCAL
            ...
            ::File.open(library_path, 'rb') do |f|
              image = f.read
            end
          ...
          request.add_tlv(TLV_TYPE_TARGET_PATH, target_path) unless target_path.nil?
          ...
          commands = []
          response.each(TLV_TYPE_METHOD) do |c|
            commands << c.value
          end
        ...
        #
        def use(mod, opts = {})
          raise RuntimeError, "No modules were specified", caller if mod.nil?
          ...
          if commands.empty?
            ...
            path = ::File.expand_path(opts['ExtensionPath']) if opts['ExtensionPath']
        ...
        def enable_ssl_hash_verify
          ...
          return nil unless client.sock.type? == 'tcp-ssl'

          request = Packet.create_request('core_transport_setcerthash')

          hash = Rex::Text.sha1_raw(client.sock.sslctx.cert.to_der)
        ...
        def get_ssl_hash_verify
          ...
          response.get_tlv_value(TLV_TYPE_TRANS_CERT_HASH)
        end
        ...
        def migrate(target_pid, writable_dir = nil, opts = {})
          ...
          if client.platform == 'linux'
            writable_dir = tmp_folder if writable_dir.to_s.strip.empty?
          ...
          if client.platform == 'linux'
            ...
            raise RuntimeError, 'The meterpreter binary is wrong', caller if pos.nil?
          ...
          client.ext.aliases.keys.each do |e|
            client.core.use(e)
          end
        ...
        def shutdown
          ...
          if client.passive_service
            ...
            client.send_packet_wait_response(request, 10)
          else
        ...
        def valid_transport?(transport)
          if transport
            VALID_TRANSPORTS.key?(transport.downcase)
          else
            false
          end
        end
        ...
        def transport_prepare_request(method, opts = {})
          return nil unless valid_transport?(opts[:transport]) && opts[:lport]
        ...
        def generate_payload_stub(process)
          ...
          when 'linux'
            ...
            raise "Unsupported platform '#{client.platform}'"
          end
        ...
        def tmp_folder
          ...
          tmp = '/tmp' if tmp.to_s.strip.empty?
