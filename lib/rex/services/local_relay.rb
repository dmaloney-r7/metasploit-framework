  ...
  module Services
    ...
    class LocalRelay
        ...
        unless relay_thread
          self.relay_thread = Rex::ThreadFactory.spawn("LocalRelay", false) {
            begin
              ...
              elog("Error in #{self} monitor_relays: #{$ERROR_INFO}", 'rex')
            end
          }
        end
      end
      ...
      def start_tcp_relay(lport, opts = {})
        ...
        if (opts['PeerHost'].nil? || opts['PeerPort'].nil?) && (opts['Stream'] != true)
      ...
      #
      def each_tcp_relay
        relays.each_pair do |_name, relay|
      ...
      def close_relay(relay)
        if relay.is_a?(ReverseRelay)
          rev_chans.delete(relay.channel)
        else
          rfds.delete(relay.listener)
        end

        relays.delete(relay.name)
      ...
      def accept_relay_conn(srvfd)
        ...
        begin
          ...
          wlog("Failed to get remote half of local connection on relay #{relay.name}: #{$ERROR_INFO}", 'rex')
          lfd.close
          return
        end
      ...
      def monitor_relays
        loop do
          ...
          begin
            ...
            # closed error
            close_relay_conn(e.stream) if e.stream.is_a?(Stream)

            dlog("monitor_relays: closed stream #{e.stream}", 'rex', LEV_3)

            next
          rescue
            elog("Error in #{self} monitor_relays select: #{$ERROR_INFO.class} #{$ERROR_INFO}", 'rex')
            return
          end
          ...
          socks[0]&.each do |rfd|
            ...
            if rfd.is_a?(StreamServer)
              ...
              # If we catch an error, close the connection
                ...
                elog("Error in #{self} monitor_relays read: #{$ERROR_INFO}", 'rex')
