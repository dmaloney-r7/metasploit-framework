      ...
      module DispatcherShell
        ...
        module CommandDispatcher
          ...
          def deprecated_cmd(method = nil, *args)
            ...
            elsif method && respond_to?("cmd_#{method}", true)
              print_error "Use #{method} instead"
              send("cmd_#{method}", *args)
            end
          end
          ...
          def deprecated_help(method = nil)
            ...
            if cmd == "db_autopwn"
              print_error "See http://r-7.co/xY65Zr instead"
            elsif method && respond_to?("cmd_#{method}_help", true)
              print_error "Use 'help #{method}' instead"
              send("cmd_#{method}_help")
            end
          end
          ...
          def cmd_help(cmd = nil, *_ignored)
            if cmd
              ...
              shell.dispatcher_stack.each do |dispatcher|
                ...
                next unless dispatcher.respond_to?("cmd_#{cmd}", true)
                cmd_found = true
              ...
              print_error("No help for #{cmd}, try -h") if cmd_found && !help_found
              print_error("No such command") unless cmd_found
            else
          ...
          #
          def tab_complete_filenames(str, _words)
            matches = ::Readline::FILENAME_COMPLETION_PROC.call(str)
            if matches && (matches.length == 1) && File.directory?(matches[0])
              dir = matches[0]
        ...
        def tab_complete(str)
          ...
          str_words << '' unless str_trail.empty?
        ...
        def tab_complete_stub(str)
          ...
          dispatcher_stack.each do |dispatcher|
            ...
            res = if dispatcher.respond_to?('tab_complete_helper')
                    dispatcher.tab_complete_helper(str, tab_words)
                  else
                    tab_complete_helper(dispatcher, str, tab_words)
                  end
        ...
        def tab_complete_helper(dispatcher, str, words)
          ...
          if dispatcher.commands.include?(words[0]) && dispatcher.respond_to?(tabs_meth)
        ...
        def run_single(line)
          ...
          if method
            ...
            dispatcher_stack.each do |dispatcher|
              ...
              begin
                if dispatcher.commands.key?(method) || dispatcher.deprecated_commands.include?(method)
                  on_command_proc&.call(line.strip)
                ...
                print_error(
                  "Error while running command #{method}: #{$ERROR_INFO}" \
                  "\n\nCall stack:\n#{$ERROR_POSITION.join("\n")}"
                )
              rescue ::Exception
                error = $ERROR_INFO
            ...
            unknown_command(method, line) if (found == false) && (error == false)
          end
        ...
        #
        def unknown_command(method, _line)
        ...
        def enstack_dispatcher(dispatcher)
          dispatcher_stack.unshift(inst = dispatcher.new(self))
        ...
        def remove_dispatcher(name)
          dispatcher_stack.delete_if do |inst|
            (inst.name == name)
          end
        end
        ...
        def help_to_s(_opts = {})
          ...
          dispatcher_stack.reverse.each do |dispatcher|
