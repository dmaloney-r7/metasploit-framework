    ...
    module Text
      ...
      module Shell
        ...
        def init_tab_complete
          if input && input.supports_readline
            self.input = Input::Readline.new(->(str) { tab_complete(str) })
            if Readline::HISTORY.empty? && histfile && File.exist?(histfile)
              File.readlines(histfile).each do |e|
        ...
        def init_ui(in_input = nil, in_output = nil)
          ...
          if input
            ...
            input.extend(InputShell) if input.intrinsic_shell? == false
        ...
        def run(&block)
          loop do
            ...
            break if stop_flag || stop_count > 1
            ...
            if framework
              ...
              if input.prompt.include?("%H")
                ...
                hostname = `hostname`.split('.')[0] if hostname.nil?

                # check if hostname is still nil
                hostname = ENV['COMPUTERNAME'] if hostname.nil?

                hostname = 'unknown' if hostname.nil?
        ...
        def update_prompt(prompt = nil, new_prompt_char = nil, mode = false)
          if input
            new_prompt = if prompt
                           init_prompt + ' ' + prompt + prompt_char + ' '
                         else
                           self.prompt || ''
                         end

            new_prompt = prompt + (new_prompt_char || prompt_char) + ' ' if mode
            ...
            input.prompt = output.update_prompt(new_prompt)
        ...
        def parse_line(line)
          ...
          begin
            ...
            print_error("Parse error: #{$ERROR_INFO}")
          end
