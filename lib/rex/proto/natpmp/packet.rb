...
module Rex
  ...
  module NATPMP
    ...
    def get_external_address(udp_sock, host, port, timeout = 1)
      ...
      while (r = udp_sock.recvfrom(12, timeout)) && r[1]
    ...
    def map_port(udp_sock, host, port, int_port, ext_port, _protocol, _lifetime, timeout = 1)
      ...
      # handle the reply
      while (r = udp_sock.recvfrom(16, timeout)) && r[1]
        (_, _, result, _, _, actual_ext_port, = parse_map_port_response(r[0])
