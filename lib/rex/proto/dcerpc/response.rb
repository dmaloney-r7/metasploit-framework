      ...
      class Response
        ...
        def parse(body = '')
          ...
          if (type == 12) || (type == 15)
            ...
            if !frag_len || data.length < frag_len
              raise Rex::Proto::DCERPC::Exceptions::InvalidPacket, 'DCERPC response packet is incomplete'
            end
            ...
            self.sec_addr = data[26, sec_addr_len]
            ...
            while ack < num_results
              ack_result[ack] = data[ x + 0, 2 ].unpack('v')[0]
              ack_reason[ack] = data[ x + 2, 2 ].unpack('v')[0]
              ack_xfer_syntax_uuid[ack] = uuid.uuid_unpack(data[ x + 4, 16 ])
              ack_xfer_syntax_vers[ack] = data[ x + 20, 4 ].unpack('V')[0]
              x += 24
              ack += 1
            end
          ...
          if type == 2
            ...
            unless alloc_hint && context_id && cancel_cnt
              raise Rex::Proto::DCERPC::Exceptions::InvalidPacket, 'DCERPC response packet is incomplete'
            end

            # Put the application data into self.stub_data
            self.stub_data = data[data.length - alloc_hint, 0xffff]
            # End of RESPONSE
          end
