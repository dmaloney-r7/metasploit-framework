    ...
    module ACPP
      ...
      class Message
        ...
        def self.decode(data, validate_checksum = true)
          data = data.dup
          raise "Incorrect ACPP message size #{data.size} -- must be 128" unless data.size == 128
          raise 'Unexpected header' unless 'acpp' == data.slice!(0, 4)
          ...
          if validate_checksum
            ...
            if actual_message_checksum != read_message_checksum
              raise "Invalid message checksum (expected #{read_message_checksum}, calculated #{actual_message_checksum})"
            end
            ...
            if actual_payload_checksum != read_payload_checksum
              raise "Invalid payload checksum (expected #{read_payload_checksum}, calculated #{actual_payload_checksum})"
            end
          end
          m
        end
