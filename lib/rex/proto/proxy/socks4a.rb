      ...
      class Socks4a
        ...
        class Client
          ...
          class Packet
            ...
            #
            def self.recv(sock, _timeout = 30)
              raw = ''
              # read in the 8 byte header
              raw << sock.read(1) while raw.length < 8
              # if its a request there will be more data
              if raw[0..0].unpack('C').first == REQUEST_VERSION
                # read in the userid
                raw << sock.read(1) while raw[8..raw.length].index("\x00").nil?
                # if a hostname is going to be present, read it in
                ip = raw[4..7].unpack('N').first
                if ((ip & 0xFFFFFF00) == 0x00000000) && ((ip & 0x000000FF) != 0x00)
                  hostname = ''
                  hostname += sock.read(1) while hostname.index("\x00").nil?
                  raw << hostname
                end
              end
            ...
            def from_r(raw)
              ...
              return false if (@version != REQUEST_VERSION) && (@version != REPLY_VERSION)
              ...
              if raw.length > 8
                ...
                if is_hostname?
                  ...
                  @dest_ip = resolve(hostname)
                  # fail if we couldnt resolve the hostname
                  return false unless @dest_ip
                end
              else
                @userid = ''
              end
          ...
          def start
            ...
            @client_thread = Rex::ThreadFactory.spawn("SOCKS4AProxyClient", false) do
              begin
                ...
                raise "Invalid Socks4 request packet received." unless request
                # handle the request
                begin
                  ...
                  if request.is_connect?
                    ...
                    params['Context'] = @server.opts['Context'] if @server.opts.key?('Context')
          ...
          def stop
            @mutex.synchronize do
              unless @closed
                ...
                @client_thread.kill if @client_thread && @client_thread.alive?
            ...
            @server_thread = Rex::ThreadFactory.spawn("SOCKS4AProxyServer", false) do
              while @running
                begin
                  ...
                  wlog("Socks4a.start - server_thread - #{$ERROR_INFO}")
                end
              end
            end
          rescue
            wlog("Socks4a.start - #{$ERROR_INFO}")
            return false
          end
          true
        end
