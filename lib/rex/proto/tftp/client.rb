    ...
    module TFTP
      ...
      class Client
        ...
        def initialize(params)
          ...
          self.remote_file = params["RemoteFile"] || (::File.split(local_file).last if local_file)
        ...
        def start_server_socket
          ...
          if server_sock && block_given?
            yield "Started TFTP client listener on #{local_host}:#{local_port}"
          end
          threads << Rex::ThreadFactory.spawn("TFTPServerMonitor", false) do
            if block_given?
        ...
        def monitor_server_sock
          ...
          res = server_sock.recvfrom(65535)
          if res && res[0]
            code, type, data = parse_tftp_response(res[0])
            if (code == OpAck) && (action == :upload)
              if block_given?
            ...
            elsif (code == OpData) && (action == :download)
              if block_given?
        ...
        def monitor_client_sock
          res = client_sock.recvfrom(65535)
        ...
        def rrq_packet
          req = [OpRead, remote_file, mode]
          packstr = "na#{remote_file.length + 1}a#{mode.length + 1}"
          req.pack(packstr)
        end
        ...
        def send_read_request
          ...
          client_sock.sendto(rrq_packet, peer_host, peer_port)
          threads << Rex::ThreadFactory.spawn("TFTPClientMonitor", false) do
            if block_given?
        ...
        def recv_data(host, port, first_block)
          ...
          recvd_blocks = 1
          if block_given?
            yield "Source file: #{remote_file}, destination file: #{local_file}"
          ...
          if block_given?
            yield("Transferred #{recv_tempfile.size} bytes in #{recvd_blocks} blocks, download complete!")
          end
          self.status = { success: [
            local_file,
            remote_file,
        ...
        def write_and_ack_data(data, blocknum, host, port)
          recv_tempfile.write(data)
          recv_tempfile.flush
          req = ack_packet(blocknum)
          server_sock.sendto(req, host, port)
        ...
        def blockify_file_or_data
          if local_file =~ /^DATA:(.*)/m
            data = Regexp.last_match(1)
          elsif ::File.file?(local_file) && ::File.readable?(local_file)
            data = begin
                     ::File.open(local_file, "rb") { |f| f.read f.stat.size }
                   rescue
                     []
                   end
          else
          ...
          data_blocks.pop if data_blocks.size > 1 && data_blocks.last.empty?
          data_blocks
        end
        ...
        def send_write_request
          ...
          client_sock.sendto(wrq_packet, peer_host, peer_port)
          threads << Rex::ThreadFactory.spawn("TFTPClientMonitor", false) do
            if block_given?
              monitor_client_sock { |msg| yield msg }
            else
        ...
        def send_data(host, port)
          ...
          if data_blocks.empty?
            ...
            self.status = { success: [local_file, local_file, 0, 0] }
            return nil
          end
          sent_data = 0
          sent_blocks = 0
          ...
          if block_given?
            yield "Source file: #{local_file =~ /^DATA:/ ? '(Data)' : remote_file}, destination file: #{remote_file}"
          ...
          data_blocks.each_with_index do |data_block, idx|
            loop do
              ...
              if server_sock.sendto(req, host, port) <= 0
                send_retries += 1
                if send_retries > 100
                  break
                else
          ...
          yield "Too many send retries, aborted" if send_retries > 100
