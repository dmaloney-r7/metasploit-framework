    ...
    module TFTP
      ...
      class Server
        ...
        def find_file(fname)
          ...
          files.each do |f|
            return f if fname == f[:name]
          end

          # Now, if we have a tftproot, see if it can serve from it
          return find_file_in_root(fname) if @tftproot
        ...
        def find_file_in_root(fname)
          ...
          return nil if !::File.file?(fn) || !::File.readable?(fn)
        ...
        def find_transfer(type, from, block)
          transfers.each do |tr|
            if (tr[:type] == type) && (tr[:from] == from) && (tr[:block] == block)
              return tr
            end
          end
          nil
            ...
            transfers.each do |tr|
              ...
              if tr[:type] == OpRead
                ...
                if tr[:last_sent]
                  ...
                  if tr[:last_size] && tr[:last_size] < tr[:blksize]
        ...
        def dispatch_request(from, buf)
          ...
          when OpRead
            ...
            if !@shutting_down && (file = find_file(fn))
              if file[:once] && file[:started]
