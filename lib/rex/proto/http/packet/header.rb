    ...
    module Http
      ...
      class Packet::Header < Hash
        ...
        def from_s(header)
          ...
          header.split(/\r\n/mn).each do |str|
            next unless (md = str.match(/^(.+?)\s*:\s*(.+?)\s*$/))
            if self[md[1]]
              self[md[1]] << ", " + md[2]
            else
              self[md[1]] = md[2]
        ...
        def []=(key, value)
          ...
          each_key do |k|
            if k.casecmp(key.downcase).zero?
              store(k, value)
              stored = true
            end
          end

          store(key, value) if stored == false
          dcase_hash[key.downcase] = value
        end
        ...
        def to_s(prefix = '')
          ...
          if junk_headers
            while str.length < 4096
              str << if fold
                       "X-#{Rex::Text.rand_text_alphanumeric(rand(30) + 5)}:\r\n\t#{Rex::Text.rand_text_alphanumeric(rand(1024) + 1)}\r\n"
                     else
                       "X-#{Rex::Text.rand_text_alphanumeric(rand(30) + 5)}: #{Rex::Text.rand_text_alphanumeric(rand(1024) + 1)}\r\n"
                     end
            end
          end

          each_pair do |var, val|
            str << if fold
                     "#{var}:\r\n\t#{val}\r\n"
                   else
                     "#{var}: #{val}\r\n"
