  ...
  module Proto
    module Http
      ...
      class Server
        ...
        # @return [String]
        def inspect
          resources_str = resources.keys.map(&:inspect).join ", "
        ...
        def start
          ...
          listener.on_client_data_proc = proc { |cli|
            on_client_data(cli)
          }
        ...
        def add_response_headers(resp)
          resp['Server'] = server_name unless resp['Server']
        end
        ...
        def mime_type(file)
          ...
          type = ExtensionMimeTypes[Regexp.last_match(1).downcase] if file =~ /\.(.+?)$/
        ...
        def dispatch_request(cli, request)
          ...
          if (request['Connection']) &&
             request['Connection'].casecmp('Keep-Alive'.downcase).zero?
            cli.keepalive = true
          end
          ...
          resources.each_pair do |k, val|
            next unless request.resource =~ /^#{k}/ && k.length > len
            p    = val
            len  = k.length
            root = k
          end
          ...
          if p
            ...
            # procedure.
            if p[1] == true
              Rex::ThreadFactory.spawn("HTTPServerRequestHandler", false) do
          ...
          close_client(cli) if cli.keepalive == false
        end
      end
    end
  end
