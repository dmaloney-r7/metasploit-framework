      ...
      class Handler::Erb < Handler
        ...
        def on_request(cli, req)
          ...
          begin
            ...
            resp.body = if File.extname(file_path) == ".rhtml"
                          # Evaluate the data and set the output as the response body.
                          evaluate(ERB.new(data), cli, req, resp)
            ...
            elog("Erb::on_request: #{$ERROR_INFO}\n#{$ERROR_POSITION.join("\n")}", LogSource)
            ...
            resp.body =
              ...
              "The server encountered an error:<br/><br/> <b>" + html_escape($ERROR_INFO) + "</b><br/><br/>" \
              "Stack trace:<br/><br/>" +
              $ERROR_POSITION.map { |e| html_escape(e.to_s) }.join("<br/>") +
              "</body></html>"
          end

          # Send the response to the
          cli.send_response(resp) if cli && resp
