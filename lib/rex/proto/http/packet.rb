  ...
  module Proto
    module Http
      ...
      class Packet
        ...
        #
        def [](key)
          return headers[key] if headers.include?(key)

          headers.each_pair do |k, v|
            return v if k.casecmp(key.downcase).zero?
          end
        ...
        def parse(buf)
          ...
          begin
            ...
            # Process the header
            parse_header if state == ParseState::ProcessingHeader
        ...
        def completed?
          ...
          # are.
          return true if (state == ParseState::ProcessingBody) && body_bytes_left < 0
        ...
        def to_s
          ...
          if content
            ...
            if auto_cl == true && transfer_chunked == true
              raise "'Content-Length' and 'Transfer-Encoding: chunked' are incompatible"
            elsif auto_cl == true
              headers['Content-Length'] = content.length
            elsif transfer_chunked == true
              raise 'Chunked encoding is only available via 1.1' if proto != '1.1'
        ...
        def parse_header
          ...
          if headers['Transfer-Encoding'].to_s.casecmp('chunked').zero?
        ...
        def parse_body
          ...
          if transfer_chunked && (inside_chunk != 1) && self.bufq.length
            ...
            return if clen.nil? && self.bufq.empty?
