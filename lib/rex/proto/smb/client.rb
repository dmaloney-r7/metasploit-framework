  ...
  module Proto
    module SMB
      class Client
        ...
        def smb_recv
          data = socket.timed_read(4, read_timeout)
          raise XCEPT::NoReply if data.nil? || data.length < 4

          recv_len = data[2, 2].unpack('n')[0]
          return data if recv_len == 0
          ...
          while data.length != recv_len
            ...
            begin
              buff << socket.timed_read(recv_len - data.length, read_timeout)
            rescue Timeout::Error
            rescue
              raise XCEPT::ReadPacket
            end

            raise XCEPT::ReadPacket if buff.nil? || buff.empty?

            data << buff
          end

          # signing
          if require_signing && signing_key != ''
            if verify_signature
              raise XCEPT::IncorrectSigningError unless CRYPT.is_signature_correct?(signing_key, sequence_counter, data)
            end
        ...
        def smb_send(data, _evasion_level = 0)
          ...
          # signing
          if require_signing && signing_key != ''
            data = CRYPT.sign_smb_packet(signing_key, self.sequence_counter, data)
            self.sequence_counter += 1
          end

          begin
            # Just send the packet and return
            return socket.put(data) if (size == 0) || (size >= data.length)

            # Break the packet up into chunks and wait between them
            ret = 0
            until (chunk = data.slice!(0, size)).empty?
              ret = socket.put(chunk)
              ::IO.select(nil, nil, nil, wait) if wait > 0
            end
            return ret
          end
        end
        ...
        def smb_defaults(packet)
          packet.v['MultiplexID'] = multiplex_id.to_i
          packet.v['TreeID'] = last_tree_id.to_i
          packet.v['UserID'] = auth_user_id.to_i
          packet.v['ProcessID'] = process_id.to_i
        end
        ...
        def smb_recv_cache_find_match(expected_type)
          ...
          @smb_recv_cache.each do |cent|
            ...
            clean << cent if Time.now.to_i - tstamp.to_i > 300
        ...
        def smb_recv_parse(expected_type, ignore_errors = false)
          ...
          begin
            ...
            if (ignore_errors == false) && (pkt['Payload']['SMB'].v['ErrorClass'] != 0)
              raise XCEPT::ErrorCode
            end

          rescue XCEPT::InvalidWordCount, XCEPT::InvalidCommand, XCEPT::ErrorCode
            $ERROR_INFO.word_count = pkt['Payload']['SMB'].v['WordCount']
            $ERROR_INFO.command = pkt['Payload']['SMB'].v['Command']
            $ERROR_INFO.error_code = pkt['Payload']['SMB'].v['ErrorClass']
            raise $ERROR_INFO
          end
        ...
        def smb_parse_negotiate(pkt, data)
          ...
          return pkt if pkt['Payload']['SMB'].v['WordCount'] == 0
        ...
        def session_request(name = '*SMBSERVER', do_recv = true)
          ...
          ret = smb_send(pkt.to_s, EVADE::EVASION_NONE)
          return ret unless do_recv
          ...
          raise XCEPT::NetbiosSessionFailed if ack.v['Type'] != 130
        ...
        def negotiate(smb_extended_security = true, do_recv = true)
          ...
          dialects.push('NT LANMAN 1.0', 'NT LM 0.12') if encrypt_passwords

          data = dialects.collect { |dialect| "\x02" + dialect + "\x00" }.join('')

          pkt = CONST::SMB_NEG_PKT.make_struct
          smb_defaults(pkt['Payload']['SMB'])

          pkt['Payload']['SMB'].v['Command'] = CONST::SMB_COM_NEGOTIATE
          pkt['Payload']['SMB'].v['Flags1'] = 0x18

          pkt['Payload']['SMB'].v['Flags2'] = if smb_extended_security
                                                0x2801
                                              else
                                                0xc001
                                              end
          ...
          ack = smb_recv_parse(CONST::SMB_COM_NEGOTIATE)

          idx = ack['Payload'].v['Dialect']

          # Check for failed dialect selection
          return nil if idx < 0 || (idx >= dialects.length)
          ...
          # set require_signing
          self.require_signing	= true if ack['Payload'].v['SecurityMode'] & 0x08 != 0
          ...
          unless ack['Payload'].v['SessionKey'].nil?
          ...
          # Set the server GUID
          self.server_guid = buf[0, 16] if extended_security && (buf.length >= 16)

          # Set the server SecurityBlob
          if extended_security && buf.length > 16
          ...
          self.system_time = ::Time.at(system_time)
        ...
        def session_setup(user = '', pass = '', domain = '', do_recv = true)
          if dialect =~ /^(NT LANMAN 1.0|NT LM 0.12)$/

            if challenge_key
              return session_setup_no_ntlmssp(user, pass, domain, do_recv)
            end

            if extended_security
              return session_setup_with_ntlmssp(user, pass, domain, nil, do_recv)
            end

          end

          session_setup_clear(*args)
        end

        # Authenticate using clear-text passwords
        def session_setup_clear(user = '', pass = '', domain = '', do_recv = true)
          data = [ pass, user, domain, native_os, native_lm ].collect { |a| a + "\x00" }.join('')
          ...
          pkt['Payload']['SMB'].v['Flags1'] = 0x18
          pkt['Payload']['SMB'].v['Flags2'] = if require_signing
                                                # ascii
                                                0x2807
                                              else
                                                # ascii
          ...
          pkt['Payload'].v['SessionKey'] = session_id
          pkt['Payload'].v['Payload'] = data

          ret = smb_send(pkt.to_s)
          return ret unless do_recv

          ack = smb_recv_parse(CONST::SMB_COM_SESSION_SETUP_ANDX)

          self.auth_user = user if (ack['Payload'].v['Action'] != 1) && !user.empty?
        ...
        def session_setup_no_ntlmssp(user = '', pass = '', domain = '', do_recv = true)
          ...
          raise XCEPT::NTLM1MissingChallenge unless challenge_key
          ...
          self.require_signing = false if require_signing
          ...
          if NTLM_UTILS.is_pass_ntlm_hash?(pass)
            ...
            hash_lm = !pass.empty? ? NTLM_CRYPT.lanman_des(pass, challenge_key) : ''
            hash_nt = !pass.empty? ? NTLM_CRYPT.ntlm_md4(pass, challenge_key)   : ''
          end
          ...
          #
          self.default_domain = info[2] if default_domain.nil?
        ...
        def session_setup_with_ntlmssp(user = '', pass = '', domain = '', name = nil, do_recv = true)
          ntlm_options = {
            ...
            usentlm2_session: usentlm2_session,
            use_ntlmv2: use_ntlmv2,
            send_lm: send_lm,
            send_ntlm: send_ntlm,
            use_lanman_key: use_lanman_key
          }

          ntlmssp_flags = NTLM_UTILS.make_ntlm_flags(ntlm_options)

          name = Rex::Text.rand_text_alphanumeric(16) if name.nil?
          ...
          native_data = ''
          native_data << native_os + "\x00"
          native_data << native_lm + "\x00"
          ...
          ack = smb_recv_parse(CONST::SMB_COM_SESSION_SETUP_ANDX, true)
          ...
          # dns domain
          self.default_domain = if blob_data[:default_name] != blob_data[:default_domain]
                                  # We're in a domain; get the domain name now
                                  blob_data[:default_domain] || ''
                                end
        ...
        def session_setup_with_ntlmssp_blob(blob = '', do_recv = true, userid = 0)
          ...
          smb_recv_parse(CONST::SMB_COM_SESSION_SETUP_ANDX, false)
        end
        ...
        def session_setup_with_ntlmssp_temp(domain = '', name = nil, do_recv = true)
          ...
          raise XCEPT::NTLM2MissingChallenge if cidx == -1
        ...
        def tree_connect(share = 'IPC$', pass = '', do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_TREE_CONNECT_ANDX)
        ...
        def tree_disconnect(tree_id = last_tree_id, do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_TREE_DISCONNECT)

          self.last_tree_id = 0 if tree_id == last_tree_id

          ack
        end

        # Returns a SMB_CREATE_RES response for a given named pipe
        def create_pipe(filename, _disposition = 1, _impersonation = 2)
          create(filename)
        end
        ...
        def create(filename, disposition = 1, impersonation = 2, do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_NT_CREATE_ANDX)
        ...
        def delete(filename, tree_id = last_tree_id, do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_DELETE)
        ...
        def open(filename, mode = 0x12, access = 0x42, do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_OPEN_ANDX)
        ...
        def close(file_id = last_file_id, tree_id = last_tree_id, do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_CLOSE)
        ...
        def write(file_id = last_file_id, offset = 0, data = '', do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_WRITE_ANDX)

          ack
        end

        # Reads data from an open file handle
        def read(file_id = last_file_id, offset = 0, data_length = 64000, do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_READ_ANDX, true)
        ...
        def trans_named_pipe(file_id, data = '', no_response = nil)
          ...
          trans(pipe, '', data, 2, [0x26, file_id].pack('vv'), no_response)
        end
        ...
        def trans_mailslot(name, data = '')
          ...
          #   Priority: 0
          #      Class: Reliable
          trans_maxzero(name, '', data, 3, [1, 0, 1].pack('vvv'), true)
        end
        ...
        def trans(pipe, param = '', body = '', setup_count = 0, setup_data = '', no_response = false, do_recv = true)
          ...
          pkt['Payload'].v['Flags'] = 2 if no_response

          ret = smb_send(pkt.to_s)
          return ret if no_response || !do_recv

          smb_recv_parse(CONST::SMB_COM_TRANSACTION)
        end
        ...
        def trans2(subcommand, param = '', body = '', do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_TRANSACTION2)
        ...
        def nttrans(subcommand, param = '', body = '', setup_count = 0, setup_data = '', do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_NT_TRANSACT)
          ack
        end
        ...
        def nttrans_secondary(param = '', body = '', do_recv = true)
          ...
          ack = smb_recv_parse(CONST::SMB_COM_NT_TRANSACT_SECONDARY)
          ack
        end
        ...
        def queryfs_info_volume
          ...
          {
            'serial' => vals[0],
            'label'  => vals[2][0, vals[1]].delete("\x00")
          }
        end
        ...
        def queryfs_fs_volume
          ...
          vals = data.unpack('VVVVCCA*')
          {
            ...
            'serial'      => vals[2],
            'label'       => vals[6][0, vals[3]].delete("\x00")
          }
        end
        ...
        def queryfs_fs_attribute
          ...
          vals = data.unpack('VVVA*')
          {
            ...
            'fs_name'       => vals[3][0, vals[2]].delete("\x00")
          }
        end
        ...
        def find_first(path)
          ...
          loop do
            ...
            break if (eos != 0) || last_search_id.nil? || (last_offset.to_i == 0)
