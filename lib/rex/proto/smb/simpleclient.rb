      ...
      class SimpleClient
        ...
        def login(name = '', user = '', pass = '', domain = '',
          ...
          begin
            ...
            client.session_request(name) if direct != true
            ...
            ok = client.session_setup(user, pass, domain)
          rescue ::Interrupt
            raise $ERROR_INFO
          rescue ::Exception => e
        ...
        def login_split_next_ntlm1(user, domain, hash_lm, hash_nt)
          begin
            ok = client.session_setup_no_ntlmssp_prehash(user, domain, hash_lm, hash_nt)
          rescue ::Interrupt
            raise $ERROR_INFO
          rescue ::Exception => e
            n = XCEPT::LoginError.new
        ...
        def open(path, perm, chunk_size = 48000)
          ...
          fh = OpenFile.new(client, path, client.last_tree_id, file_id)
        ...
        def create_pipe(path, perm = 'c')
          ...
          file_id = ok['Payload'].v['FileID']
          fh = OpenPipe.new(client, path, client.last_tree_id, file_id)
        end
