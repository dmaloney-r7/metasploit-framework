  ...
  module Proto
    module DRDA
      ...
      class DDM_PARAM < Struct.new(:length, :codepoint, :payload)
        def read(str = "")
          raise DRDA::Error, "Input isn't a String." unless str.is_a? String
      ...
      class BASIC_DDM < Struct.new(:length, :magic, :format, :correlid,
        ...
        def read(str = "")
          ...
          i = 0
          while i < rest.size
            this_param = if self[:codepoint] == Constants::SQLCARD # These aren't DDM's.
                           rest[i, self[:length] - 10]
                         else
                           DDM_PARAM.new.read(rest[i, rest.size])
                         end
        ...
        def to_s
          to_a.pack("nCCnnn") + self[:payload].map(&:to_s).join
        end
      end
