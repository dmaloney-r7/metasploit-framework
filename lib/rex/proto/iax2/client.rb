    ...
    module IAX2
      class Client
        ...
        def create_call
          ...
          calls[ cid ] = IAX2::Call.new(self, cid)
        end
        ...
        def matching_call(pkt)
          ...
          mcall = calls.values.select { |x| (x.dcall == src_call) || (dst_call && (x.scall == dst_call)) }.first
          unless mcall
            dprint("Packet received for non-existent call #{[src_call, dst_call].inspect}  vs #{calls.values.map { |x| [x.dcall, x.scall] }.inspect}")
            return
          end
          mcall
        end
        ...
        def send_data(call, data, inc_seq = true)
          r = sock.sendto(data, server_host, server_port, 0)
          call.oseq = (call.oseq + 1) & 0xff if inc_seq
          r
        end
        ...
        def send_new(call, number)
          ...
          cid = call.caller_number || caller_number
          cid = number if cid == 'SELF'

          data << create_ie(IAX_IE_CALLING_NUMBER, cid)
          data << create_ie(IAX_IE_CALLING_NAME, call.caller_name || caller_name)
          ...
          data << create_ie(IAX_IE_USERNAME, username) if username
        ...
        def send_authrep_chall_response(call, chall)
          data =
            ...
            create_ie(IAX_IE_CHALLENGE_RESP, ::Digest::MD5.hexdigest(chall + password))
        ...
        def send_regreq_chall_response(call, chall)
          data =
            ...
            create_ie(IAX_IE_USERNAME, username) +
            create_ie(IAX_IE_CHALLENGE_RESP, ::Digest::MD5.hexdigest(chall + password)) +
