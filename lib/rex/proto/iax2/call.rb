      ...
      class Call
        ...
        def wait_for(*stypes)
          ::Timeout.timeout(IAX_DEFAULT_TIMEOUT) do
            while (res = queue.pop)
              return res if stypes.include?(res[1])
            end
          end
        ...
        def register
          ...
          client.send_regreq_chall_response(self, chall)
          ...
          if res[2][IAX_IE_APPARENT_ADDR]
            ...
            r_addr = r_addr.unpack("C*").map(&:to_s).join(".")
        ...
        def dial(number)
          ...
          if res[1] == IAX_SUBTYPE_AUTHREQ
            ...
            client.send_authrep_chall_response(self, chall)
        ...
        def ring_time
          (ring_finish || Time.now).to_i - ring_start.to_i
        end

        def timestamp
          ((::Time.now - itime) * 1000.0).to_i & 0xffffffff
        end
        ...
        def handle_control(pkt)
          ...
          if dst_call != scall
            dprint("Incoming packet to inactive call: #{dst_call} vs #{scall}: #{phdr.inspect} #{stype.inspect} #{info.inspect}")
            return
          end

          # Increment the received sequence number
          self.iseq = (iseq + 1) & 0xff

          if state == :hangup
            dprint("Packet received after hangup, replying with invalid")
            client.send_invalid(self)
            return
          end
          ...
          when IAX_TYPE_DTMF_BEGIN
            dprint("DTMF BEG: #{pkt[11, 1]}")
            dtmf << pkt[11, 1]

          when IAX_TYPE_DTMF_END
            dprint("DTMF END: #{pkt[11, 1]}")
        ...
        def each_audio_frame
          audio_buff.each do |frame|
            yield(frame)
          end
        end
        ...
        def decode_audio_frame(buff)
          ...
          # Unsupported codec, return empty
            ...
            dprint("UNKNOWN CODEC: #{codec.inspect}")
            ''
          end
        end
