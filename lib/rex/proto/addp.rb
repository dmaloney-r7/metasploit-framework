...
module Rex
  module Proto
    ...
    class ADDP
      ...
      MAGICS = %w(DIGI DVKT DGDP).freeze
      ERRORS          = %w(no_response unknown success authenticaton_failed unit_has_address invalid_value invalid_data unsupported_command).freeze
      WLAN_ENC_MODES  = %w(unknown none wep40 wep128).freeze
      WLAN_AUTH_MODES = %w(unknown open shared_key open_shared_key).freeze
      HWTYPES         = %w(
      ...
      def self.request_config_all(dmac = "\xff\xff\xff\xff\xff\xff")
        mac = dmac.length == 6 ? dmac : Rex::Socket.eth_aton(dmac)
        res = []
        MAGICS.each { |m| res << request_config(m, dmac) }
        res
      end
