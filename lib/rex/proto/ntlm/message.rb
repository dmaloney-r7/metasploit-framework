  ...
  module Proto
    module NTLM
      class Message < Rex::Proto::NTLM::Base::FieldSet
        ...
        def serialize
          deflag
          super + security_buffers.map { |_n, f| f.value }.join
        end
        ...
        def security_buffers
          @alist.find_all { |_n, f| f.instance_of?(BASE::SecurityBuffer) }
        end
        ...
        Type0 = Message.define do
          string        :sign,      size: 8, value: CONST::SSP_SIGN
          int32LE       :type,      value: 0
        end

        Type1 = Message.define do
          string          :sign,         size: 8, value: CONST::SSP_SIGN
        ...
        class Type2
          ...
          def response(arg, opt = {})
            ...
            ws = if opt[:workstation]
                   opt[:workstation]
                 else
                   ""
                 end

            cc = if opt[:client_challenge]
                   opt[:client_challenge]
                 else
                   rand(CONST::MAX64)
                 end
            ...
            if (reqflags & CONST::REQUEST_TARGET) == CONST::REQUEST_TARGET
              ...
              if downgrade
                ...
                if (reqflags & CONST::NEGOTIATE_NTLM2_KEY) == CONST::NEGOTIATE_NTLM2_KEY
                  reqflags -= CONST::NEGOTIATE_NTLM2_KEY
                end
                if (reqflags & CONST::NEGOTIATE_ALWAYS_SIGN) == CONST::NEGOTIATE_ALWAYS_SIGN
                  reqflags -= CONST::NEGOTIATE_ALWAYS_SIGN
                end
              end
        ...
        def self.downgrade_type_message(message)
          ...
          if type > 0 && type < 4
            reqflags = decode[12..15] if (type == 1) || (type == 3)
            ...
            0.upto(8) do |idx|
              flags.insert(0, "0") if idx > flags.length
            end
