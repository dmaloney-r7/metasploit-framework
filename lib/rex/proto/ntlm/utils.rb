    ...
    module NTLM
      class Utils
        ...
        def self.make_ntlm_flags(opt = {})
          ...
          ntlmssp_flags &= 0xfff7ffff if usentlm2_session && !use_ntlmv2 && !send_ntlm
        ...
        def self.parse_ntlm_type_2_blob(blob)
          ...
          raise XCEPT::NTLMMissingChallenge unless cidx
        ...
        def self.make_ntlmv2_clientchallenge(win_domain, win_name, dns_domain, dns_name,
          ...
          timestamp = chall_MsvAvTimestamp != '' ? chall_MsvAvTimestamp : time_unix_to_smb(Time.now.to_i).reverse.pack("VV")
        ...
        def self.create_lm_ntlm_responses(user, pass, challenge_key, domain = '', default_name = '', default_domain = '',
          ...
          if send_ntlm # should be default
            if usentlm2_session
              if use_ntlmv2
                ntlm_cli_challenge = make_ntlmv2_clientchallenge(default_domain, default_name, dns_domain_name,
                                                                 dns_host_name, client_challenge,
                                                                 chall_MsvAvTimestamp, spnopt)
                if is_pass_ntlm_hash?(pass)
                  argntlm = {
                ...
                argntlm = if is_pass_ntlm_hash?(pass)
                            {
              ...
              if send_lm
                arglm = if is_pass_ntlm_hash?(pass)
                          {
          ...
          [resp_lm, resp_ntlm, client_challenge, ntlm_cli_challenge]
        end
        ...
        def self.create_session_key(ntlmssp_flags, server_ntlmssp_flags, user, pass, domain, challenge_key,
          ...
          if server_ntlmssp_flags & CONST::NEGOTIATE_KEY_EXCH != 0
            key_exchange = true
            ntlmssp_flags |= CONST::NEGOTIATE_KEY_EXCH
          end
          # Check 128bits
          if server_ntlmssp_flags & CONST::NEGOTIATE_128 != 0
            key_size = 128
            ntlmssp_flags |= CONST::NEGOTIATE_128
            ntlmssp_flags |= CONST::NEGOTIATE_56
          # Check 56bits
          else
            if server_ntlmssp_flags & CONST::NEGOTIATE_56 != 0
              key_size = 56
              ntlmssp_flags |= CONST::NEGOTIATE_56
            end
          end
          ...
          if send_ntlm # Should be default
            if usentlm2_session
              ...
              if send_lm && use_lanman_key
                ...
                user_session_key = if is_pass_ntlm_hash?(pass)
                                     CRYPT.ntlmv1_user_session_key([ pass.upcase[33, 65] ].pack('H32'),
                                                                   pass_is_hash: true)
                                   else
                                     CRYPT.ntlmv1_user_session_key(pass)
                                   end
              end
            end
          else
            if usentlm2_session && use_ntlmv2
              ...
              user_session_key = if is_pass_ntlm_hash?(pass)
                                   CRYPT.lmv1_user_session_key([ pass.upcase[0, 32] ].pack('H32'),
                                                               pass_is_hash: true)
                                 else
                                   CRYPT.lmv1_user_session_key(pass)
                                 end
            end
          end
          ...
          [signing_key, enc_session_key, ntlmssp_flags]
        end
      end
    end
  end
