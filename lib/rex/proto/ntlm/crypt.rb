    ...
    module NTLM
      class Crypt
        ...
        def self.apply_des(plain, keys)
          raise "No OpenSSL support" unless @@loaded_openssl
        ...
        def self.ntlm_hash(password, opt = {})
          raise "No OpenSSL support" unless @@loaded_openssl
          pwd = password.dup
          pwd = Rex::Text.to_unicode(pwd) unless opt[:unicode]
        ...
        def self.ntlmv2_hash(user, password, domain, opt = {})
          ...
          userdomain = Rex::Text.to_unicode(userdomain) unless opt[:unicode]
        ...
        def self.lanman_des(password, challenge)
          lm_response(lm_hash: lm_hash(password),
                      challenge: challenge)
        end
        ...
        def self.ntlm_md4(password, challenge)
          ntlm_response(ntlm_hash: ntlm_hash(password),
                        challenge: challenge)
        end
        ...
        def self.ntlm2_session(arg, opt = {})
          ...
          chal = arg[:challenge]
          raise "ntlm_hash and challenge are required" unless passwd_hash && chal
        ...
        def self.is_hash_from_empty_pwd?(arg)
          hash_type = arg[:type]
          raise ArgumentError, "arg[:type] is mandatory" unless hash_type
          raise ArgumentError, "arg[:type] must be lm or ntlm" unless hash_type =~ /^((lm)|(ntlm))$/

          ntlm_ver = arg[:ntlm_ver]
          raise ArgumentError, "arg[:ntlm_ver] is mandatory" unless ntlm_ver

          hash = arg[:hash]
          raise ArgumentError, "arg[:hash] is mandatory" unless hash

          srv_chall = arg[:srv_challenge]
          raise ArgumentError, "arg[:srv_challenge] is mandatory" unless srv_chall
          ...
          when CONST::NTLM_V1_RESPONSE
            ...
            case hash_type
            when 'lm'
              arglm = {	lm_hash: lm_hash(''),
                        challenge: srv_chall }
              calculatedhash = lm_response(arglm)
            when 'ntlm'
              argntlm = {	ntlm_hash: ntlm_hash(''),
                          challenge: srv_chall }
              calculatedhash = ntlm_response(argntlm)
            end
          when CONST::NTLM_V2_RESPONSE
            ...
            raise ArgumentError, "arg[:cli_challenge] is mandatory in this case" unless cli_chall
            user = arg[:user]
            raise ArgumentError, "arg[:user] is mandatory in this case" unless user
            domain = arg[:domain]
            raise ArgumentError, "arg[:domain] is mandatory in this case" unless domain

            case hash_type
            when 'lm'
              raise ArgumentError, "Client challenge length must be exactly 8 bytes " if cli_chall.length != 8
              arglm = {	ntlmv2_hash: ntlmv2_hash(user, '', domain),
                        challenge: srv_chall }
              optlm = {	client_challenge: cli_chall }
              calculatedhash = lmv2_response(arglm, optlm)[0, 16]
            when 'ntlm'
              raise ArgumentError, "Client challenge length must be bigger then 8 bytes " if cli_chall.length <= 8
              argntlm = { 	ntlmv2_hash: ntlmv2_hash(user, '', domain),
                           challenge: srv_chall }
              optntlm = { 	nt_client_challenge: cli_chall }
              calculatedhash = ntlmv2_response(argntlm, optntlm)[0, 16]
            end
          when CONST::NTLM_2_SESSION_RESPONSE
            ...
            calculatedhash = ntlm2_session(argntlm, optntlm).join[24, 24]
          else
        ...
        def self.lmv1_user_session_key(pass, opt = {})
          usk = if opt[:pass_is_hash]
                  pass[0, 8]
                else
                  lm_hash(pass.upcase[0, 7], true)
                end
          usk.ljust(16, "\x00")
        end
        ...
        def self.ntlm2_session_user_session_key(pass, srv_chall, cli_chall, opt = {})
          ...
          ntlm_key = ntlmv1_user_session_key(pass, opt)
        ...
        def self.lmv2_user_session_key(user, pass, domain, srv_chall, cli_chall, opt = {})
          ...
          ntlmv2_key = ntlmv2_hash(user, pass, domain, opt)
        ...
        class << self; alias ntlmv2_user_session_key lmv2_user_session_key; end

        # Used when LanMan Key flag is set
        def self.lanman_session_key(pass, srvchall, opt = {})
          halfhash = if opt[:pass_is_hash]
                       pass[0, 8]
                     else
                       lm_hash(pass.upcase[0, 7], true)
                     end
