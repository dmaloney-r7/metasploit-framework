...
module Msf
  ...
  class Message
    ...
    def self.register_message_type(type)
      raise "duplicate message type registration" if MsgTypeMap.key?(type)
  
      MsgTypeMap[type] = self
  
      const_set(:MsgType, type)
    ...
    def self.fields(*attribs)
      names = attribs.map { |name, _type| name.to_s }
      arg_list = names.join(", ")
  ...
  class Authentification < Message
    ...
    def self.register_auth_type(type)
      raise "duplicate auth type registration" if AuthTypeMap.key?(type)
      AuthTypeMap[type] = self
      const_set(:AuthType, type)
    ...
    def dump
      super(4) do |buffer|
        buffer.write_int32_network(auth_type)
      end
    end
    ...
    def parse(buffer)
      super do
        ...
        raise ParseError unless auth_t == auth_type
  ...
  module SaltedAuthentificationMixin
    ...
    def dump
      raise DumpError unless @salt.size == salt_size
  
      message__dump(4 + salt_size) do |buffer|
        buffer.write_int32_network(auth_type)
        buffer.write(@salt)
      end
    end
  ...
  module NoticeErrorMixin
    ...
    def initialize(field_type = 0, field_values = [])
      raise ArgumentError if (field_type == 0) && (!field_values.empty?)
  ...
  class Parse < Message
    ...
    def parse(buffer)
      super do
        ...
        @parameter_oids = (1..n_oids).collect do
