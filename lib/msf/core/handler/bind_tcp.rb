  ...
  module Handler
    ...
    module BindTcp
      ...
      def start_handler
        ...
        if exploit_config && exploit_config['active_timeout']
        ...
        return if listener_pairs[phash]
        listener_pairs[phash] = true

        # Start a new handling thread
        listener_threads << framework.threads.spawn("BindTcpHandlerListener-#{lport}", false) do
          client = nil
          ...
          while stime + ctimeout > Time.now.to_i
            begin
              ...
              wlog("Exception caught in bind handler: #{$ERROR_INFO.class} #{$ERROR_INFO}")
            end
          ...
          if client
            ...
            conn_threads << framework.threads.spawn("BindTcpHandlerSession", false, client) do |client_copy|
              begin
                handle_connection(wrap_aes_socket(client_copy), opts)
              rescue
                elog("Exception raised from BindTcp.handle_connection: #{$ERROR_INFO}")
              end
            end
          else
      ...
      def wrap_aes_socket(sock)
        if datastore["PAYLOAD"] !~ /java\// || ((datastore["AESPassword"] || "") == "")
          return sock
        end
        ...
        Rex::ThreadFactory.spawn('AESEncryption', false) do
