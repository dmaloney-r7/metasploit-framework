    ...
    module ReverseHttp
      ...
      def print_prefix
        if Thread.current[:cli]
          super + "#{listener_uri} handling request from #{Thread.current[:cli].peerhost}; (UUID: #{uuid}) "
        else
          super
        end
      end
      ...
      def payload_uri(req = nil)
        ...
        callback_host = "[#{callback_host}]" if Rex::Socket.is_ipv6? callback_host
      ...
      def setup_handler
        ...
        service.server_name = datastore['MeterpreterServerName']
      ...
      def stop_handler
        if service
          service.remove_resource((luri + "/").gsub("//", "/"))
          if service.resources.empty? && sessions == 0
      ...
      def lookup_proxy_settings
        ...
        uri_host = "[#{info[:host]}]" if Rex::Socket.is_ipv6?(uri_host)
      ...
      def on_request(cli, req)
        ...
        when :init_python
          ...
          create_session(cli, passive_dispatcher: service,
                              conn_id: conn_id,
                              url: url,
