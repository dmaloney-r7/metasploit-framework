...
module Msf
  ...
  class PayloadSet < ModuleSet
    ...
    #
    def each_module_filter(_opts, _name, _mod)
      false
    end
    ...
    def recalculate
      ...
      _stagers.each_pair do |stager_name, op|
        ...
        _stages.each_pair do |stage_name, ip|
          ...
          # the stager's
          if stage_inst && (stage_inst.compatible?(stager_inst) == false)
          ...
          handler_type = if stager_mod.respond_to?('handler_type_alias') == true
                           stager_mod.handler_type_alias
                         else
          ...
          # Add the stage
          add_stage(p, combined, stage_name, handler_type, 'files' => op[5]['files'] + ip[5]['files'],
      ...
      delete_if do |k, v|
        ...
        !!(old_keys.include?(k) && !new_keys.include?(k))
      end
    ...
    def find_payload(platform, arch, handler, session, payload_type)
      ...
      each_module(
        ...
        next if p.is_a?(Msf::Payload::Generic)

        # Check to see if the handler classes match.
        next if handler && !p.handler_klass.ancestors.include?(handler)

        # Check to see if the session classes match.
        next if session && p.session && !p.session.ancestors.include?(session)

        # Check for matching payload types
        next if payload_type && (p.payload_type != payload_type)
    ...
    # returns an instance of that payload.
    #
    def find_payload_from_set(set, _platform, _arch, handler, session, payload_type)
      set.each do |_name, mod|
        p = mod.new
        ...
        # Check to see if the handler classes match.
        next if handler && (p.handler_klass != handler)

        # Check to see if the session classes match.
        next if session && (p.session != session)
    ...
    def add_stage(p, full_name, stage_name, handler_type, modinfo)
      ...
      stages[stage_name] = {} unless stages[stage_name]
    ...
    #
    def instance(name)
      _instances[name] = create(name) if _instances[name].nil?
    ...
    def on_module_reload(mod)
      @blob_cache.each_key do |key|
        @blob_cache.delete(key) if key.start_with? mod.refname
      end
    end
    ...
    #
    def _singles
      payload_type_modules[Payload::Type::Single] || {}
    end
    ...
    #
    def _stagers
      payload_type_modules[Payload::Type::Stager] || {}
    end
    ...
    #
    def _stages
      payload_type_modules[Payload::Type::Stage] || {}
    end
