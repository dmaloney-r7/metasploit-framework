...
module Msf
  ...
  class EventDispatcher
    ...
    def on_module_load(name, mod)
      general_event_subscribers.each do |subscriber|
    ...
    def method_missing(name, *args)
      ...
      when "on"
        if respond_to?(subscribers, true)
          found = true
          send(subscribers).each do |sub|
            next unless sub.respond_to?(name, true)
            sub.send(name, *args)
          end
        else
      ...
      when "add"
        if respond_to?(subscribers, true)
          found = true
          add_event_subscriber(send(subscribers), *args)
        end
      when "remove"
        if respond_to?(subscribers, true)
          found = true
          remove_event_subscriber(send(subscribers), *args)
        end
      end
