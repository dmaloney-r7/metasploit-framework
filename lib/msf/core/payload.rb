...
module Msf
  ...
  class Payload < Msf::Module
    ...
    def initialize(info = {})
      ...
      if self.class.include?(Msf::Payload::Single) &&
         self.class.include?(Msf::Payload::Stager)
        module_info['Stage'] = {}

        if module_info['Payload']
          module_info['Stage']['Payload']  = module_info['Payload']['Payload'] || ""
          module_info['Stage']['Assembly'] = module_info['Payload']['Assembly'] || ""
          module_info['Stage']['Offsets']  = module_info['Payload']['Offsets'] || {}
        else
      ...
      module_info['ConnectionType'] = connection_type
    end
    ...
    #
    def staged?
      (@staged || (payload_type == Type::Stager) || (payload_type == Type::Stage))
    end
    ...
    #
    def payload
      module_info['Payload'] ? module_info['Payload']['Payload'] : nil
    end
    ...
    #
    def assembly
      module_info['Payload'] ? module_info['Payload']['Assembly'] : nil
    end
    ...
    #
    def offsets
      module_info['Payload'] ? module_info['Payload']['Offsets'] : nil
    end
    ...
    def compatible_convention?(conv)
      ...
      elsif (payload_type == Type::Stager) &&
            conv.nil?
        true
    ...
    #
    def handler_klass
      module_info['Handler'] || Msf::Handler::None
    end
    ...
    def substitute_vars(raw, offsets)
      offsets.each_pair do |name, info|
        offset, pack = info
        ...
        if (val = datastore[name])
          if pack == 'ADDR'
            ...
            raise "IPv6 address specified for IPv4 payload." if val.length == 16
          elsif pack == 'ADDR6'
            val = Rex::Socket.resolv_nbo(val)
          ...
          if ['ADDR16MSB', 'ADDR16LSB'].include?(pack)
            if offset.length != 2
              raise "Missing value for payload offset, there must be two offsets."
            end

            val = val.unpack('N').pack('V') if pack == 'ADDR16LSB'
            ...
            val = val.unpack('N').pack('V') if pack == 'ADDR22LSB'
    ...
    #
    def replace_var(_raw, _name, _offset, _pack)
      false
    end
    ...
    def on_session(session)
      ...
      if assoc_exploit
        ...
        begin
          ...
          begin
            dlog("#{assoc_exploit.refname}: on_new_session handler triggered exception: #{e.class} #{e} #{e.backtrace}", 'core', LEV_1)
          rescue
            nil
          end
        end
        ...
        if
          (assoc_exploit.exploit_type == Exploit::Type::Remote) &&
          (assoc_exploit.passive? == false) &&
    ...
    def build(asm, off = {})
      raise ArgumentError, "Assembly must not be empty" if asm.nil? || asm.empty?
    ...
    def internal_generate
      ...
      raw = if assembly && !assembly.empty?
              build(assembly, offsets)
            else
              payload.dup
            end
      ...
      # do that now.
      substitute_vars(raw, offsets) if raw && offsets
