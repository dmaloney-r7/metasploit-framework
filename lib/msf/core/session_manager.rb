  ...
  class SessionManager < Hash
      ...
      self.monitor_thread = framework.threads.spawn("SessionManager", true) do
        ...
        begin
          loop do
            ...
            #
            rings = values.select { |s| s.respond_to?(:ring) && s.ring && s.rstream }
            ready = ::IO.select(rings.map(&:rstream), nil, nil, 0.5) || [[], [], []]
            ...
            ready[0].each do |fd|
              ...
              begin
                ...
                if buff
                  ...
                  begin
                    framework.events.on_session_output(s, buff)
                  rescue
                    nil
                  end
                end
            ...
            #
            next unless framework.db && framework.db.active
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do |_conn|
              ::Mdm::Session.where(closed_at: nil).each do |db_session|
                next unless db_session.last_seen.nil? || ((Time.now.utc - db_session.last_seen) > (2 * LAST_SEEN_INTERVAL))
    ...
    def initialize_scheduler_threads
      ...
      1.upto(SCHEDULER_THREAD_COUNT) do |i|
        scheduler_threads << framework.threads.spawn("SessionScheduler-#{i}", true) do
          loop do
    ...
    def register(session)
      ...
      if session.register?
        ...
        if session.respond_to?("console")
          session.console.on_command_proc = proc { |command, _error| framework.events.on_session_command(session, command) }
          session.console.on_print_proc = proc { |output| framework.events.on_session_output(session, output) }
        end
      end
    ...
    def deregister(session, reason = '')
      return unless session.register?

      return if session.dead? && !(self[session.sid.to_i])
