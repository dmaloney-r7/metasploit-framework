  ...
  class DBManager
    class Export
      ...
      def to_xml_file(path)
        ...
        report_file.write %(<MetasploitV5>\n)
        report_file.write %(<generated time="#{Time.now.utc}" user="#{myusername}" project="#{myworkspace.name.gsub(/[^A-Za-z0-9\x20]/n, '_')}" product="framework"/>\n)

        yield(:status, "start", "hosts") if block_given?
        report_file.write %(<hosts>\n)
        report_file.flush
      ...
      def marshalize(obj)
        ...
        when String
          obj.strip
        when TrueClass, FalseClass, Float, Integer, Integer, Time
          obj.to_s.strip
        when BigDecimal
          obj.to_s("F")
        when NilClass
