...
module Msf
  ...
  class Module
    ...
    def initialize(info = {})
      ...
      options.add_options(info['Options'], self.class)
      options.add_advanced_options(info['AdvancedOptions'], self.class)
      options.add_evasion_options(info['EvasionOptions'], self.class)
    ...
    def replicant
      ...
      obj.datastore    = datastore.copy
      obj.user_input   = user_input
      obj.user_output  = user_output
      obj.module_store = module_store.clone
    ...
    def perform_extensions
      if datastore[REPLICANT_EXTENSION_DS_KEY].present?
        if datastore[REPLICANT_EXTENSION_DS_KEY].respond_to?(:each)
          ...
          raise "Invalid settings in datastore at key #{REPLICANT_EXTENSION_DS_KEY}"
        end
      end
    end
    ...
    def workspace
      datastore['WORKSPACE'] ||
        (framework.db && framework.db.active && framework.db.workspace && framework.db.workspace.name)
    end
    ...
    def owner
      ...
      username = datastore['MODULE_OWNER'].to_s.strip

      # Specific method used by the commercial products
      username = datastore['PROUSER'].to_s.strip if username.empty?
    ...
    def register_parent(ref)
      datastore['WORKSPACE']    = (ref.datastore['WORKSPACE'] ? ref.datastore['WORKSPACE'].dup : nil)
      datastore['PROUSER']      = (ref.datastore['PROUSER']   ? ref.datastore['PROUSER'].dup   : nil)
      datastore['MODULE_OWNER'] = ref.owner.dup
      datastore['ParentUUID']   = ref.uuid.dup
    end
    ...
    def derived_implementor?(parent, method_name)
      method(method_name).to_s =~ /#{parent}[^:]/ ? false : true
    end
