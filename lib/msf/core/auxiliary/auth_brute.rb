...
module Msf
  ...
  module Auxiliary::AuthBrute
    ...
    def each_user_pass(noconn = false)
      ...
      credentials.each do |u, p|
        ...
        when :next_user # This means success for that user.
          ...
          @@credentials_skipped[fq_rest] = true if datastore['STOP_ON_SUCCESS'] # See?
    ...
    def counters_expired?(this_service, credentials)
      ...
      current_max_per_service = begin
                                  self.class.class_variable_get("@@max_per_service")
                                rescue
                                  nil
                                end
    ...
    def translate_proto_datastores
      ...
      ['SMBUser', 'FTPUSER'].each do |u|
        if datastore[u] && !datastore[u].empty?
          datastore['USERNAME'] = datastore[u]
        end
      end
      ['SMBPass', 'FTPPASS'].each do |p|
        if datastore[p] && !datastore[p].empty?
    ...
    def gen_blank_passwords(user_array, cred_array)
      blank_passwords = []
      blank_passwords.concat(user_array.map { |u| [u, ""] }) unless user_array.empty?
      unless cred_array.empty?
        cred_array.each { |u, _p| blank_passwords << [u, ""] }
      end
      (blank_passwords + cred_array)
    end
    ...
    def gen_user_as_password(user_array, cred_array)
      ...
      unless cred_array.empty?
        cred_array.each { |u, _p| user_as_passwords << [u, u] }
      end
      (user_as_passwords + cred_array)
    end

    def combine_users_and_passwords(user_array, pass_array)
      return [] if (user_array.length + pass_array.length) < 1
      combined_array = []
      if pass_array.empty?
    ...
    def extract_word_pair(wordfile)
      ...
      if wordfile.to_s =~ /^memory:/
        ...
        upfile_contents.split(/\n/).each do |line|
          user, pass = line.split(/\s+/, 2).map(&:strip)
    ...
    def extract_word_pair_from_memory(memloc)
      ...
      these_creds = if obj.all_creds.empty?
                      obj.builders.select { |x| x.respond_to? :imported_users }.map(&:imported_users).flatten
                    else
                      obj.all_creds
                    end
      these_creds.each do |cred|
        if @strip_passwords
          user = cred.split(/\s+/, 2).map(&:strip)[0]
          pass = ""
        elsif @strip_usernames
          user = ""
          pass = cred.split(/\s+/, 2).map(&:strip)[1]
        else
          user, pass = cred.split(/\s+/, 2).map(&:strip)
        end
    ...
    def vprint_brute(opts = {})
      print_brute(opts) if datastore['VERBOSE']
    end
    ...
    def print_brute(opts = {})
      if opts[:level] && opts[:level].to_s[/^v/]
        return unless datastore["VERBOSE"]
        level = opts[:level].to_s[1, 16].strip
      else
        level = opts[:level].to_s.strip
      end
      host_ip = opts[:ip] || opts[:rhost] || opts[:host] || (begin
                                                               rhost
                                                             rescue
                                                               nil
                                                             end) || datastore['RHOST']
      host_port = opts[:port] || opts[:rport] || (begin
                                                    rport
                                                  rescue
                                                    nil
                                                  end) || datastore['RPORT']
      ...
      if respond_to? print_method
        send print_method, complete_message
      else
    ...
    def build_brute_message(host_ip, host_port, proto, msg, legacy)
      ...
      if legacy # TODO: This is all a workaround until I get a chance to get rid of the legacy messages
        ...
        if old_msg.match(msg_regex) && !old_msg.match(msg_regex)[6].to_s.strip.empty?
          complete_message = ''
          complete_message << if ip.blank? && port.blank?
                                (old_msg.match(msg_regex)[4] || proto).to_s
                              else
                                "#{ip}:#{rport}"
          end
    ...
    def proto_from_fullname
      File.split(fullname).last.match(/^(.*)_(login|auth|identify)/)[1].upcase
    rescue
      nil
    end
    ...
    def cleanup_files
      ...
      if path && datastore['REMOVE_USERPASS_FILE']
        begin
          ::File.unlink(path)
        rescue
          nil
      ...
      if path && datastore['REMOVE_USER_FILE']
        begin
          ::File.unlink(path)
        rescue
          nil
      ...
      if path && datastore['REMOVE_PASS_FILE']
        begin
          ::File.unlink(path)
        rescue
          nil
