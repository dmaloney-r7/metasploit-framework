...
module Msf
  ...
  module Auxiliary::Login
    ...
    def password_prompt?(username = nil)
      ...
      if username
        return true if !username.empty? && @recvd =~ /#{username}'s/
      end
      false
    end
    ...
    def login_failed?
      ...
      return true if @recvd.strip.empty?

      # If we have not seen a newline, this is likely an echo'd prompt
      return true unless @recvd.index("\n")

      # We do have a set of highly-accurate success patterns
      return false if @recvd =~ @success_regex

      if @recvd =~ @failure_regex
        return true if @recvd !~ @false_failure_regex
      end
      false
    end
    ...
    #
    def send_user(user, _nsock = sock)
    ...
    #
    def send_pass(pass, _nsock = sock)
    ...
    def send_recv(msg, nsock = sock)
      raw_send(msg, nsock)
      recv_all(nsock)
      @recvd
    end

    def recv_all(_nsock = sock, _timeout = 10)
    ...
    # timeout
    #
    def wait_for(expect, nsock = sock)
      regx = if expect.is_a? Regexp
               expect
             else
               /#{Regexp.quote(expect)}/i
             end
      return true if @recvd =~ regx

      resp = ''
      resp = recv(nsock) while resp && !(@recvd =~ regx)
