  ...
  module Auxiliary::Scanner
    ...
    def run
      ...
      begin
        ...
        if respond_to?('run_host')
          ...
          loop do
            ...
            while @tl.length < threads_max
              ...
              @tl << framework.threads.spawn("ScannerHost(#{refname})-#{ip}", false, ip.dup) do |tip|
                targ = tip
                nmod = replicant
        ...
        if respond_to?('run_batch')

          unless respond_to?('run_batch_size')
          ...
          loop do
            ...
            while @tl.length < threads_max
              ...
              unless batch.empty?
                thread = framework.threads.spawn("ScannerBatch(#{refname})", false, batch) do |bat|
                  nmod = replicant
                  mybatch = bat.dup
                  begin
    ...
    def add_delay_jitter(_delay, _jitter)
      ...
      if delay_value > 0
        if jitter_value > 0
          ...
          delay_value = 0 if delay_value < 0
        end
