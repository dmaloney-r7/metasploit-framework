...
module Msf
  ...
  module Auxiliary::HttpCrawler
    ...
    def setup
      raise "Could not load Anemone/Nokogiri: #{@anemone_error}" unless @anemone_loaded
      super
    end
    ...
    def run
      ...
      if datastore['HttpUsername'] && (datastore['HttpUsername'] != '')
      ...
      if datastore['HTTPAdditionalHeaders']
        t[:headers] = datastore['HTTPAdditionalHeaders'].to_s.split("\x01").select { |x| !x.to_s.empty? }
      end
    ...
    def crawler_process_page(_t, page, cnt)
    ...
    def crawler_options(t)
      ...
      opts[:inject_headers] = t[:headers] if t[:headers] && !t[:headers].empty?

      opts[:cookies] = t[:cookies] if t[:cookies]
    ...
    #
    def ssl
      ((datastore.default?('SSL') && (rport.to_i == 443)) || datastore['SSL'])
    end
