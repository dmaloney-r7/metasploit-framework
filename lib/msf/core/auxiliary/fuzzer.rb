...
module Msf
  ...
  module Auxiliary::Fuzzer
    ...
    def fuzz_numbers
      res = []
      methods.sort.grep(/^fuzzer_number/).each do |m|
        @last_fuzzer_input = m
        block_given? ? send(m) { |x| yield(x) } : (res << send(m))
      end
      res
    end
    ...
    def fuzz_strings
      res = []
      methods.sort.grep(/^fuzzer_string/).each do |m|
        @last_fuzzer_input = m
        block_given? ? send(m) { |x| yield(x) } : (res << send(m))
      end
      res
    ...
    def fuzzer_gen_string(len)
      ...
      res = ''
      res += @gen_string_block while res.length < len
      res[0, len]
    end
