...
module Msf
  module Auxiliary::Web
    module Analysis::Timing
      ...
      def responsive_async?(timeout = 120)
        submit_async(timeout: timeout) { |r| yield !r.timed_out? }
      end

      def if_responsive(timeout = 120)
        responsive_async?(timeout) { |b| yield if b }
      end

      def if_unresponsive(timeout = 120)
        responsive_async?(timeout) { |b| yield unless b }
      end
      end
  end
end
