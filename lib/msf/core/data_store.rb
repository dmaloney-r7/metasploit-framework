...
module Msf
  ...
  class DataStore < Hash
      ...
      unless opt.nil?
        if opt.validate_on_assignment?
          unless opt.valid?(v, check_empty: false)
            raise OptionValidateError, ["Value '#{v}' is not valid for option '#{k}'"]
          end
          v = opt.normalize(v)
        end
      end
    ...
    def import_options_from_s(option_str, delim = nil)
      ...
      if delim.nil?
        ...
        if (option_str.split('=').length <= 2) || !option_str.index(',').nil?
          delim = ','
        end
      end

      # Split on the delimeter
      option_str.split(delim).each do |opt|
        var, val = opt.split('=')
    ...
    def from_file(path, name = 'global')
      ...
      import_options_from_hash(ini[name], false) if ini.group?(name)
    end
    ...
    def clear_non_user_defined
      @imported.delete_if do |k, v|
        if v && (@imported_by[k] != 'self')
          delete(k)
    ...
    def find_key_case(k)
      ...
      each_key do |rk|
        return rk if rk.casecmp(k.downcase).zero?
      end
  ...
  class ModuleDataStore < DataStore
    ...
    def fetch(key)
      ...
      if val.nil? && @_module && @_module.framework
    ...
    def copy
      ...
      keys.each do |k|
        clone.import_option(k, self[k].is_a?(String) ? self[k].dup : self[k], @imported[k], @imported_by[k])
      end
      clone
    end
  end
