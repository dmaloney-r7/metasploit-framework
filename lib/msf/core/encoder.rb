...
module Msf
  ...
  class EncoderState
    ...
    #
    def buf=(buf)
      @orig_buf = buf if @orig_buf.nil? || @buf.nil?
      @buf = buf
    end
  ...
  class Encoder < Module
    ...
    def encode(buf, badchars = nil, state = nil, platform = nil)
      ...
      badchars = '' unless badchars

      # Initialize the encoding state and key as necessary
      state = EncoderState.new if state.nil?
      ...
      if decoder_key_size &&
        ...
        if state.key.nil?
          raise NoKeyError, "A key could not be found for the #{name} encoder.", caller
        end
      end
      ...
      dlog("#{name} result: #{state.encoded}") if arch?(ARCH_CMD)
    ...
    def do_encode(state)
      ...
      if !state.key.nil? && state.decoder_key_offset
      ...
      if (badchar_idx = has_badchars?(state.encoded, state.badchars)) != nil
        raise BadcharError.new(state.encoded, badchar_idx, stub.length, state.encoded[badchar_idx]),
              "The #{name} encoder failed to encode without bad characters.",
              caller
      end
    ...
    #
    def encode_finalize_stub(_state, stub)
      stub
    end
    ...
    def obtain_key(buf, badchars, state)
      if datastore['EnableContextEncoding']
        find_context_key(buf, badchars, state)
      else
        find_key(buf, badchars, state)
      end
    end
    ...
    #
    def find_key(buf, badchars, _state)
      ...
      until found
        ...
        0.upto(decoder_key_size - 1) do |index|
        ...
        key_bytes.each do |byte|
          found = false unless badchars.index(byte.chr).nil?
        end
      ...
      return nil if key_bytes.length != decoder_key_size

      key_bytes_to_integer(key_bytes)
    end
    ...
    def find_context_key(buf, badchars, state)
      ...
      unless File.exist?(datastore['ContextInformationFile'])
      ...
      File.open(datastore['ContextInformationFile']) do |f|
        begin
          ...
          while key.nil?
            ...
            while data.length > decoder_key_size
              ...
              # My ruby is rusty
              data[0, decoder_key_size].each_byte do |b|
                key_bytes << b
              end
              ...
              if find_key_verify(buf, key_bytes, badchars)
                ...
                invalid_key = false
                address_bytes.each do |byte|
                  invalid_key = true if badchars.index(byte.chr)
                end
    ...
    #
    def find_bad_keys(_buf, _badchars)
      Array.new(decoder_key_size) { Hash.new }
    end
    ...
    def key_bytes_to_integer(key_bytes)
      key_bytes_to_buffer(key_bytes).unpack(decoder_key_pack)[0]
    end
    ...
    def key_bytes_to_buffer(key_bytes)
      key_bytes.pack('C*')[0, decoder_key_size]
    end
    ...
    def integer_to_key_bytes(integer)
      [ integer.to_i ].pack(decoder_key_pack).unpack('C*')[0, decoder_key_size]
    end

    #
    # Determines if the key selected by find_key is usable
    #
    def find_key_verify(_buf, _key_bytes, _badchars)
      true
    end
  end
end
