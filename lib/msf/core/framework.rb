...
module Msf
  ...
  class FrameworkEventSubscriber
    ...
    def report_event(data)
      framework.db.report_event(data) if framework.db.active
    end
    ...
    # :category: ::Msf::UiEventSubscriber implementors
    def on_ui_stop
      report_event(name: "ui_stop") if framework.db.active
    end
    ...
    def session_event(name, session, opts = {})
      ...
      unless address && !address.empty?
        elog("Session with no session_host/target_host/tunnel_peer")
        dlog(session.inspect.to_s, LEV_3)
        return
      end
    ...
    def on_session_upload(session, lpath, rpath)
      ...
      framework.db.report_session_event(etype: 'upload',
                                        session: session,
                                        local_path: lpath,
                                        remote_path: rpath)
    end
    ...
    def on_session_download(session, rpath, lpath)
      ...
      framework.db.report_session_event(etype: 'download',
                                        session: session,
                                        local_path: lpath,
                                        remote_path: rpath)
    end
    ...
    def on_session_command(session, command)
      ...
      framework.db.report_session_event(etype: 'command',
                                        session: session,
                                        command: command)
    end
    ...
    def on_session_output(session, output)
      ...
      chunks = []
      if buff.length > 1024
        chunks << buff.slice!(0, 1024) until buff.empty?
      else
        chunks << buff
      end
      chunks.each do |chunk|
        session_event('session_output', session, output: chunk)
        framework.db.report_session_event(etype: 'output',
                                          session: session,
                                          output: chunk)
      end
    end
    ...
    def on_session_script_run(session, script)
      framework.db.report_session_event(etype: 'script_run',
                                        session: session,
                                        local_path: script)
    end
    ...
    def on_session_module_run(session, mod)
      framework.db.report_session_event(etype: 'module_run',
                                        session: session,
