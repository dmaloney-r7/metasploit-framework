    ...
    class RPC_Session < RPC_Base
      ...
      def rpc_list
        ...
        framework.sessions.each do |sess|
          ...
          res[s.sid]['platform'] = s.platform.to_s if s.type.to_s == "meterpreter"
        end
        res
      end
      ...
      def rpc_stop(sid)
        ...
        error(500, "Unknown Session ID") unless s
        begin
          s.kill
        rescue
          nil
      ...
      def rpc_shell_read(sid, ptr = nil)
        ...
        unless ring_buffer["seq"].nil? || ring_buffer["seq"].empty?
      ...
      def rpc_meterpreter_read(sid)
        ...
        unless s.user_output.respond_to? :dump_buffer
      ...
      def rpc_compatible_modules(sid)
        ...
        names.each do |mname|
          ...
          next unless m.session_compatible?(sid)
          ret << m.fullname
        end
        { "modules" => ret }
      end
      ...
      def _valid_session(sid, type)
        ...
        error(500, "Unknown Session ID #{sid}") unless s

        if type == "ring"
          unless s.respond_to?(:ring)
