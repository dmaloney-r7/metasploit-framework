  ...
  module RPC
    class RPC_Db < RPC_Base
      ...
      def find_workspace(wspace = nil)
        if wspace && (wspace != "")
          return framework.db.find_workspace(wspace) || error(500, "Invalid workspace")
        end
      ...
      def db_check
        error(500, "Database Not Loaded") unless db
      end
        ...
        ::ActiveRecord::Base.connection_pool.with_connection do
          ...
          if opts[:proto] && opts[:port]
            ...
            services.each do |s|
              nret = nil
              nret = if opts[:ntype]
                       s.notes.find_by_ntype(opts[:ntype])
                     else
                       s.notes
                     end
              next if nret.nil?
      ...
      def rpc_current_workspace
        db_check
        { "workspace" => framework.db.workspace.name, "workspace_id" => framework.db.workspace.id }
      end
      ...
      def rpc_report_note(xopts)
        ::ActiveRecord::Base.connection_pool.with_connection do
          opts, wspace = init_db_opts_workspace(xopts)
          if (opts[:host] || opts[:address]) && opts[:port] && opts[:proto]
      ...
      def rpc_del_vuln(xopts)
        ::ActiveRecord::Base.connection_pool.with_connection do
          ...
          if opts[:port] || opts[:proto]
            services = if opts[:host] || opts[:address] || opts[:addresses]
                         opts_to_services(hosts, xopts)
                       else
                         opts_to_services([], xopts)
                       end
          end
          ...
          if opts[:port] || opts[:proto]
            services.each do |s|
              vret = nil
              vret = if opts[:name]
                       s.vulns.find_by_name(opts[:name])
                     else
                       s.vulns
                     end
              next if vret.nil?
          ...
          elsif opts[:address] || opts[:host] || opts[:addresses]
            hosts.each do |h|
              vret = nil
              vret = if opts[:name]
                       h.vulns.find_by_name(opts[:name])
                     else
                       h.vulns
                     end
              next if vret.nil?
            ...
            vret = nil
            vret = if opts[:name]
                     wspace.vulns.find_by_name(opts[:name])
                   else
                     wspace.vulns
                   end
            vulns << vret if vret.class == ::Mdm::Vuln
      ...
      def rpc_del_service(xopts)
        ::ActiveRecord::Base.connection_pool.with_connection do
          ...
          if opts[:host] || opts[:address]
            host = opts[:host] || opts[:address]
            hent = wspace.hosts.find_by_address(host)
            return { result: 'failed' } if hent.nil? || (hent.class != ::Mdm::Host)
            hosts << hent
          elsif opts[:addresses]
          ...
          if opts[:addresses] || opts[:address] || opts[:host]
            hosts.each do |h|
              sret = nil
              if opts[:port] || opts[:proto]
                conditions = {}
            ...
            conditions[:proto] = opts[:proto] if opts[:proto]
            sret = wspace.services.where(conditions)
            services << sret if sret && (sret.class == ::Mdm::Service)
            services |= sret if sret && (sret.class == Array)
          end
      ...
      def rpc_del_client(xopts)
        ::ActiveRecord::Base.connection_pool.with_connection do
          ...
          hosts = if opts[:host] || opts[:address] || opts[:addresses]
                    opts_to_hosts(xopts)
                  else
                    wspace.hosts
                  end
      ...
      def rpc_connect(xopts)
        opts = fix_options(xopts)
        return { result: 'failed' } if !framework.db.driver && !(opts[:driver])
        ...
        return { result: 'failed' } unless framework.db.connect(opts)
        { result: 'success' }
      end
      ...
      #  rpc.call('db.status')
      def rpc_status
        return { driver: 'None' } unless framework.db.driver

        cdb = ""
        if framework.db.connection_established?
          ::ActiveRecord::Base.connection_pool.with_connection do |conn|
            cdb = if conn.respond_to? :current_database
                    conn.current_database
                  else
                    conn.instance_variable_get(:@config)[:database]
                  end
          end
