  ...
  module RPC
    class RPC_Auth < RPC_Base
      ...
      def rpc_login_noauth(user, pass)
        error(401, "Login Failed") unless user.is_a?(::String) && pass.is_a?(::String)
        ...
        service.tokens[token] = [user, Time.now.to_i, Time.now.to_i]
      ...
      def rpc_logout(token)
        found = service.tokens[token]
        error("500", "Invalid Authentication Token") unless found
        error("500", "Permanent Authentication Token") if found[3] == true

        # Delete the token if its not marked as permanent
        service.tokens.delete(token)
      ...
      def rpc_token_add(token)
        ...
        service.tokens[token] = [nil, nil, nil, true] unless db
      ...
      def rpc_token_generate
        ...
        unless db
          ...
          service.tokens[token] = [nil, nil, nil, true]
        end
      ...
      def db_validate_auth(user, pass)
        return true unless framework.db && framework.db.active
        return true unless @@loaded_sha2

        user_info = ::Mdm::User.find_by_username(user)
        return true unless user_info
