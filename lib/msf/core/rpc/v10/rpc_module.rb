    ...
    class RPC_Module < RPC_Base
      ...
      def rpc_info(mtype, mname)
        ...
        if m.type == "auxiliary"
          ...
          res['default_action'] = m.default_action if m.default_action
        end
      ...
      def rpc_options(mtype, mname)
        ...
        m.options.each_key do |k|
          ...
          res[k]['default'] = o.default unless o.default.nil?

          res[k]['enums'] = o.enums if o.enums.length > 1
        end
      ...
      def rpc_encode(data, encoder, options)
        ...
        badchars = ''
        badchars = options['badchars'] if options['badchars']
        ...
        ecount = 1
        ecount = options['ecount'].to_i if options['ecount']
        ...
        begin
          ...
          1.upto(ecount) do |_iteration|
            # Encode it up
            raw = enc.encode(raw, badchars, nil, platform)
          end

          output = Msf::Util::EXE.to_executable_fmt(framework, arch, platform, raw, fmt, exeopts)
      ...
      def _run_exploit(mod, opts)
        s = Msf::Simple::Exploit.exploit_simple(mod, 'Payload' => opts['PAYLOAD'],
                                                     'Target'   => opts['TARGET'],
                                                     'RunAsJob' => true,
                                                     'Options'  => opts)
        {
      ...
      def _run_auxiliary(mod, opts)
        Msf::Simple::Auxiliary.run_simple(mod, 'Action' => opts['ACTION'],
                                               'RunAsJob' => true,
                                               'Options'  => opts)
        {
          "job_id" => mod.job_id,
      ...
      def _run_post(mod, opts)
        Msf::Simple::Post.run_simple(mod, 'RunAsJob' => true,
                                          'Options' => opts)
        {
          "job_id" => mod.job_id,
          "uuid" => mod.uuid
      ...
      def _run_payload(mod, opts)
        ...
        begin
          res = Msf::Simple::Payload.generate_simple(mod, 'BadChars' => badchars,
