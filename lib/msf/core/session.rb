...
module Msf
  ...
  module Session
    ...
    #
    def inspect
      "#<Session:#{type} #{tunnel_peer} (#{session_host}) #{info ? "\"#{info}\"" : nil}>" # " Fixes highlighting
    end
    ...
    def set_from_exploit(m)
      ...
      via['Payload'] = ('payload/' + m.datastore['PAYLOAD'].to_s) if m.datastore['PAYLOAD']
      self.target_host = Rex::Socket.getaddress(m.target_host) unless m.target_host.to_s.strip.empty?
      ...
      self.exploit = m
      self.exploit_task = m[:task] if m[:task]
    end
    ...
    #
    def cleanup
      if db_record && framework.db.active
