...
module Msf::DBManager::Web
  ...
  def report_web_form(opts)
    ...
    ::ActiveRecord::Base.connection_pool.with_connection do
      ...
      unless %w(GET POST PATH).include?(meth)
        raise ArgumentError, "report_web_form requires the method to be one of GET, POST, PATH"
      end

      if opts[:web_site] && opts[:web_site].is_a?(::Mdm::WebSite)
  ...
  def report_web_page(opts)
    ...
    ::ActiveRecord::Base.connection_pool.with_connection do
      ...
      page.ctype = if opts[:ctype].blank? || opts[:ctype] == [""]
                     ""
                   else
                     opts[:ctype]
                 end
  ...
  def report_web_site(opts)
    ...
    ::ActiveRecord::Base.connection_pool.with_connection do |_conn|
      ...
      if opts.keys.include?(:ssl) || serv.name.to_s.empty?
        name = opts[:ssl] ? 'https' : 'http'
        serv.name = name
      end
      # Add the info if it's there.
      serv.info = info unless info.to_s.empty?
