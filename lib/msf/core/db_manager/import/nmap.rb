...
module Msf::DBManager::Import::Nmap
  def import_nmap_noko_stream(args, &block)
    doc = if block
            Rex::Parser::NmapDocument.new(args, framework.db) { |type, data| yield type, data }
          else
            Rex::Parser::NmapDocument.new(args, self)
          end
  ...
  def import_nmap_xml(args = {}, &block)
    return nil if args[:data].nil? || args[:data].empty?
    ...
    parser.on_found_host = proc { |h|
      ...
      if h["os_vendor"]
        ...
        note[:data][:os_match] = h['os_match'] if h["os_match"]
      ...
      h["ports"].each do |p|
        ...
        data = {}
        data[:workspace] = wspace
        data[:proto] = if fix_services
                         nmap_msf_service_map(p["protocol"])
                       else
                         p["protocol"].downcase
                       end
        ...
        data[:state] = p["state"]
        data[:host]  = hobj || addr
        data[:info]  = extra unless extra.empty?
        data[:task]  = args[:task]
      ...
      h["scripts"]&.each do |key, val|
        next unless key == "smb-check-vulns"
