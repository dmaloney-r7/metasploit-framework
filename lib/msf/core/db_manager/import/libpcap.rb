...
module Msf::DBManager::Import::Libpcap
  ...
  def import_libpcap(args = {}, &block)
    ...
    data.read_packet_bytes do |p|
      if (decoded_packets >= last_count + 1000) && block
      ...
      pkt = begin
              PacketFu::Packet.parse(p)
            rescue
              next
            end # Just silently skip bad packets
      ...
      unless bl.include?(saddr) || rfc3330_reserved(saddr)
        yield(:address, saddr) if block && !seen_hosts.keys.include?(saddr)
      ...
      unless bl.include?(daddr) || rfc3330_reserved(daddr)
        yield(:address, daddr) if block && !seen_hosts.keys.include?(daddr)
      ...
      if pkt.is_tcp? # First pass on TCP packets
        if ((pkt.tcp_flags.syn == 1) && (pkt.tcp_flags.ack == 1)) || # Oh, this kills me
        ...
        if pkt.udp_src == pkt.udp_dst # Very basic p2p detection.
          [saddr, daddr].each do |xaddr|
            next unless seen_hosts[xaddr]
            next if seen_hosts[xaddr].include? [pkt.udp_src, "udp"]
            report_service(
  ...
  def inspect_single_packet(pkt, wspace, args)
    inspect_single_packet_http(pkt, wspace, args) if pkt.is_tcp? || pkt.is_udp?
  end
  ...
  def inspect_single_packet_http(pkt, wspace, args)
    ...
    if pkt.is_tcp? && (pkt.tcp_src == 80) && !pkt.payload.nil? && !pkt.payload.empty?
      if pkt.payload =~ /^HTTP\x2f1\x2e[01]/n
        http_server_match = pkt.payload.match(/\nServer:\s+([^\r\n]+)[\r\n]/n)
        if http_server_match.is_a?(MatchData) && http_server_match[1]
          report_service(
            workspace: wspace,
            host: pkt.ip_saddr,
            port: pkt.tcp_src,
    ...
    if pkt.is_tcp? && (pkt.tcp_dst == 80) && !pkt.payload.nil? && !pkt.payload.empty?
      if pkt.payload =~ /[\x00-\x20]HTTP\x2f1\x2e[10]/n
        auth_match = pkt.payload.match(/\nAuthorization:\s+Basic\s+([A-Za-z0-9=\x2b]+)/n)
        if auth_match.is_a?(MatchData) && auth_match[1]
