...
module Msf::DBManager::Import::Nexpose::Simple
  def import_nexpose_noko_stream(args, &block)
    doc = if block
            Rex::Parser::NexposeSimpleDocument.new(args, framework.db) { |type, data| yield type, data }
          else
            Rex::Parser::NexposeSimpleDocument.new(args, self)
          end
