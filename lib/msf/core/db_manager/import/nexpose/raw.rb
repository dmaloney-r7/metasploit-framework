...
module Msf::DBManager::Import::Nexpose::Raw
  def import_nexpose_raw_noko_stream(args, &block)
    doc = if block
            Rex::Parser::NexposeRawDocument.new(args, framework.db) { |type, data| yield type, data }
          else
            Rex::Parser::NexposeRawDocument.new(args, self)
          end
  ...
  def nexpose_host_from_rawxml(h, vstructs, wspace, task = nil)
    ...
    if h["hardware-address"]
      ...
      data[:mac] = h["hardware-address"].delete(':').scan(/../).join(':')
    end
    ...
    # take the first one.
    data[:name] = h["names"].first if h["names"] && h["names"].first
    ...
    h["endpoints"].each do |p|
      ...
      data[:task]      = task
      data[:name] = p["name"] if p["name"] != "<unknown>"
      report_service(data)
    end

    h["vulns"].each_pair do |_k, v|
      next if v["status"] !~ /^vulnerable/
      vstruct = vstructs.select { |vs| vs.id.to_s.casecmp(v["id"].to_s.downcase).zero? }.first
      next unless vstruct
      data             = {}
      data[:workspace] = wspace
      data[:host]      = hobj || addr
  ...
  def nexpose_refs_to_struct(vulns)
    ret = []
    vulns.each do |vuln|
      next if ret.map(&:id).include? vuln["id"]
