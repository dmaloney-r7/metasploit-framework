...
module Msf::DBManager::Vuln
  ...
  #
  def each_vuln(wspace = workspace)
  ...
  def find_vuln_by_details(details_map, host, service = nil)
    ...
    vuln = service.vulns.includes(:vuln_details).where(crit).first if service
  ...
  def find_vuln_by_refs(refs, host, service = nil)
    ...
    unless service.try(:id).nil?
      return relation.where(service_id: service.try(:id), refs: { id: ref_ids }).first
    end
    relation.where(refs: { id: ref_ids }).first
  end

  def get_vuln(_wspace, host, service, name, _data = '')
    raise "Not workspace safe: #{caller.inspect}"
    ::ActiveRecord::Base.connection_pool.with_connection do
      vuln = nil
      vuln = if service
               ::Mdm::Vuln.find.where("name = ? and service_id = ? and host_id = ?", name, service.id, host.id).order("vulns.id DESC").first
             else
               ::Mdm::Vuln.find.where("name = ? and host_id = ?", name, host.id).first
             end
  ...
  def report_vuln(opts)
    ...
    raise ArgumentError, "Deprecated data column for vuln, use .info instead" if opts[:data]
    ...
    ::ActiveRecord::Base.connection_pool.with_connection do
      ...
      if opts[:refs]
        rids ||= []
        opts[:refs].each do |r|
          if r.respond_to?(:ctx_id) && r.respond_to?(:ctx_val)
      ...
      if service || opts[:port].to_i > 0
        ...
        # If there are multiple matches, choose the one with the most matches
        vuln = find_vuln_by_refs(rids, host) if rids && !rids.empty?
      end

      # Try to match based on vuln_details records
      if !vuln && opts[:details_match]
        vuln = find_vuln_by_details(opts[:details_match], host, service)
        vuln.service = service if vuln && service && !vuln.service
      end

      # No matches, so create a new vuln record
      unless vuln
        vuln = if service
                 service.vulns.find_by_name(name)
               else
                 host.vulns.find_by_name(name)
               end
      ...
      vuln.refs << (rids - vuln.refs) if rids
