...
module Msf::DBManager::Cred
  ...
  def each_cred(wspace = workspace)
    ::ActiveRecord::Base.connection_pool.with_connection do
  ...
  def report_auth_info(opts = {})
    ...
    if !opts[:host].is_a?(::Mdm::Host) && !validate_ips(opts[:host])
      raise ArgumentError, "Invalid address or object for :host (#{opts[:host].inspect})"
    end
