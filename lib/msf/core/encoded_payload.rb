...
module Msf
  ...
  class EncodedPayload
    ...
    def generate(raw = nil)
      ...
      Thread.current.priority = 1 if priority == 0
      ...
      begin
        ...
        if reqs['Encoder']
          ...
          encoder_str.scan(/([^:, ]+):?([^,]+)?/).map do |encoder_opt|
            ...
            self.iterations = 1 if iterations < 1
    ...
    def encode
      ...
      if !reqs['BadChars'].to_s.empty? || reqs['Encoder'] || reqs['ForceEncode']
        ...
        if reqs['Encoder'] && (preferred = framework.encoders[reqs['Encoder']])
        ...
        encoders.each do |encname, encmod|
          ...
          # options don't apply.
          if reqs['Exploit'] &&
             !reqs['Exploit'].compatible?(encoder) &&
          ...
          if reqs['EncoderType'] &&
             (encoder.encoder_type.split(/\s+/).include?(reqs['EncoderType']) == false)
            wlog("#{pinst.refname}: Encoder #{encoder.refname} is not a compatible encoder type: #{reqs['EncoderType']} != #{encoder.encoder_type}",
                 'core', LEV_1)
            next
          end
          ...
          if reqs['EncoderType'].nil? &&
             reqs['Encoder'].nil? &&
             (encoder.rank == ManualRanking)
          ...
          if reqs['ForceSaveRegisters'] &&
             reqs['EncoderOptions'] &&
             !reqs['EncoderOptions']['SaveRegisters'].to_s.empty? &&
             !encoder.can_preserve_registers?
          ...
          encoder.share_datastore(pinst.datastore)

          # If we have any encoder options, import them into the datastore
          # of the encoder.
          if reqs['EncoderOptions']
            encoder.datastore.import_options_from_hash(reqs['EncoderOptions'])
          end
          ...
          begin
            ...
            wlog("#{pinst.refname}: Failed to validate encoder #{encoder.refname}: #{$ERROR_INFO}",
                 'core', LEV_1)
            next
          end

          # Tell the encoder how much space is available
          encoder.available_space = space
          ...
          1.upto(iterations) do |iter|
            ...
            begin
              eout = encoder.encode(eout, reqs['BadChars'], nil, pinst.platform)
            rescue EncodingError
              wlog("#{err_start}: Encoder #{encoder.refname} failed: #{$ERROR_INFO}", 'core', LEV_1)
              dlog("#{err_start}: Call stack\n#{$ERROR_POSITION.join("\n")}", 'core', LEV_3)
              next_encoder = true
              break

            rescue ::Exception
              elog("#{err_start}: Broken encoder #{encoder.refname}: #{$ERROR_INFO}", 'core', LEV_0)
              dlog("#{err_start}: Call stack\n#{$ERROR_POSITION.join("\n")}", 'core', LEV_1)
              next_encoder = true
              break
            end
            ...
            if reqs['Space'] && (reqs['Space'] < eout.length + min)
      ...
      self.encoded = (reqs['PrependEncoder'] || '') + encoded
      encoded << (reqs['AppendEncoder'] || '')
    end
    ...
    def generate_sled
      ...
      if space &&
        ...
        self.nop_sled_size = reqs['Space'] - encoded.length
      end

      # If the maximum number of NOPs has been exceeded, wrap it back down.
      if reqs['MaxNops'] &&
         (reqs['MaxNops'] < nop_sled_size)
      ...
      if nop_sled_size > 0
        ...
        nops.each do |_nopname, nopmod|
          ...
          begin
            ...
            dlog("#{pinst.refname}: Nop generator #{nop.refname} failed to generate sled for payload: #{$ERROR_INFO}",
                 'core', LEV_1)
    ...
    def encoded_exe(opts = {})
      ...
      opts[:arch] = [ opts[:arch] ] if opts[:arch] && (!opts[:arch].is_a? Array)
      if opts[:platform].is_a? Msf::Module::PlatformList
      ...
      if emod
        if emod.datastore["EXE::Custom"] && emod.respond_to?(:get_custom_exe)
        ...
        opts.merge!(template_path: emod.datastore['EXE::Path'],
