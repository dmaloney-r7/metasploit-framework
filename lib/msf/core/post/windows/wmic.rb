      ...
      module WMIC
        ...
        def wmic_query(query, server = datastore['RHOST'])
          ...
          if datastore['SMBUser']
            if server.casecmp("localhost").zero? || server.downcase.starts_with?('127.')
              raise "WMIC: User credentials cannot be used for local connections"
            end
          end
          ...
          if extapi && !is_system?
            ...
            result_text = if result && result[1] && result[1].key?('Text')
                            result[1]['Text']
                          else
                            ""
                          end
        ...
        def wmic_command(cmd, server = datastore['RHOST'])
          ...
          vprint_error("[#{server}] WMIC Command Error") if parsed_result.nil?
        ...
        def parse_wmic_result(result_text)
          if result_text.blank?
            ...
            pid = Regexp.last_match(1).to_i if result_text =~ /ProcessId = (\d+);/

            return_value = Regexp.last_match(1).to_i if result_text =~ /ReturnValue = (\d+);/

            { return: return_value, pid: pid, text: result_text }
          end
        end
