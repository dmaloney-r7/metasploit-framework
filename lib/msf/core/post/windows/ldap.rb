    ...
    module Windows
      ...
      module LDAP
        ...
        def query_ldap(session_handle, base, scope, filter, fields)
          ...
          max_search = if datastore['MAX_SEARCH'] == 0
                         search_count
                       else
                         [datastore['MAX_SEARCH'], search_count].min
                       end
          ...
          0.upto(max_search - 1) do |i|
            ...
            pEntries[i + 1] = if client.arch == ARCH_X64
                                entry[4]
                              else
                                entry[3]
                            end
        ...
        def get_entry(pEntry)
          client.railgun.memread(pEntry, 41).unpack('VVVVVVVVVvCCC')
        end
        ...
        def get_ber(msg)
          ...
          ber_data = if client.arch == ARCH_X64
                       client.railgun.memread(ber[4], ber[0])
                     else
                       client.railgun.memread(ber[3], ber[0])
                     end
