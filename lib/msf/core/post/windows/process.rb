    ...
    module Windows
      module Process
        ...
        def execute_shellcode(shellcode, base_addr = nil, pid = nil)
          ...
          shell_addr = if base_addr.nil?
                         host.memory.allocate(shellcode.length)
                       else
                         host.memory.allocate(shellcode.length, nil, base_addr)
                       end
          ...
          vprint_status("Creating the thread to execute in 0x#{shell_addr.to_s(16)} (pid=#{pid})")
