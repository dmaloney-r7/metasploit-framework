...
module Msf
  class Post
    module Windows
      ...
      #
      module Services
        START_TYPE = ["Boot", "System", "Auto", "Manual", "Disabled"].freeze
        START_TYPE_BOOT       = 0
        START_TYPE_SYSTEM     = 1
        START_TYPE_AUTO       = 2
        START_TYPE_MANUAL     = 3
        ...
        def close_service_handle(handle)
          advapi32.CloseServiceHandle(handle) if handle
        end
        ...
        def each_service(&block)
          if load_extapi
            ...
            keys.each do |sk|
              ...
              yield sk if srvtype == 32 || srvtype == 16
            end
        ...
        def service_list
          if load_extapi
            ...
            keys.each do |s|
              if a.length >= 10
                a.first.join
                a.delete_if { |x| !x.alive? }
              end
              t = framework.threads.spawn(refname + "-ServiceRegistryList", false, s) do |sk|
                begin
                  srvtype = registry_getvaldata("#{serviceskey}\\#{sk}", "Type").to_s
                  services << { name: sk } if (srvtype == "32") || (srvtype == "16")
                rescue
                end
              end
              a.push(t)
        ...
        def service_create(name, opts, server = nil)
          ...
          open_sc_manager(host: server, access: access) do |manager|
            ...
            close_service_handle(newservice["return"]) if newservice
