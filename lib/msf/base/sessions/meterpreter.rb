  ...
  module Sessions
    ...
    class Meterpreter < Rex::Post::Meterpreter::Client
      ...
      def initialize(rstream, opts = {})
        ...
        opts[:skip_ssl] = !supports_ssl? unless opts[:skip_ssl]
        ...
        if opts[:datastore] && opts[:datastore]['HandlerSSLCert']
      ...
      def is_valid_session?(timeout = 10)
        return true if machine_id

        begin
          self.machine_id = core.machine_id(timeout)
          ...
          dlog("Session #{sid} did not respond to validation request #{e.class}: #{e}")
        end
        false
      end

      def update_session_info
        username = sys.config.getuid
        ...
        # subset of platforms.
        if ['java', 'python', 'php'].include?(platform)
          new_platform = guess_target_platform(sysinfo['OS'])
          if platform != new_platform
            payload_uuid.platform = new_platform
            core.set_uuid(payload_uuid)
          end
        end
      ...
      def load_session_info
        ::Timeout.timeout(60) do
          ...
          original_session_host = session_host
          ...
          ::ActiveRecord::Base.connection_pool.with_connection do
            ...
            if nhost && db_record
              ...
              if hobj
                self.session_host = nhost
                db_record.host_id = hobj[:id]
              end
            end

            framework.db.report_note(type: "host.os.session_fingerprint",
                                     host: self,
                                     workspace: wspace,
                                     data: {
            ...
            if nhost
              framework.db.report_note(type: "host.nat.server",
              ...
              framework.db.report_note(type: "host.nat.client",
                                       host: nhost,
                                       workspace: wspace,
      ...
      def find_internet_connected_address
        ifaces = begin
                   net.config.get_interfaces.flatten
                 rescue
                   []
                 end
        routes = begin
                   net.config.get_routes.flatten
                 rescue
                   []
                 end
        ...
        unless found
          ...
          default_routes.each do |route|
            ...
            ifaces.each do |i|
              ...
              addr_and_mask = i.addrs.zip(i.netmasks).find do |addr, netmask|
                ...
                range = begin
                          Rex::Socket::RangeWalker.new("#{addr}/#{bits}")
                        rescue
                          nil
                        end
          ...
          unless nhost
            ...
            nhost = non_loopback.ip if non_loopback
          end
        end
