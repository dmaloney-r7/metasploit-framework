  ...
  module Sessions
    ...
    class CommandShell
      ...
      def shell_command(cmd)
        ...
        # reached.
        while ::Time.now.to_f < etime && (respond_to?(:ring) || ::IO.select([rstream], nil, nil, timeo))
      ...
      def shell_read(length = -1, timeout = 1)
        return shell_read_ring(length, timeout) if respond_to?(:ring)
      ...
      def shell_read_ring(length = -1, timeout = 1)
        ...
        if (self.ring_buff.length >= length) && length > 0
        ...
        begin
          ::Timeout.timeout(timeout) do
            while (length > 0 && buff.length < length) || ((length == -1) && buff.empty?)
              ring.select
              nseq, data = ring.read_data(ring_seq)
              if data
                self.ring_seq = nseq
                buff << data
              end
        ...
        if length > 0 && buff.length > length
      ...
      def _interact_stream
        ...
        while interacting
          ...
          user_output.print(shell_read) if sd[0].include? rstream.fd
          shell_write(user_input.gets) if sd[0].include? user_input.fd
          Thread.pass
        end
      end
