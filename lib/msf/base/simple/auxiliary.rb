  ...
  module Simple
    ...
    module Auxiliary
      ...
      def self.run_simple(omod, opts = {})
        ...
        # Verify the ACTION
        if !mod.actions.empty? && !mod.action
          raise MissingActionError, "Please use: #{mod.actions.collect(&:name) * ', '}"
        end
        ...
        ctx = [ mod ]
        if mod.passive? || opts['RunAsJob']
          mod.job_id = mod.framework.jobs.start_bg_job(
            "Auxiliary: #{mod.refname}",
            ctx,
            proc { |ctx_| job_run_proc(ctx_) },
            proc { |ctx_| job_cleanup_proc(ctx_) }
          )
      ...
      def self.check_simple(mod, opts)
        mod.init_ui(opts['LocalInput'], opts['LocalOutput']) if opts['LocalInput']
