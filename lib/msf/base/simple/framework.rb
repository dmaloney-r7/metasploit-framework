    ...
    module Framework
      ...
      module PluginManager
        ...
        def load(path, opts = {})
          ...
          if File.exist?(def_path) || File.exist?(def_path + ".rb")
            super(def_path, opts)
          else
            super
          end
      ...
      def self.simplify(framework, opts)
        ...
        if framework.is_a?(Msf::Simple::Framework) == false
        ...
        framework.init_module_paths unless opts['DeferModuleLoads']
      ...
      def self.simplify_module(instance, load_saved_config = true)
        if (ModuleSimplifiers[instance.type]) &&
      ...
      #
      def load_config
        datastore.from_file(Msf::Config.config_file, 'framework/core')
      end
      ...
      #
      def save_config
        datastore.to_file(Msf::Config.config_file, 'framework/core')
      end
