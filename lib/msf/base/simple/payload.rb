    ...
    module Payload
      ...
      def self.generate_simple(payload, opts)
        ...
        unless output
          ...
          # Prepend a comment
          if (fmt != 'raw') && (opts['NoComment'] != true)
            (ou = payload.options.options_used_to_s(payload.datastore)) && !ou.empty? ? ou += "\n" : ou = ''
            output =
              Buffer.comment(
