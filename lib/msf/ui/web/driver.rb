    ...
    module Web
      ...
      class Driver < Msf::Ui::Driver
        ...
        def create_console(opts = {})
          ...
          console = WebConsole.new(framework, last_console, opts)
          self.last_console += 1
          consoles[console.console_id.to_s] = console
        ...
        def write_console(id, buf)
          consoles[id] ? consoles[id].write(buf) : nil
        end

        def read_console(id)
          consoles[id] ? consoles[id].read : nil
        end

        def clean_consoles(timeout = 300)
          consoles.each_pair do |id, con|
        ...
        def write_session(id, buf)
          ses = framework.sessions[id]
          return unless ses
          return unless ses.user_input
