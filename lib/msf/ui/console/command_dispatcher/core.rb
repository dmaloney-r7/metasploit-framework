      ...
      module CommandDispatcher
        ...
        class Core
          ...
          def cmd_connect(*args)
            ...
            if fileval
              begin
                ...
                print_error("Can't read from '#{fileval}': #{$ERROR_INFO}")
                return false
              end
            end
            ...
            if commval
              ...
              unless comm
                ...
                comm = session if session.is_a?(Msf::Session::Comm)
              end
            ...
            begin
              ...
              print_error("Unable to connect: #{$ERROR_INFO}")
              return false
            end
          ...
          def cmd_irb(*args)
            ...
            if expressions.empty?
              ...
              begin
                ...
                print_error("Error during IRB: #{$ERROR_INFO}\n\n#{$ERROR_POSITION.join("\n")}")
              end
          ...
          def cmd_threads(*args)
            ...
            # Parse the command options
            @@threads_opts.parse(args) do |opt, _idx, val|
              case opt
              when "-v"
                verbose = true
              when "-l"
          ...
          def cmd_threads_tabs(_str, words)
            return @@threads_opts.fmt.keys if words.length == 1

            if (words.length == 2) && (@@threads_opts.fmt[words[1]] || [false])[0]
              return framework.threads.each_index.map(&:to_s)
            end
          ...
          def cmd_route(*args)
            ...
            when "add", "remove", "del"
              ...
              when /^(-1|[0-9]+)$/
                ...
                if session.is_a?(Msf::Session::Comm)
                  gateway = session
                elsif session.nil?
            ...
            when "print"
              ...
              Rex::Socket::SwitchBoard.each do |route|
                gw = if route.comm.is_a?(Msf::Session)
                       "Session #{route.comm.sid}"
                     else
                       route.comm.name.split(/::/)[-1]
                     end
          ...
          def cmd_route_tabs(_str, words)
            return %w(add remove get flush print) if words.length == 1
            ...
            when "remove", "del"
              Rex::Socket::SwitchBoard.each do |route|
                ...
                when 2
                  ret << route.subnet
                when 3
                  ret << route.netmask if route.subnet == words[2]
                when 4
                  if route.subnet == words[2]
                    ret << route.comm.sid.to_s if route.comm.is_a? Msf::Session
                  end
                end
              end
              ret
            when "add"
              ...
              ret = framework.sessions.keys.map(&:to_s) if words.length == 4
          ...
          def cmd_save(*_args)
            ...
            begin
              ...
              log_error("Save failed: #{$ERROR_INFO}")
              return false
            end
            ...
            begin
              ...
              @@sessions_opts.parse(args) do |opt, _idx, val|
                ...
                when "-h"
                  ...
                  return false
                when "-t"
                  response_timeout = val.to_i if val.to_s =~ /^\d+$/
                else
                  extra << val
                end
              end

              method = 'interact' if !method && sid
                    ...
                    begin
                      if session.type == 'meterpreter'
                        ...
                        begin
                          ...
                          print_error("Failed: #{$ERROR_INFO.class} #{$ERROR_INFO}")
              ...
              log_error("Session manipulation failed: #{$ERROR_INFO} #{$ERROR_INFO.backtrace.inspect}")
            end
          ...
          def cmd_sessions_tabs(_str, words)
            return @@sessions_opts.fmt.keys if words.length == 1
          ...
          def cmd_set_tabs(str, words)
            ...
            return tab_complete_option(str, words) if words.length == 2
            ...
            res << 'ENCODER' if mod.exploit? || mod.payload?
          ...
          def cmd_unload(*args)
            ...
            framework.plugins.each do |plugin|
              ...
              next unless plugin.name.casecmp(args[0].downcase).zero?
          ...
          def cmd_unload_tabs(_str, words)
          ...
          def cmd_get(*args)
            ...
            # Determine which data store we're operating on
            datastore = if active_module && !global
          ...
          def cmd_getg_tabs(_str, _words)
            framework.datastore.keys
          end
          ...
          def cmd_unset(*args)
            ...
            # Determine which data store we're operating on
            datastore = if active_module && (global == false)
                          active_module.datastore
                        else
                          framework.datastore
                        end
          ...
          def cmd_unset_tabs(_str, _words)
            datastore = active_module ? active_module.datastore : framework.datastore
            datastore.keys
          end
          ...
          def cmd_unsetg_tabs(_str, _words)
            framework.datastore.keys
          end
          ...
          def cmd_grep(*args)
            ...
            @@grep_opts.parse(args.dup) do |opt, _idx, val|
              case opt
              when "-h"
            ...
            statement = if match_mods[:invert]
                          'not line =~ rx'
                        else
                          'line =~ rx'
                        end
            ...
            all_lines.each_with_index do |line, line_num|
              next if output_mods[:skip] && line_num < output_mods[:skip]
              our_lines << line if output_mods[:keep] && line_num < output_mods[:keep]
              # we don't wan't to keep processing if we have a :max and we've reached it already (not counting skips/keeps)
              break if match_mods[:max] && (count >= match_mods[:max])
              next unless eval statement
              count += 1
          ...
          def tab_complete_option(str, words)
            ...
            if mod.exploit?
              return option_values_payloads if opt.casecmp('PAYLOAD').zero?
              return option_values_targets  if opt.casecmp('TARGET').zero?
              return option_values_nops     if opt.casecmp('NOPS').zero?
              return option_values_encoders if opt.casecmp('STAGEENCODER').zero?
            end

            # Well-known option names specific to modules with actions
            if mod.is_a?(Msf::Module::HasActions)
              return option_values_actions if opt.casecmp('ACTION').zero?
            end

            # The ENCODER option works for payloads and exploits
            if (mod.exploit? || mod.payload?) && opt.casecmp('ENCODER').zero?
              return option_values_encoders
            end

            # Well-known option names specific to post-exploitation
            if mod.post? || mod.exploit?
              return option_values_sessions if opt.casecmp('SESSION').zero?
            end
            ...
            if mod.exploit? && mod.datastore['PAYLOAD']
              p = framework.payloads.create(mod.datastore['PAYLOAD'])
              if p && p.options.include?(opt)
                res.concat(option_values_dispatch(p.options[opt], str, words))
              elsif p && p.options.include?(opt.upcase)
          ...
          def option_values_dispatch(o, str, words)
            ...
            when Msf::OptPort
              ...
              res << (rand(65534) + 1).to_s if res.empty?
          ...
          def option_values_sessions
            active_module.compatible_sessions.map(&:to_s)
          end
          ...
          def option_values_target_addrs
            res = [ ]
            res << Rex::Socket.source_address
            return res unless framework.db.active
            ...
            if mport
              ...
              framework.db.each_service(framework.db.workspace) do |service|
                hosts[ service.host.address ] = true if service.port == mport
              end
          ...
          def option_values_target_ports
            res = [ ]
            return res unless framework.db.active
            return res unless active_module.datastore['RHOST']
            host = framework.db.has_host?(framework.db.workspace, active_module.datastore['RHOST'])
            return res unless host

            framework.db.each_service(framework.db.workspace) do |service|
              res << service.port.to_s if service.host_id == host.id
            end
          ...
          def binary_install
            ...
            binary_paths.include? Msf::Config.install_root
          end
