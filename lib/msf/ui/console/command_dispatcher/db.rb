    ...
    module Console
      module CommandDispatcher
        class Db
          ...
          def cmd_workspace_tabs(_str, words)
            return [] unless active?
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
              ...
              while (arg = args.shift)
                ...
                when '-c'
                  ...
                  col_search.each do |c|
                    next unless !default_columns.include?(c) && !virtual_columns.include?(c)
                ...
                when '-h', '--help'
                  ...
                  return unless arg_host_range(arg, host_ranges)
                end
              end

              col_names = if col_search
                            col_search
                          else
                            default_columns + virtual_columns
                          end
              ...
              if mode == [:new_info]
                ...
                return
              elsif mode.include?(:tag) && mode.include?(:delete)
                ...
                framework.db.hosts(framework.db.workspace, onlyup, host_search).each do |host|
                  ...
                  columns = col_names.map do |n|
                    ...
                    if virtual_columns.include?(n)
                      case n
                      when "svcs" then      host.services.length
                      when "vulns" then     host.vulns.length
                      when "workspace" then host.workspace.name
                      when "tags"
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
              ...
              while (arg = args.shift)
                ...
                when '-c'
                  ...
                  col_search = list.strip.split(",")
                  col_search.each do |c|
                    unless default_columns.include? c
                      print_error("Invalid column list. Possible values are (#{default_columns.join('|')})")
                      return
                    end
                  end
                when '-p'
                  return unless arg_port_range(args.shift, port_ranges, true)
                when '-r'
                  proto = args.shift
                  unless proto
              ...
              if mode == :add
                ...
                host_ranges.each do |range|
                  range.each do |addr|
                    ...
                    info[:name]  = names.first.downcase if names && names.first
              ...
              col_names = col_search if col_search
              tbl = Rex::Text::Table.new('Header' => "Services",
              ...
              each_host_range_chunk(host_ranges) do |host_search|
                framework.db.hosts(framework.db.workspace, false, host_search).each do |host|
                  host.vulns.each do |vuln|
                    if search_term
                      next unless
                        vuln.host.attribute_names.any? { |a| vuln.host[a.intern].to_s.match(search_term) } ||
                        vuln.attribute_names.any? { |a| vuln[a.intern].to_s.match(search_term) }

                    end
                    reflist = vuln.refs.map(&:name)
                    if vuln.service
                      # Skip this one if the user specified a port and it
                      # doesn't match.
                      next unless ports.empty? || ports.include?(vuln.service.port)
                      # Same for service names
                      next unless svcs.empty? || svcs.include?(vuln.service.name)
                      ...
                      next unless ports.empty? && svcs.empty?
                      print_status("Time: #{vuln.created_at} Vuln: host=#{host.address} name=#{vuln.name} refs=#{reflist.join(',')} #{show_info && vuln.info ? "info=#{vuln.info}" : ''}")
                    end
                    if set_rhosts
                      addr = (host.scope ? host.address + '%' + host.scope : host.address)
          ...
          def creds_add(private_type, username, password = nil, realm = nil, realm_type = nil)
            ...
            if realm.present?
              ...
              cred_data[:realm_value] = realm
              cred_data[:realm_key] = realm_key
            end
          ...
          def creds_search(*args)
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
              ...
              query = query.where(Mdm::Service[:name].in(svcs)) if svcs.present?

              query = query.where(Mdm::Service[:port].in(ports)) if ports.present?
              ...
              query.find_each do |core|
                ...
                next if user == "" && core.public && !core.public.username.blank?

                # Exclude non-blank password creds if that's what we're after
                next if pass == "" && core.private && !core.private.data.blank?

                origin = ''
                if core.origin.is_a?(Metasploit::Credential::Origin::Service)
                  origin = core.origin.service.host.address
                elsif core.origin.is_a?(Metasploit::Credential::Origin::Session)
                ...
                if core.logins.empty? && origin_ranges.empty?
                  ...
                  core.logins.each do |login|
                    ...
                    row << if login.service.name.present?
                             "#{login.service.port}/#{login.service.proto} (#{login.service.name})"
                           else
                             "#{login.service.port}/#{login.service.proto}"
                           end
          ...
          def cmd_creds_tabs(str, words)
            tabs = case words.length
                   when 1
                     # subcommands
                     [ 'add-ntlm', 'add-password', 'add-hash', 'add-ssh-key' ]
                   when 2
                     if words[1] == 'add-ssh-key'
          ...
          def cmd_notes(*args)
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
                ...
                note_list.sort_by! do |note|
                  ...
                  sort_terms.each do |term|
                    ...
                    if term.nil?
                      ...
                      orderlist << make_sortable(note.service.name) unless note.service.nil?
                    elsif term == "port"
                      orderlist << make_sortable(note.service.port) unless note.service.nil?
                    elsif term == "output"
              ...
              note_list.each do |note|
                next if types && types.index(note.ntype).nil?
                csv_note = []
                ...
                print_status(msg)
                csv_table << csv_note if out_file
                if mode == :delete
                  note.destroy
                  delete_count += 1
                end
              ...
              save_csv_notes(out_file, csv_table) if out_file
          ...
          def make_sortable(input)
            input = case input
                    ...
                    when Integer
                      "%016" % input
                    when Time
                      input.strftime("%Y%m%d%H%M%S%L")
                    when NilClass
                      ""
                    else
                      input.inspect.downcase
                    end
            input
          end
          ...
          def cmd_loot(*args)
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
              ...
              tbl = Rex::Text::Table.new('Header' => "Loot",
              ...
              each_host_range_chunk(host_ranges) do |host_search|
                framework.db.hosts(framework.db.workspace, false, host_search).each do |host|
                  host.loots.each do |loot|
                    next if types && types.index(loot.ltype).nil?
                    if search_term
                      next unless
                        loot.attribute_names.any? { |a| loot[a.intern].to_s.match(search_term) } ||
          ...
          def cmd_db_import(*args)
            return unless active?
            ::ActiveRecord::Base.connection_pool.with_connection do
              if args.include?("-h") || !(args && !args.empty?)
                cmd_db_import_help
                return
              end
              args.each do |glob|
                ...
                files.each do |filename|
                  ...
                  begin
                    ...
                    print_error("Failed to import #{filename}: #{$ERROR_INFO}")
                    elog("Failed to import #{filename}: #{$ERROR_INFO.class}: #{$ERROR_INFO}")
          ...
          def cmd_db_export(*args)
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
              ...
              exporter.send("to_#{format}_file".intern, output) do |mtype, mstatus, mname|
                if mtype == :status
                  print_status("    >> Starting export of #{mname}") if mstatus == "start"
                  if mstatus == "complete"
          ...
          def cmd_db_nmap(*args)
            ...
            ::ActiveRecord::Base.connection_pool.with_connection do
              ...
              begin
                ...
                if Rex::Compat.is_cygwin && nmap =~ /cygdrive/
          ...
          def cmd_db_nmap_tabs(_str, _words)
            nmap =
              Rex::FileUtils.find_full_path('nmap') ||
              Rex::FileUtils.find_full_path('nmap.exe')
          ...
          #
          def cmd_db_status(*_args)
            return unless db_check_driver

            if framework.db.connection_established?
              cdb = ''
              ::ActiveRecord::Base.connection_pool.with_connection do |conn|
                cdb = conn.current_database if conn.respond_to?(:current_database)
              end
          ...
          def cmd_db_connect(*args)
            ...
            if args[0] == "-y"
              if args[1] && (!::File.exist? ::File.expand_path(args[1]))
              ...
              if ::File.exist? file
                ...
                if framework.db.active && !framework.db.modules_cached
          ...
          def set_rhosts_from_addrs(rhosts)
            ...
            mydatastore = if active_module
                            active_module.datastore
                          else
          ...
          def db_find_tools(tools)
            missed = []
            tools.each do |name|
              missed << name unless Rex::FileUtils.find_full_path(name)
            end
            unless missed.empty?
          ...
          def db_connect_postgresql(*args)
            if args[0].nil? || (args[0] == "-h") || (args[0] == "--help")
            ...
            # Default to loopback
            opts['host'] = '127.0.0.1' unless opts['host']

            unless framework.db.connect(opts)
              raise "Failed to connect to the database: #{framework.db.error}"
            end
          end
          ...
          def db_parse_db_uri_postgresql(path)
            ...
            if path
              ...
              ((dest = auth) && (auth = nil)) unless dest
              res[:user], res[:pass] = auth.split(':') if auth
              targ, name = dest.split('/')
              ((name = targ) && (targ = nil)) unless name
          ...
          # blocks of 1024.
          #
          def each_host_range_chunk(host_ranges)
            ...
            host_ranges.each do |range|
              if range.nil? || range.length.nil?
                ...
                while chunk.length < 1024 && chunk.length < range.length
                  n = range.next_ip
                  if n.nil?
                    end_of_range = true
                    break
