    ...
    module Console
      ...
      class Driver < Msf::Ui::Driver
        ...
        def initialize(prompt = DefaultPrompt, prompt_char = DefaultPromptChar, opts = {})
          ...
          if framework.datastore['Prompt']
            prompt = framework.datastore['Prompt']
            prompt_char = framework.datastore['PromptChar'] || DefaultPromptChar
          end
          ...
          output = if opts['LocalOutput']
                     if opts['LocalOutput'].is_a?(String)
                       Rex::Ui::Text::Output::File.new(opts['LocalOutput'])
                     else
                       opts['LocalOutput']
                              end
                   else
          ...
          if framework.db.usable && !(opts['SkipDatabaseInit'])
          ...
          unless opts['Framework'] || opts['DeferModuleLoads']
            ...
            framework.init_module_paths(module_paths: opts['ModulePath'])
          end
          ...
          if opts['Resource'].blank?
            ...
            opts['Resource'].each do |r|
              load_resource(r)
            end
          end

          # Process any additional startup commands
          if opts['XCommands'] && opts['XCommands'].is_a?(Array)
            opts['XCommands'].each do |c|
              run_single(c)
            end
          end
        end
        ...
        def junit_error(tname, ftype, data = nil)
          raise "No output path, call junit_setup() first" unless @junit_output_path
        ...
        def load_preconfig
          begin
            ...
            wlog("Failed to load configuration: #{$ERROR_INFO}")
            return
          end

          if conf.group?(ConfigCore)
            conf[ConfigCore].each_pair do |k, v|
        ...
        def load_config(path = nil)
          ...
          if conf.group?(ConfigGroup)
            conf[ConfigGroup].each_pair do |k, v|
              case k.downcase
              when 'activemodule'
                run_single("use #{v}")
        ...
        def save_config
          ...
          begin
            ...
            print_error("Failed to save console config: #{$ERROR_INFO}")
          end
        end
          ...
          until lines.empty?
            ...
            if line =~ /<ruby/
              if line =~ /\s+binding=(?:'(\w+)'|"(\w+)")(>|\s+)/
                bin = ($LAST_MATCH_INFO[1] || $LAST_MATCH_INFO[2])
                bindings[bin] = binding unless bindings.key? bin
                bin = bindings[bin]
              else
                bin = binding
              end
        ...
        def on_startup(opts = {})
          ...
          self.on_command_proc = proc { |command| framework.events.on_ui_command(command) }
        end
        ...
        def on_variable_set(glob, var, val)
          ...
          when "payload"
            ...
            if framework && (framework.payloads.valid?(val) == false)
              false
        ...
        def unknown_command(method, line)
          ...
          [method, method + ".exe"].each do |cmd|
            next unless command_passthru && Rex::FileUtils.find_full_path(cmd)
