...
options_set_by_ancestor_reference_name = Hash.new do |hash, ancestor_reference_name|
  hash[ancestor_reference_name] = Set.new
end

framework.payloads.each do |reference_name, payload_class|
  module_ancestors = payload_class.ancestors.select do |ancestor|
    # need to use try because name may be nil for anonymous Modules
    ancestor.name.try(:start_with?, 'Msf::Modules::')
  end
  ancestor_reference_names = module_ancestors.map do |module_ancestor|
...
File.open('log/untested-payloads.log') do |f|
  f.each_line do |line|
    ...
    options_set.each do |options|
      ...
      next if tested_options.include? options
